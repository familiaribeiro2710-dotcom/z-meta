...
| `push_preferences` | PK `profile_id`, `advertencia`, `premiacao`, `lead_novo`, `venda_equipe`, `venda_loja`, `meta_batida` (todos boolean, default `true`), `updated_at` | 2026-07-20 — preferência de notificação **por usuário** (não por dispositivo). Cada papel só usa as chaves que de fato recebe (colaborador: `advertencia`/`premiacao`/`lead_novo` — este último só em consórcio; gerente: `venda_equipe`; sócio/supervisor: `venda_loja`/`meta_batida`). Sem linha = tudo habilitado (default do banco, opt-out). RLS: usuário só lê/escreve a própria linha |
