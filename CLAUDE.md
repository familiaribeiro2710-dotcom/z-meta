...
- Notificações push (Fase 5 + hierarquia 2026-07-20): `push_notify(profile_id, title, body, url)` manda o push pra um perfil; `push_notify_hierarquia_loja(loja_id, empresa_id, title, body, url)` resolve e notifica todo sócio da empresa + todo supervisor com `loja_access` naquela loja. Ambas `SECURITY DEFINER` com `EXECUTE` revogado de `anon`/`authenticated`/`PUBLIC` (só `postgres`/`service_role`) — são `returns void`, diferente das triggers de evento (`returns trigger`), então ficariam chamáveis via RPC por qualquer usuário logado se o `EXECUTE` não fosse revogado. Triggers de evento: `notify_push_advertencia`/`notify_push_premiacao`/`notify_push_novo_lead`/`notify_push_venda` (Fase 5, notificam só o colaborador dono ou o gerente da equipe) + `notify_push_venda_vestuario`/`notify_push_venda_consorcio_hierarquia`/`notify_push_meta_vestuario`/`notify_push_meta_consorcio` (2026-07-20, notificam sócio/supervisor via `push_notify_hierarquia_loja` — "meta batida" só dispara na transação que efetivamente cruza uma nova camada de `sales_goals`/`consorcio_goals`, comparando total antes/depois, não a cada venda).
