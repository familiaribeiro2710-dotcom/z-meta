...
master_admin  → dono do Z Meta. Cadastra empresas-cliente, vê/opera tudo.
   └── socio         → dono de uma empresa-cliente. Vê TODAS as lojas da empresa automaticamente.
         ...
         └── administrativo → EXCLUSIVO de empresas categoria consórcio. Mesmo mecanismo de
               ...
               advertências, premiações nem colaboradores (nunca ganha permission='gerenciar',
               que daria can_manage_loja() de graça). 2 abas só: Início e Vendas.
...
| `app/administrativo/page.js` | 2026-07-21 — Monta `AppShell` + `AdministrativoView`. Papel exclusivo de consórcio, sem branch de categoria (só existe nesse segmento) |
| `lib/AdministrativoView.js` | 2026-07-21 — Experiência completa do papel **administrativo** (exclusivo de consórcio): só 2 abas, Início (dashboards de leitura — vendido no mês, ranking, conversão, meta em jogo — da loja selecionada, ele pode ter várias via `loja_access`) e Vendas (fila de confirmação cross-loja, com filtro por loja/colaborador/status/busca — editar/aprovar/recusar venda `vendido_pendente`) |
...
| `lib/HierarchyHome.js` | Experiência completa de **sócio/supervisor** — tela própria (não tem rota dedicada por sub-view), inclusive "ver como" gerente/colaborador e aba exclusiva de cadastro de supervisores. Categoria-aware: resolve a categoria da empresa uma vez, troca `EmpresaDashboard`→`ConsorcioDashboard` e o herocard quando consórcio. 2026-07-20: as abas Rankings/Faturamento (cross-loja) também foram portadas pra consórcio (`loadRankingsConsorcio`/`loadFaturamentoConsorcio`) — já não mostram mais aviso "em breve" nesse caso |
| `lib/GerenteView.js` | Experiência completa de **gerente** no segmento **vestuário** — dentro, renderiza `EmpresaDashboard` |
| `lib/GerenteViewConsorcio.js` | Espelha `GerenteView.js` pro segmento **consórcio** — dentro, renderiza `ConsorcioDashboard`. Fonte de vendas é `crm_leads`/`consorcio_goals` em vez de `sales_entries`/`sales_goals`. Exporta `GERENTE_TABS` (Início/**Calendário**/Metas/**Leads**/**Vendas**) — só o gerente tem aba Calendário própria (agenda mensal de TODA a equipe, agregando `crm_leads.agendamento_at` de todos os colaboradores ativos, com atribuição por nome); supervisor/sócio/master seguem em `CONSORCIO_TABS` (sem Calendário nem Leads nem Vendas). Início também tem um card "Agenda de hoje — equipe inteira", sempre visível. 2026-07-20: aba **Leads** (`LeadsTab`) — listagem paginada (20/página) de todos os leads da equipe com filtro por colaborador/busca, seleção múltipla e transferência em lote (`UPDATE crm_leads.employee_id`) pra outro colaborador — uso principal: redistribuir a carteira de um colaborador desligado. 2026-07-23: aba **Vendas** (`VendasTab`) é **só leitura** — mostra a fila de `vendido_pendente` da equipe com os mesmos filtros de `AdministrativoView.js`, mas sem editar/aprovar/recusar (removido nessa data — antes o gerente tinha a mesma capacidade de ação do administrativo, o que quebrava a separação de funções do fluxo de confirmação de venda: quem ganha comissão em cima da venda não pode ser quem valida ela) |
...
| `tasks` | `id`, `employee_id`, `loja_id`, `title`, `active`, `recurrence_type` (`daily`/`weekly`/`once`), `weekday`, `once_date`, `start_date`, `task_type` (`checklist`/`contatos`, default `checklist`), `contacts_target` | exclusão é **soft-delete** (`active=false`) — nunca hard-delete, porque `task_completions.task_id` é `ON DELETE CASCADE` e apagaria histórico. 2026-07-20: `task_type='contatos'` é exclusivo do segmento **consórcio** — tarefa diária sem checklist manual, o gestor define `contacts_target` (meta de ligações/dia) e o próprio banco marca `task_completions` comparando com a contagem real de `crm_leads.data_ligacao` daquele colaborador/dia (ver `sync_contatos_completions` abaixo) |
...
| `employee_prizes` | `employee_id`, `month`, `amount`, `description`, `empresa_id`, `loja_id` | premiações lançadas por gestor |
...
| `crm_leads` | `id`, `empresa_id`, `loja_id`, `employee_id`, `nome_completo`, `telefone`, `endereco`, `email`, `data_ligacao`, `agendamento_at`, `status` (`novo`/`agendado`/`follow_up`/`perdido`/`vendido_pendente`/`vendido`), `feedback`, `valor`, `categoria_produto_id`, `observacoes`, `vendido_at`, `venda_revisada_por`, `venda_revisada_em`, `venda_motivo_recusa` | funil de vendas do segmento **consórcio** (ligação → agendamento → resultado). Registro único por lead evoluindo de status, não 3 tabelas separadas. `CHECK` de forma garante `agendamento_at` preenchido quando `status='agendado'` e `valor`+`categoria_produto_id` preenchidos quando `status` é `vendido` **ou** `vendido_pendente` (mesmo padrão de `tasks.recurrence_type`). `vendido_at` é setado uma única vez, só quando o Administrativo confirma a venda (`status` vira `vendido` de verdade) — 2026-07-21: colaborador/gerente/sócio/supervisor não fecham mais a venda direto, ela nasce `vendido_pendente` e só o papel `administrativo` (via `can_confirm_vendas`) aprova (`vendido`) ou recusa (`follow_up` + `venda_motivo_recusa`, `valor`/`categoria_produto_id` voltam a `null`). `vendido_at` continua sendo a fonte de verdade pra meta/comissão (`consorcio_goals` soma por ele), então nenhum motor de meta precisou mudar. Sem policy de `DELETE` — funil não é hard-deletável, só editável, pra preservar histórico |
...
| `notifications` | `id`, `profile_id`, `title`, `body`, `url`, `created_at` | 2026-07-21 — histórico de notificações pro sino mostrar as últimas 48h (antes só existia o push em si, nada persistido). Gravada dentro de `push_notify()` (único ponto que efetivamente manda pra um `profile_id`, todo o resto roteia por ele) — nenhum trigger de notificação precisou de lógica duplicada. RLS: usuário só lê a própria (`profile_id = auth.uid()`), sem policy de escrita pra ninguém além da função `SECURITY DEFINER` |
| `monthly_closing_confirmations` | `profile_id`, `role` (`gerente`/`supervisor`), `empresa_id`, `month`, `confirmed_at`, `UNIQUE(profile_id, month)` | 2026-07-23 — ritual de fechamento de mês, **funciona nas duas categorias**. Clicar "sim" no modal (`lib/MonthClosingAskModal.js`) não trava nada no sistema — é só um ritual pra disparar os modais de parabéns, decisão explícita do Felipe. Se não existir linha pro `profile_id`+mês anterior, o modal pergunta de novo em toda visita ao Início (nunca some sozinho até confirmar). RLS: usuário só lê a própria linha; só as RPCs `SECURITY DEFINER` abaixo escrevem |
| `monthly_closing_winners` | `profile_id`, `month`, `position` (1-3), `scope` (`colaborador`/`gerente`), `sold`, `seen_at` | 2026-07-23 — premiados do ranking do mês fechado: `scope='colaborador'` = top 1-2 da própria equipe do gerente que confirmou; `scope='gerente'` = top 1-3 entre **todas as lojas que o supervisor acessa** (não só uma loja — a maioria só tem 1 gerente, não existiria "2º/3º lugar" dentro dela). Só entra quem vendeu > R$0. `seen_at` marca quando o premiado já viu o modal de parabéns (`lib/MonthClosingCongratsModal.js`), pra não repetir — diferente do modal de pergunta, que insiste. Trigger `trg_prevent_winner_row_tamper` trava que o próprio premiado edite `position`/`sold`/etc via API direta, só `seen_at` é editável pelo dono da linha |
...
- RPCs usadas pelo app: `get_team_progress` (Barra Geral de Atividades), `get_store_sales_ranking`, `admin_overview`, `admin_financeiro`, `admin_faturamento_mensal`, `admin_lojas_health` (dashboards do Master Admin), `admin_delete_empresa` (exclusão em cascata de empresa, cobre vestuário e consórcio), `admin_delete_loja` (exclusão em cascata de uma loja só, mesmo padrão).
...
- 2026-07-20 — tarefa tipo "contatos" (consórcio): `sync_contatos_completions(employee_id, date)` (`SECURITY DEFINER`, `search_path` fixo, `EXECUTE` revogado de `anon`/`authenticated`/`PUBLIC`) conta `crm_leads` daquele colaborador/dia e faz upsert em `task_completions` pra cada tarefa `contatos` ativa dele, comparando com `contacts_target`. Chamada por `trg_sync_contatos_on_crm_leads` (trigger em `crm_leads`, AFTER INSERT/UPDATE/DELETE — recalcula o par employee/data afetado, e o antigo também se mudou num UPDATE) e `trg_sync_contatos_on_task_change` (trigger em `tasks`, AFTER INSERT/UPDATE — recalcula HOJE na hora ao criar/editar a tarefa, usando `(now() at time zone 'America/Sao_Paulo')::date` pra bater com o fuso do app, não o fuso do servidor).
...
- Ritual de fechamento de mês (2026-07-23, `lib/monthClosing.js` + `lib/MonthClosingAskModal.js`/`MonthClosingCongratsModal.js`): `confirm_month_closing_gerente(p_month)`/`confirm_month_closing_supervisor(p_month)` (vestuário, somam `sales_entries`) e as variantes `_consorcio` (mesma coisa, somam `crm_leads.valor` onde `status='vendido'`, cohort por `vendido_at` — mesmo padrão de `notify_push_meta_consorcio`). Todas `SECURITY DEFINER`, `EXECUTE` só pra `authenticated` (revogado de `anon` explicitamente — Supabase concede EXECUTE default pra `anon`/`authenticated` na criação da function, revoke de `PUBLIC` sozinho não reverte esse grant direto). Cada uma: valida que quem chama (`auth.uid()`) é de fato gerente/supervisor, grava `monthly_closing_confirmations` (`on conflict do nothing`, idempotente) e insere em `monthly_closing_winners` só quem vendeu > R$0, até 2 (gerente→colaborador) ou 3 (supervisor→gerente, cross-loja). Client (`needsClosingAsk`/`unseenClosingWinners`/`markClosingWinnerSeen` em `lib/monthClosing.js`) só faz `select`/`update` própria linha, nunca escreve em nome de outro perfil. Os 4 pontos de montagem (`lib/GerenteView.js`, `lib/GerenteViewConsorcio.js`, `lib/HierarchyHome.js` role=supervisor, `lib/ColaboradorView.js`, `lib/ColaboradorViewConsorcio.js`) sempre checam a flag de impersonação (`viewedBySupervisor`/`viewedByManager`/`impersonate`) antes de rodar — a RPC roda com o `auth.uid()` de verdade da sessão, nunca do perfil "visto como", e falharia silenciosamente/erradamente se disparasse durante um "ver como".
...
- **Todo botão de ativar/desativar/excluir precisa de `lib/ConfirmModal.js` (antes) + `useSavedNotice()`/`notifySaved()` (depois)** — nunca `window.confirm`/`window.prompt`/`alert()`. Padrão estabelecido em 2026-07-20 e aplicado em todo o app (colaborador/gerente/sócio/supervisor, tarefas, metas, premiações, advertências, categorias de produto consórcio, empresas/lojas). **Editar** (nome, username, valores) leva só o modal de sucesso depois de salvar — não o de confirmação antes, pra não colocar fricção em toda edição de texto. Exceção documentada de propósito: remover o acesso de um sócio/supervisor a UMA loja específica (`loja_access`, dentro de um painel já aberto) mantém só `window.confirm` nativo — ação granular e frequente, não a exclusão de uma entidade.
