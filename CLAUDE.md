...
| `app/gerente/page.js` | Monta `AppShell` + `GerenteView` |
| `app/colaborador/page.js` | Monta `AppShell` + `ColaboradorView` |
...
| `lib/HierarchyHome.js` | Experiência completa de **sócio/supervisor** — tela própria (não tem rota dedicada por sub-view), inclusive "ver como" gerente/colaborador e aba exclusiva de cadastro de supervisores |
| `lib/GerenteView.js` | Experiência completa de **gerente** — dentro, renderiza `EmpresaDashboard` |
| `lib/ColaboradorView.js` | Experiência completa de **colaborador** — herocard de meta do dia, checklist, histórico de vendas |
| `lib/EmpresaDashboard.js` | Componente central **compartilhado** por gerente/supervisor/sócio/master — abas Placar/Colaboradores/Tarefas/Advertências/Premiações/Metas/Lançamentos por loja |
| `lib/AppShell.js` | Casca visual (header, "Meu perfil", trocar senha) usada em toda tela autenticada |
...
| `crm_leads` | `id`, `empresa_id`, `loja_id`, `employee_id`, `nome_completo`, `telefone`, `endereco`, `email`, `data_ligacao`, `agendamento_at`, `status` (`novo`/`agendado`/`follow_up`/`perdido`/`vendido`), `feedback`, `valor`, `categoria_produto_id`, `observacoes` | funil de vendas do segmento **consórcio** (ligação → agendamento → resultado). Registro único por lead evoluindo de status, não 3 tabelas separadas. `CHECK` de forma garante `agendamento_at` preenchido quando `status='agendado'` e `valor`+`categoria_produto_id` preenchidos só quando `status='vendido'` (mesmo padrão de `tasks.recurrence_type`). Sem policy de `DELETE` — funil não é hard-deletável, só editável, pra preservar histórico |
| `consorcio_goals` / `consorcio_goal_allocations` / `consorcio_commission_settings` | espelham `sales_goals`/`sales_goal_allocations`/`commission_settings` 1:1 | motor de metas em camada do consórcio, **tabelas separadas** das de vestuário por decisão explícita do Felipe (isolar blast radius de bug entre os dois segmentos). Alimentadas por `sum(crm_leads.valor)` em vez de `sum(sales_entries.daily_amount)` — a lógica pura de cálculo (`currentGoalTarget`/`achievedTier`, `lib/scoring.js`) é agnóstica de fonte, reaproveitada sem mudança |
