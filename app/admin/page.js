            ...
            <div className="relative shrink-0">
              <button
                ...
                className={`p-2 rounded-full border transition-all ${
                  filterOpen ? "bg-goldlight text-navyfixed border-goldlight" : "border-white/15 text-white/60 hover:border-goldlight hover:text-goldlight"
                }`}
                ...
                >
                  <div className="flex items-start gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      <p className="font-semibold text-white text-sm flex items-center gap-1.5 flex-wrap">
                        ...
                        {row.categoria_nome && (
                          <span className="text-[10px] font-bold px-2.5 py-1 rounded-full bg-surface/10 text-goldlight shrink-0">{row.categoria_nome}</span>
                        )}
                  ...
                  <div className="flex items-center gap-2 shrink-0" onClick={(e) => e.stopPropagation()}>
                    ...
                    <button
                      ...
                      className={`p-1.5 rounded-lg hover:bg-surface/10 transition-colors ${row.active ? "text-white/60 hover:text-warn" : "text-success"}`}
                    >
                      <Power size={13} />
                    </button>
                    <button
                      ...
                      className="p-1.5 rounded-lg text-white/60 hover:text-danger hover:bg-surface/10 transition-colors"
                    >
                      <Trash2 size={13} />
                    </button>
                  </div>
  ...
  return (
    ...
    >
      {logoUrl ? (
        ...
        <div
          className="w-full h-full flex items-center justify-center text-navyfixed font-bold text-sm"
          style={{ background: "linear-gradient(135deg, #e4c789, #c9a15a)" }}
        >
          {initials}
        </div>
      )}
      <div className="absolute inset-0 flex items-center justify-center bg-navyfixed/0 group-hover:bg-navyfixed/50 opacity-0 group-hover:opacity-100 transition-all">
        <Camera size={14} className="text-white" />
      </div>
      {uploading && (
        <div className="absolute inset-0 bg-navyfixed/60 flex items-center justify-center">
    ...
    <div className={`card-dark ${!row.active ? "opacity-60" : ""}`}>
      <div className="flex items-start justify-between gap-3 flex-wrap mb-3">
        ...
        <div className="flex items-center gap-2.5">
          ...
          <button
            ...
            className={`p-1.5 rounded-lg transition-colors ${hasLink ? "text-goldlight hover:bg-surface/10" : "text-white/20 cursor-not-allowed"}`}
          >
            <LinkIcon size={15} />
          </button>
      ...
      <div className="grid sm:grid-cols-3 gap-3">
        ...
        <div>
          ...
          <p className="!py-1.5 !text-xs font-bold text-goldlight flex items-center bg-surface/5 border border-white/10 rounded-2xl px-3.5">
      ...
      {dirty && (
        <div className="mt-3">
          <button className="btn-outline !py-1.5 !text-xs !border-goldlight !text-goldlight hover:!bg-goldlight hover:!text-navyfixed" onClick={save} disabled={saving}>
        ...
        {[{ key: "mes", label: "No mês" }, { key: "total", label: "Total histórico" }].map((m) => (
          <button
            ...
            className={`px-3 py-1.5 rounded-full text-xs font-bold border-2 transition-all ${
              viewMode === m.key ? "bg-navyfixed text-white border-navyfixed" : "border-line text-muted hover:border-navy hover:text-navy"
            }`}
          >
            {m.label}
          </button>
          ...
          <div className="flex items-center gap-1.5 flex-wrap">
            {PERIOD_PRESETS.map((p) => (
              <button
                ...
                className={`px-3 py-1.5 rounded-full text-xs font-bold border-2 transition-all ${
                  activePreset === p.key ? "bg-navyfixed text-white border-navyfixed" : "border-line text-muted hover:border-navy hover:text-navy"
                }`}
              >
                {p.label}
              </button>
            ...
            {NEW_USER_ROLES.map((r) => (
              <button
                ...
                className={`px-3.5 py-1.5 rounded-full text-xs font-bold border-2 transition-all ${
                  role === r.key ? "bg-navyfixed text-white border-navyfixed" : "border-line text-muted hover:border-navy hover:text-navy"
                }`}
              >
                {r.label}
              </button>
