...
import { Store, PhoneCall, CalendarClock, ListTodo, Coins, Gift, Eye, ArrowLeft, CheckSquare, Check, AlertTriangle, Target, Trophy } from "lucide-react";
...
import { greeting, todayStr, firstDayOfMonth, monthLabel, isTaskDueOn, daysElapsedInMonth } from "./date";
...
export default function GerenteViewConsorcio({ profile, tab, viewedBySupervisor = false, onBack }) {
  ...
  const loadStats = useCallback(async (prof, monthArg) => {
    ...
    setHero({ metaLoja, soldLoja, ligacoesHoje, agendamentosHoje, pendingToday, commissionSoFar, prizesSoFar, commissionPct, commissionTierLabel, leaderName });
  }, []);
