...
import { greeting, todayStr, firstDayOfMonth, nextMonth, monthLabel, isTaskDueOn, daysElapsedInMonth, daysInMonth, remainingPeriodsInMonth, periodLabel, isPrizePeriodActive, prizeWindowLabel } from "./date";
...
// 2026-08-08: "Pipeline" entrou depois de Metas, escondida em telas pequenas (hideOnMobile, ver
// AppShell.js) — pedido do Felipe. Cai no mesmo <ConsorcioDashboard tab="pipeline"> renderizado
// mais abaixo (bloco `tab !== "calendario" && tab !== "vendas"`), que já escopa o Pipeline pela
// equipe do gerente (employeeIds) em vez da loja inteira quando viewerRole="gerente". 2026-08-12:
// "Leads" passou a cair no mesmo bloco (deixou de ter exclusão própria) — ver LeadsTab em
// ConsorcioDashboard.js.
...
export default function GerenteViewConsorcio({ profile, tab, viewedBySupervisor = false, onBack }) {
  ...
  const [storeGoalCelebrations, setStoreGoalCelebrations] = useState([]); // fila de níveis de meta da loja batidos, um modal por vez — mesmo padrão de GerenteView.js (vestuário)
  ...
  // 2026-07-25 (pedido do Felipe): mesmo card "Agenda de hoje" do colaborador, só que cross-equipe
  // — precisa de filtro por colaborador pra não virar uma lista gigante numa equipe grande.
  const [agendaFiltroEmp, setAgendaFiltroEmp] = useState("todos");
  ...
  const loadStats = useCallback(async (prof, monthArg) => {
    ...
    // 2026-09-03: não filtra active=true aqui — soldLoja/ranking/conversão do mês não podem sumir
    // com o dado de quem foi desativado depois daquele mês. Agenda/pendências de HOJE e o status de
    // atividades usam `activeEmpIds` à parte (só time atual); `teamEmps` (lista/modal de equipe)
    // também fica só com quem está ativo.
    ...
    setTeamEmps((emps || []).filter((e) => e.active));
    // aba Calendário navega por mês (inclusive passados) — usa o conjunto histórico (empIds), não
    // só quem está ativo hoje, senão a agenda de um mês fechado sumiria com quem foi desativado.
    ...
    if (activeEmpIds.length) {
      ...
      teamActivityData = (emps || []).filter((emp) => emp.active).map((emp) => {
    ...
    const { data: goalRows } = await supabase
      ...
      .select("id, name, store_total, commission_pct_colaborador, commission_pct_gerente, non_monetary_prize, pacing, prize_period_type, prize_period_start, prize_period_end")
    ...
    // ativo hoje OU vendeu algo no mês visualizado — senão todo ex-colaborador da equipe entraria
    // no ranking com R$0.
    const ranking = (emps || [])
  ...
  // Modal de "a loja bateu a meta" — mesmo padrão de GerenteView.js (vestuário): um pop-up por
  // nível de meta da loja batido, uma única vez por gerente/mês/nível (goal_celebration_seen, no
  // banco). Só na sessão real do gerente (nunca em "ver como") e só no mês corrente.
  useEffect(() => {
    if (viewedBySupervisor) return;
    if (month !== firstDayOfMonth(today)) return;
    if (!goalsList.length) return;
    let active = true;
    (async () => {
      const achieved = goalsList.filter((g) => hero.soldLoja >= Number(g.store_total || 0));
    ...
    return () => { active = false; };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [goalsList, hero.soldLoja, month, viewedBySupervisor]);
  ...
  // Ritmo (semana/quinzena/estágio) opcional por meta, 2026-08-05 — "dia" continua mostrando o
  // total puro (mesmo padrão de sempre do consórcio, que não fatia por dia); as outras réguas
  // passam a mostrar a fatia do período em vez do total restante.
  const activePacing = activeGoal?.pacing || "dia";
  const heroPeriodsLeft = activePacing !== "dia" && month === firstDayOfMonth(today) ? remainingPeriodsInMonth(month, today, activePacing) : 0;
  const heroPeriodTarget = activePacing !== "dia" && heroPeriodsLeft > 0 ? restoDaMeta / heroPeriodsLeft : restoDaMeta;

  // Prêmio extra: fixo o mês inteiro assim que QUALQUER meta da loja tiver premiação extra
  // cadastrada, não só a que está "em jogo" (mesmo ajuste de GerenteView.js). Com 2+ metas
  ...
  const prizeGoalsAll = goalsList.filter((g) => g.non_monetary_prize && isPrizePeriodActive(g));
  const extraPrizeGoal = prizeGoalsAll.find((g) => hero.soldLoja < Number(g.store_total)) || prizeGoalsAll[prizeGoalsAll.length - 1] || null;
  ...
  async function handleDownloadReport(options) {
    ...
    try {
      ...
      const reportData = await buildMonthlyReportData({
        ...
        gerenteId: profile.id,
        gerenteName: profile.full_name,
        month,
      });
  ...
  return (
    <div className="space-y-6">
      {storeGoalCelebrations.length > 0 && (
      ...
      {storeGoalCelebrations.length === 0 && closingWinners.length > 0 && (
      ...
      <PendingActivitiesModal
        ...
        empIds={teamEmps.map((e) => e.id)}
        employeeNameById={Object.fromEntries(teamEmps.map((e) => [e.id, e.full_name]))}
        onChanged={() => loadStats(profile, selectedMonth)}
      />
      ...
      {tab === "atividades" && (
        <>
          <div className="flex items-start justify-between gap-3 flex-wrap">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              <button
                ...
                className="btn-outline !py-1.5 !text-xs flex items-center gap-1.5 whitespace-nowrap disabled:opacity-60"
              >
          ...
          <div
            ...
            style={{ background: "linear-gradient(160deg, #1c2e4d 0%, #12203a 100%)", borderColor: "#16a34a", boxShadow: "0 10px 26px rgba(18,32,58,0.35)" }}
          >
            <div className="absolute -top-14 -right-10 w-48 h-48 rounded-full bg-[#16a34a]/15" />
            <div className="relative flex items-center gap-2 mb-3">
              <Store size={18} className="text-[#4ade80]" />
              <span className="text-xs font-bold uppercase tracking-wider text-[#4ade80]">Funil de hoje · {lojaName || "sua loja"}</span>
            </div>
            ...
            <p className="relative text-xs font-semibold text-white/60 mt-1">
              {hero.metaLoja === 0
                ...
                : activePacing === "dia"
                ? `faltam ${formatBRL(restoDaMeta)} pra próxima meta`
                : `faltam ${formatBRL(heroPeriodTarget)} ${periodLabel(month, today, activePacing)}`}
            </p>
            ...
            <div className="relative grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-x-3 gap-y-5 sm:gap-4 mt-6 pt-5 border-t border-white/10">
              ...
              <div className="min-w-0">
                ...
                {hero.metaLoja > 0 && (
                  <p className="text-[10px] text-white/45 mt-0.5">{hero.commissionPct}% · {hero.commissionTierLabel}</p>
                )}
              </div>
              <div className="min-w-0">
                    ...
                    <li key={l.id} className="py-3">
                      <div className="flex items-start justify-between gap-2 flex-wrap">
                        <div className="min-w-0">
                          ...
                          {l.status === "em_negociacao" && l.valor != null && <p className="text-xs font-bold mt-1 text-teal">Negociando {formatBRL(l.valor)}</p>}
                        </div>
                      </div>
                    </li>
                  ))}
      ...
      {/* Metas não tem herocard próprio (só a aba Início tem), mas ainda depende do mês
          selecionado — sem esse seletor aqui, ficava presa no mês corrente/último escolhido na
          aba Início, sem forma de trocar (mesmo ajuste de GerenteView.js pro segmento vestuário).
          maxMonth libera o mês seguinte de propósito (só aqui, Início continua travado no
          corrente) — pedido do Felipe pra dar pra planejar a meta antes do mês virar de verdade. */}
      {tab === "metas" && (
        <div className="flex items-center justify-end">
          <MonthNav month={month} onChange={setSelectedMonth} maxMonth={nextMonth(today)} />
        </div>
      )}

      {tab !== "calendario" && tab !== "vendas" && (
        <ConsorcioDashboard
          lojaId={profile.loja_id}
      ...
      {/* 2026-07-27 (pedido do Felipe): Agenda de hoje precisa aparecer DENTRO da aba Funil (depois
          da navegação de sub-abas, junto com o resto do conteúdo do Funil) — não acima da barra de
          abas. É o primeiro card dentro dessa área, antes de Conversão. */}
      {tab === "atividades" && atSub === "funil" && (
        <>
          <div className="card animate-pop border-blue/20">
            <div className="flex items-center justify-between gap-2 mb-3 flex-wrap">
              <p className="label mb-0 flex items-center gap-1.5"><CalendarDays size={14} /> Agenda de hoje — equipe inteira</p>
              {teamEmps.length > 1 && (
                <SelectField icon={Users} className="w-full sm:w-56" value={agendaFiltroEmp} onChange={(e) => setAgendaFiltroEmp(e.target.value)}>
            ...
            {(() => {
              const meetingsFiltradas = agendaFiltroEmp === "todos" ? todayMeetings : todayMeetings.filter((l) => l.employee_id === agendaFiltroEmp);
              if (meetingsFiltradas.length === 0) {
                return <p className="text-sm text-muted">{agendaFiltroEmp === "todos" ? "Nenhum agendamento pra hoje na equipe." : "Nenhum agendamento pra hoje desse colaborador."}</p>;
              }
              return (
                <ul className="divide-y divide-line">
          ...
          {hero.metaLoja > 0 && (
            <div className="card animate-pop border-orange/20">
              <p className="label flex items-center gap-1.5"><Trophy size={14} /> Barra de vendas da equipe — {monthLabel(month)}</p>
              <ProgressBar pct={Math.min(100, (hero.soldLoja / hero.metaLoja) * 100)} />
              <p className="text-xs text-muted mt-2">{formatBRL(hero.soldLoja)} vendido de {formatBRL(hero.metaLoja)} de meta.</p>
            </div>
          )}
            ...
            {hero.ligacoesMes === 0 ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                ...
                <div className="rounded-2xl bg-gold/5 border border-gold/20 p-3 min-w-0">
                  <AutoFitText className="text-2xl font-extrabold text-gold">{formatPct(hero.pctLigacaoVenda)}</AutoFitText>
            ...
            <div className="card border-teal/20">
              ...
              <div className="flex items-center gap-4 text-xs text-muted flex-wrap">
                ...
                {personalPrizes.length > 0 && (
                  <span className="flex items-center gap-1"><Gift size={13} className="text-gold" /> {formatBRL(personalPrizes.reduce((s, p) => s + Number(p.amount || 0), 0))} em premiações no mês</span>
                )}
              </div>
            </div>
          )}
...
// 2026-08-12: a aba "Leads" deixou de ser exclusiva daqui — foi generalizada e movida pra
// LeadsTab em ConsorcioDashboard.js (export nomeado), que agora escopa por `employees` (equipe do
// gerente OU loja inteira do sócio/supervisor/master, conforme quem monta o componente) em vez de
// só `teamEmps`. A renderização de `tab === "leads"` passou a delegar pra
// `<ConsorcioDashboard tab="leads">` como qualquer outra aba (ver bloco de render abaixo) — não
// precisa mais de um componente/estado próprio aqui.
