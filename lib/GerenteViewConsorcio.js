...
export default function GerenteViewConsorcio({ profile, tab, viewedBySupervisor = false, onBack }) {
  ...
  // nível de meta da loja batido, uma única vez por gerente/mês/nível (localStorage). Só na sessão
  // real do gerente (nunca em "ver como") e só no mês corrente.
  useEffect(() => {
    ...
    const newlyAchieved = [];
    goalsList.forEach((g) => {
      if (hero.soldLoja >= Number(g.store_total || 0)) {
