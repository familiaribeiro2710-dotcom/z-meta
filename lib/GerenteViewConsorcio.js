          ...
          >
            ...
            {activeGoal?.non_monetary_prize && (
              <p className="relative text-xs font-bold text-navy mt-1 flex items-center gap-1.5">
                <PartyPopper size={13} /> Prêmio da equipe: {activeGoal.non_monetary_prize}
              </p>
            )}
            <p className="relative text-xs font-semibold text-navy/70 mt-1">Vendido no mês — {monthLabel(month)}</p>
            {hero.metaLoja > 0 && (
              <div className="relative mt-3"><ProgressBar pct={Math.min(100, (hero.soldLoja / hero.metaLoja) * 100)} showLabel={false} height="h-2" /></div>
            )}
