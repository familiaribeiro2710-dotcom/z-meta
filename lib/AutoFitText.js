...
export default function AutoFitText({ children, className = "", as: Tag = "span", minPx = 10 }) {
  ...
  useLayoutEffect(() => {
    ...
    function fit() {
      ...
      const target = Math.max(minPx, Math.floor(naturalFontSize * (containerWidth / textWidth) * 0.95));
      setFontSize(target);
    }
    ...
    // A fonte do app (Inter, via @fontsource) carrega de forma assíncrona. Se o fit() acima rodar
    // antes dela terminar de carregar, a medição usa a largura da fonte de fallback do navegador —
    // que costuma ser mais estreita. Quando a Inter troca de verdade, o texto pode ficar mais largo
    // que o calculado e vazar do card (cortado pelo overflow-hidden), sem nada disparando um novo
    // cálculo, porque o ResizeObserver só reage a mudança de tamanho do próprio elemento, não a troca
    // de fonte. Por isso, reforça o cálculo assim que as fontes terminarem de carregar.
    let cancelled = false;
