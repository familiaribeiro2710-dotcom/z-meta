                      ...
                      <div className="flex items-start justify-between gap-2 flex-wrap">
                        ...
                        {l.status !== "cancelado" && (
                          <div className="flex items-center gap-1.5 shrink-0">
                            <button type="button" onClick={() => openEdit(l)} title="Editar" aria-label="Editar" className="p-1.5 rounded-xl border border-line text-muted hover:border-blue hover:text-blue transition-colors">
                              <Edit3 size={14} />
                            </button>
                            {l.status === "vendido_pendente" && (
                              <>
                                <button type="button" onClick={() => setConfirmAprovar(l)} title="Aprovar" aria-label="Aprovar" className="p-1.5 rounded-xl border border-success/30 text-success hover:bg-success/10 transition-colors">
                                  <CheckCircle2 size={14} />
                                </button>
                                <button type="button" onClick={() => { setRecusarModal(l); setMotivoRecusa(""); }} title="Recusar" aria-label="Recusar" className="p-1.5 rounded-xl border border-danger/30 text-danger hover:bg-danger/10 transition-colors">
                                  <XCircle size={14} />
                                </button>
                              </>
                            )}
                            <button type="button" onClick={() => { setCancelarModal(l); setMotivoCancelamento(""); }} title="Cancelar (excluir)" aria-label="Cancelar" className="p-1.5 rounded-xl border border-line text-muted hover:border-danger hover:text-danger transition-colors">
                              <Trash2 size={14} />
                            </button>
                          </div>
                        )}
