...
// 3 abas, pedido explícito do Felipe: Início (dashboards de leitura — ranking, vendas,
// agendamentos, metas), Vendas (fila de confirmação — aprovar/recusar/editar) e, desde 2026-08-08,
// Pipeline (visão em quadro da loja selecionada, hideOnMobile — ver AppShell.js). Pipeline aqui é
// só leitura (canManage=false no <Pipeline>, ver render abaixo) — administrativo continua sem
// poder mexer em agendamento/follow-up/perdido/negociação, isso é dos colaboradores/gerente; a
// única ação dele (aprovar/recusar venda) continua exclusivamente na aba Vendas de sempre. Nada de
...
const STATUS_CHIP = {
  ...
  cancelado: "bg-navyfixed/10 text-muted line-through",
...
export default function AdministrativoView({ profile, tab }) {
  ...
  // Cancelar (soft-delete) — 2026-07-23, pedido do Felipe: "excluir uma venda" tem que ser
  // exclusivo do administrativo, em qualquer status (inclusive já vendido), mas sem apagar de
  // vez — vira status 'cancelado', some das filas/rankings/faturamento mas fica no banco pra
  // auditoria (cancelado_por/cancelado_em/motivo_cancelamento). Continua sem DELETE de verdade.
  const [cancelarModal, setCancelarModal] = useState(null); // lead
  ...
  const loadInicio = useCallback(async (lojaId, monthArg) => {
    ...
    // 2026-09-03: não filtra active=true aqui — o ranking do mês (storeRanking, mais abaixo) não
    // pode sumir com a venda de quem foi desativado depois daquele mês. "Colaboradores ativos" no
    // herocard usa o filtro explícito (empregados.filter(e => e.active).length), não o array cru.
  ...
  const storeRanking = (() => {
    ...
    return empregados
      .map((e) => ({ id: e.id, name: e.full_name, avatar_url: e.avatar_url, sold: byEmp[e.id] || 0 }))
      .filter((r) => r.sold > 0)
  ...
  async function cancelarVenda() {
    if (!cancelarModal || !motivoCancelamento.trim()) return;
    setCancelando(true);
    const { error } = await supabase
      .from("crm_leads")
      .update({
    ...
    if (!error) {
      ...
      notifySaved("Lead cancelado com sucesso.");
      await refreshVendas();
      if (selectedLojaId) await loadInicio(selectedLojaId, month);
    }
  }
  ...
  return (
    <div className="space-y-6">
          ...
          {lojas.length === 0 ? (
            ...
            <>
              <div
                ...
                style={{ background: "linear-gradient(160deg, #1c2e4d 0%, #12203a 100%)", borderColor: "#94a3b8", boxShadow: "0 10px 26px rgba(18,32,58,0.35)" }}
              >
                <div className="absolute -top-14 -right-10 w-48 h-48 rounded-full bg-[#94a3b8]/15" />
                <div className="relative flex items-center gap-2 mb-3">
                  <Store size={18} className="text-[#cbd5e1]" />
                  <span className="text-xs font-bold uppercase tracking-wider text-[#cbd5e1]">Vendido no mês · {selectedLoja?.loja_name || "loja"}</span>
                </div>
                <AutoFitText className="relative text-4xl sm:text-5xl font-extrabold text-white leading-tight">{formatBRL(vendidoMes)}</AutoFitText>
                <p className="relative text-xs font-semibold text-white/60 mt-1">{monthLabel(month)}</p>

                <div className="relative grid grid-cols-2 sm:grid-cols-4 gap-x-3 gap-y-5 mt-6 pt-5 border-t border-white/10">
                  <div className="min-w-0">
                    <AutoFitText className="text-lg sm:text-xl font-extrabold text-white">{pendentesLoja}</AutoFitText>
                    <p className="text-[11px] font-semibold text-white/60 mt-0.5 flex items-center gap-1"><Clock size={11} className="shrink-0" /> Aguardando confirmação</p>
                  </div>
                  <div className="min-w-0">
                    <AutoFitText className="text-lg sm:text-xl font-extrabold text-white">{ligacoesHoje}</AutoFitText>
                  ...
                  <div className="min-w-0">
                    <AutoFitText className="text-lg sm:text-xl font-extrabold text-white">{agendamentosHoje}</AutoFitText>
                    <p className="text-[11px] font-semibold text-white/60 mt-0.5 flex items-center gap-1"><CalendarClock size={11} className="shrink-0" /> Agendamentos hoje</p>
                  </div>
                  <div className="min-w-0">
                    <AutoFitText className="text-lg sm:text-xl font-extrabold text-white">{empregados.filter((e) => e.active).length}</AutoFitText>
                    <p className="text-[11px] font-semibold text-white/60 mt-0.5 flex items-center gap-1"><ListTodo size={11} className="shrink-0" /> Colaboradores ativos</p>
                  </div>
                </div>
              </div>
      ...
      {tab === "pipeline" && (
        <div className="space-y-4">
          <div className="flex items-center justify-between gap-3 flex-wrap">
            <h1 className="text-xl sm:text-2xl font-extrabold text-navy flex items-center gap-2">
              <Kanban size={22} className="text-orange" /> Pipeline
            </h1>
            {lojas.length > 0 && (
              <SelectField icon={Store} className="flex-1 max-w-xs" value={selectedLojaId} onChange={(e) => setSelectedLojaId(e.target.value)}>
                {lojas.map((l) => <option key={l.loja_id} value={l.loja_id}>{l.loja_name}</option>)}
              </SelectField>
            )}
          </div>
                    ...
                    <li key={l.id} className="py-3">
                      <div className="flex items-start justify-between gap-2 flex-wrap">
                        <div className="min-w-0">
                          ...
                          {l.status === "cancelado" && l.motivo_cancelamento && <p className="text-[11px] text-muted mt-1">Cancelado: {l.motivo_cancelamento}</p>}
                        </div>
                        {l.status !== "cancelado" && (
                          <div className="flex items-center gap-1.5 shrink-0">
                            ...
                            <button type="button" onClick={() => { setCancelarModal(l); setMotivoCancelamento(""); }} title="Cancelar (excluir)" aria-label="Cancelar" className="p-1.5 rounded-lg border border-line text-muted hover:border-danger hover:text-danger transition-colors">
                              <Trash2 size={14} />
                            </button>
                          </div>
                        )}
      ...
      {/* cancelar (excluir) — soft-delete, exige motivo, mesmo espírito do recusarModal acima */}
      {cancelarModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-navyfixed/70 p-6">
          <div className="card max-w-sm w-full animate-bounce-in border-danger/30">
            <h2 className="text-lg font-extrabold text-navy flex items-center gap-2"><Trash2 className="text-danger" size={20} /> Cancelar lead/venda</h2>
            <p className="text-xs text-muted mt-1">{cancelarModal.nome_completo}{cancelarModal.status === "vendido" ? ` — ${formatBRL(cancelarModal.valor)}` : ""}</p>
            {cancelarModal.status === "vendido" && (
              <p className="text-[11px] text-danger mt-2">Essa venda já está confirmada — cancelar tira ela do faturamento/comissão do mês imediatamente, mesmo se o mês já foi fechado.</p>
            )}
            <div className="mt-4">
              <label className="label">Motivo do cancelamento</label>
              <textarea className="input" rows={3} value={motivoCancelamento} onChange={(e) => setMotivoCancelamento(e.target.value)} placeholder="Por que esse lead/venda está sendo cancelado?" autoFocus />
            </div>
            <p className="text-[11px] text-muted mt-2">Não apaga de vez — fica marcado como cancelado, com seu nome e o motivo registrados. Some da fila e das somas de faturamento/comissão.</p>
            <div className="flex gap-2 mt-5">
              <button className="btn-outline flex-1" onClick={() => setCancelarModal(null)}>Voltar</button>
              <button className="btn-danger flex-1" disabled={cancelando || !motivoCancelamento.trim()} onClick={cancelarVenda}>{cancelando ? "Cancelando…" : "Cancelar lead/venda"}</button>
            </div>
          </div>
        </div>
      )}
