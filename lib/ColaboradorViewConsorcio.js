...
// (agendar/reagendar + os 3 de resolver: perdido/follow-up/vendido).
function LeadRow({ lead, onAgendar, onResolve }) {
...
export default function ColaboradorViewConsorcio({ profile, tab, viewedByManager = false, onBack }) {
  ...
  const loadCrm = useCallback(async (uid, empresaId) => {
  ...
  useEffect(() => {
    let active = true;
    (async () => {
      await Promise.all([loadCrm(profile.id, profile.empresa_id), loadTasks(profile.id)]);
      if (!active) return;
  ...
  async function submitLead(e) {
    ...
    resetForm();
    setFormOpen(false);
    await loadCrm(profile.id, profile.empresa_id);
  }
  ...
  return (
    <div className="space-y-6">
      ...
      {resolveModal && resolveModal.type === "vendido" && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-navy/70 p-6">
          <div className="card max-w-sm w-full animate-bounce-in border-success/30">
        ...
        <div className="space-y-6">
          ...
          >
            ...
            <div className="relative flex items-center gap-2 mb-3">
              <PhoneCall size={18} className="text-navy" />
              <span className="text-xs font-bold uppercase tracking-wider text-navy">Funil de vendas — hoje</span>
            </div>
            <div className="relative grid grid-cols-2 sm:grid-cols-4 gap-x-3 gap-y-5 mt-2">
              <div className="min-w-0">
                <AutoFitText className="text-2xl sm:text-3xl font-extrabold text-navy">{leadsToday.length}</AutoFitText>
              ...
              <div className="min-w-0">
                <AutoFitText className="text-2xl sm:text-3xl font-extrabold text-navy">{agendamentosHojeCount}</AutoFitText>
              ...
              <div className="min-w-0">
                <AutoFitText className="text-2xl sm:text-3xl font-extrabold text-navy">{abertosCount}</AutoFitText>
              ...
              <div className="min-w-0">
                <AutoFitText className="text-2xl sm:text-3xl font-extrabold text-navy">{formatBRL(vendasMes)}</AutoFitText>
          ...
          <div className="card">
            ...
            {listaFiltrada.length === 0 ? (
              ...
              <ul className="divide-y divide-line">
                {listaFiltrada.map((l) => <LeadRow key={l.id} lead={l} onAgendar={openAgendar} onResolve={openResolve} />)}
              </ul>
            )}
          </div>
        </div>
