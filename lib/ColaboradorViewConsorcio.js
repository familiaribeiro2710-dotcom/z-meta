...
export default function ColaboradorViewConsorcio({ profile, tab, viewedByManager = false, onBack }) {
  ...
  function openResolve(lead, type) {
    // 2026-08-10: "novo" -> "perdido" vira "sem_contato" automaticamente — lead que nunca teve
    // nenhuma movimentação é diferente de um que já foi agendado/negociado e não fechou.
    const resolvedType = type === "perdido" && lead.status === "novo" ? "sem_contato" : type;
    setResolveModal({ lead, type: resolvedType });
  ...
  async function confirmResolve() {
    ...
    if (type === "vendido") {
      ...
      ({ error } = await supabase
        .from("crm_leads")
        .update({ status: type, valor: null, feedback: resolveFeedback.trim() || null, updated_at: new Date().toISOString() })
        .eq("id", lead.id));
    }
    setResolveSaving(false);
    if (!error) {
