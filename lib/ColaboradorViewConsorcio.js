...
export default function ColaboradorViewConsorcio({ profile, tab, viewedByManager = false, onBack }) {
  ...
  // colaborador também (pedido do Felipe: todo mundo da loja vê, não só o gerente). Uma flag por
  // colaborador/mês/meta, pra não repetir.
  useEffect(() => {
    if (viewedByManager) return;
    ...
    const newlyAchieved = [];
    goals.forEach(({ goal }) => {
      if (storeSoldLoja >= Number(goal.store_total || 0)) {
        const flagKey = `zmeta_storegoalhit_${profile.id}_${monthArg}_${goal.id}`;
        if (typeof window !== "undefined" && !window.localStorage.getItem(flagKey)) {
          window.localStorage.setItem(flagKey, "1");
    ...
    if (newlyAchieved.length) setStoreGoalCelebrations((q) => [...q, ...newlyAchieved]);
    // eslint-disable-next-line react-hooks/exhaustive-deps
