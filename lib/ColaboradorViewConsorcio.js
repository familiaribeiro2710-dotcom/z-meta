...
function LeadRowDark({ lead, onAgendar }) {
  const meta = STATUS_META[lead.status] || STATUS_META.novo;
  const terminal = lead.status === "vendido" || lead.status === "perdido" || lead.status === "vendido_pendente";
  const hora = fmtHora(lead.agendamento_at);
    ...
    <li className="row-card items-start flex-wrap gap-y-2">
      ...
      {!terminal && (
        <button
          ...
          aria-label="Agendar"
          className="p-1.5 rounded-lg text-white/60 hover:text-bluelight hover:bg-white/10 transition-colors shrink-0"
        >
          <CalendarPlus size={14} />
        </button>
      )}
        ...
        <div className="space-y-6">
          ...
          <div className="card-dark">
            ...
            {listaFiltrada.length === 0 ? (
              ...
              <ul>
                {listaFiltrada.map((l) => <LeadRowDark key={l.id} lead={l} onAgendar={openAgendar} />)}
              </ul>
            )}
          </div>
