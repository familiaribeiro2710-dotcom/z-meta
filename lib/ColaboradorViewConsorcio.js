          ...
          <div className="card-dark">
            <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
              ...
              <div className="flex items-center gap-1 bg-white/5 rounded-full p-0.5 border border-white/10 shrink-0">
                <button type="button" onClick={() => setListFilter("abertos")} className={`text-[11px] font-bold px-2.5 py-1 rounded-full transition-all ${listFilter === "abertos" ? "bg-goldlight text-navy" : "text-white/60"}`}>Em aberto</button>
                <button type="button" onClick={() => setListFilter("todos")} className={`text-[11px] font-bold px-2.5 py-1 rounded-full transition-all ${listFilter === "todos" ? "bg-goldlight text-navy" : "text-white/60"}`}>Todos</button>
              </div>
            </div>
            {listaFiltrada.length === 0 ? (
              <p className="text-sm text-white/50">Nenhuma ligação {listFilter === "abertos" ? "em aberto" : "registrada"} ainda.</p>
            ) : (
              <ul>
