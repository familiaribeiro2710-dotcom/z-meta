...
import { todayStr, firstDayOfMonth, monthLabel, greeting, isTaskDueOn, daysInMonth, remainingPeriodsInMonth, periodLabel, PACING_HERO_LABEL, PACING_UNIT } from "./date";
...
export default function ColaboradorViewConsorcio({ profile, tab, viewedByManager = false, onBack }) {
  ...
  const prizeGoalsAll = goals.filter(({ goal }) => goal.non_monetary_prize).map(({ goal }) => goal);
        ...
        <div className="space-y-6">
          ...
          {extraPrizeGoal && (
            <div className="rounded-xl py-2.5 px-4 bg-pink/10 flex items-center gap-2 reveal-up">
              ...
              <p className="text-xs font-bold text-navy">Prêmio extra ativo este mês: {extraPrizeGoal.non_monetary_prize} — vale se a loja bater a {extraPrizeGoal.name}</p>
            </div>
          )}
