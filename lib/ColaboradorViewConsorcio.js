...
export default function ColaboradorViewConsorcio({ profile, tab, viewedByManager = false, onBack }) {
  ...
  // Filtro de "Minhas ligações" (2026-07-21, pedido do Felipe: colaborador do consórcio também
  // precisa poder filtrar os próprios leads) — rascunho (draft*) só passa a valer quando clica em
  // "Aplicar filtros", mesmo padrão do resto do app (nenhum filtro pode recalcular ao digitar/
  // mudar campo, sem exceção).
  const [draftListFilter, setDraftListFilter] = useState("abertos");
  ...
  const listaFiltrada = (listFilter === "todos" ? leads : leads.filter((l) => l.status !== "vendido" && l.status !== "perdido" && l.status !== "vendido_pendente"))
    .filter((l) => {
      if (!buscaLead.trim()) return true;
      const q = buscaLead.trim().toLowerCase();
      return (l.nome_completo || "").toLowerCase().includes(q) || (l.telefone || "").includes(q);
    });
  const filtroLeadPendente = draftListFilter !== listFilter || draftBuscaLead !== buscaLead;

  function aplicarFiltroLeads(e) {
    if (e) e.preventDefault();
    setListFilter(draftListFilter);
    setBuscaLead(draftBuscaLead);
  }

  function limparFiltroLeads() {
    setDraftListFilter("abertos");
    setDraftBuscaLead("");
      ...
      {tab === "atividades" && (
        <div className="space-y-6">
          ...
          <div className="card-dark">
            <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
              <p className="label-dark mb-0 flex items-center gap-1.5"><ListTodo size={14} className="text-goldlight" /> Minhas ligações</p>
              <div className="flex items-center gap-1 bg-white/5 rounded-full p-0.5 border border-white/10 shrink-0">
                <button type="button" onClick={() => setDraftListFilter("abertos")} className={`text-[11px] font-bold px-2.5 py-1 rounded-full transition-all ${draftListFilter === "abertos" ? "bg-goldlight text-navy" : "text-white/60"}`}>Em aberto</button>
                <button type="button" onClick={() => setDraftListFilter("todos")} className={`text-[11px] font-bold px-2.5 py-1 rounded-full transition-all ${draftListFilter === "todos" ? "bg-goldlight text-navy" : "text-white/60"}`}>Todos</button>
              </div>
            </div>
            <form onSubmit={aplicarFiltroLeads} className="flex flex-col sm:flex-row items-stretch sm:items-end gap-2 mb-3">
              <div className="flex-1">
                <div className="relative">
                  <Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-white/40" />
                  <input
                    className="input !pl-9 !bg-white/5 !border-white/10 !text-white placeholder:text-white/40"
                    placeholder="nome ou telefone"
                    value={draftBuscaLead}
                    onChange={(e) => setDraftBuscaLead(e.target.value)}
                  />
                </div>
              </div>
              <div className="flex items-center gap-3 shrink-0">
                <button type="submit" className="btn !py-1.5 !text-xs whitespace-nowrap">Aplicar</button>
                {(!!buscaLead.trim() || listFilter !== "abertos" || filtroLeadPendente) && (
                  <button type="button" onClick={limparFiltroLeads} className="text-[11px] font-bold uppercase tracking-wider text-white/50 hover:text-goldlight whitespace-nowrap">
                    Limpar
                  </button>
                )}
              </div>
            </form>
            {filtroLeadPendente && <p className="text-[11px] text-warn mb-2">alterações pendentes — clique em Aplicar</p>}
            {listaFiltrada.length === 0 ? (
              <p className="text-sm text-white/50">Nenhuma ligação {listFilter === "abertos" ? "em aberto" : "registrada"} encontrada{buscaLead.trim() ? " com esse filtro" : ""}.</p>
            ) : (
              <ul>
