...
export default function ColaboradorViewConsorcio({ profile, tab, viewedByManager = false, onBack }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {!showCongrats && storeGoalCelebrations.length > 0 && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-navy/70 p-6">
          <Confetti />
          <div className="card max-w-sm w-full text-center animate-bounce-in border-purple/30">
            <div className="flex justify-center mb-3 animate-wiggle">
      ...
      {!showCongrats && storeGoalCelebrations.length === 0 && closingWinners.length > 0 && (
          ...
          >
            ...
            {metaAtual > 0 && (
              <div className="relative mt-3">
                <p className="text-[11px] font-bold text-navy/70 mb-1">Sua meta</p>
                <ProgressBar pct={Math.min(100, (vendasMes / metaAtual) * 100)} showLabel={false} height="h-2" />
              </div>
            )}
            {metaLojaGeral > 0 && (
              <div className="relative mt-3">
                <p className="text-[11px] font-bold text-navy/70 mb-1">Meta geral da loja</p>
                <ProgressBar pct={Math.min(100, (storeSoldLoja / metaLojaGeral) * 100)} showLabel={false} height="h-2" />
                {activeGoalGeralItem?.goal?.non_monetary_prize && (
                  <p className="text-xs font-bold text-navy mt-1.5 flex items-center gap-1.5">
                    <PartyPopper size={13} /> Prêmio da equipe: {activeGoalGeralItem.goal.non_monetary_prize}
                  </p>
                )}
              </div>
            )}
