...
function LeadRow({ lead, onAgendar, onResolve, onEdit, showResolve = true }) {
  ...
  const terminal = lead.status === "vendido" || lead.status === "perdido" || lead.status === "vendido_pendente";
  // Editar dados do lead (nome/telefone/endereço/email/ligação/observações) fica disponível pro
  // colaborador em qualquer status, MENOS quando já virou venda confirmada ('vendido') — a partir
  // daí só administrativo/sócio/supervisor/gerente podem editar (pedido do Felipe 2026-07-23).
  // Reforçado no banco por trigger (prevent_colaborador_edit_vendida_lead), isso aqui é só UX.
  const editable = lead.status !== "vendido";
...
export default function ColaboradorViewConsorcio({ profile, tab, viewedByManager = false, onBack }) {
  ...
  const abertosCount = leads.filter((l) => l.status !== "vendido" && l.status !== "perdido" && l.status !== "vendido_pendente").length;
