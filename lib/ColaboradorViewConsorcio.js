...
export default function ColaboradorViewConsorcio({ profile, tab, viewedByManager = false, onBack }) {
  ...
  // cadastrada, não só a que está "em jogo" (mesmo ajuste de ColaboradorView.js).
  const extraPrizeGoal = goals.find(({ goal }) => goal.non_monetary_prize && storeSoldLoja < Number(goal.store_total))?.goal
    || goals.find(({ goal }) => goal.non_monetary_prize)?.goal
    || null;
