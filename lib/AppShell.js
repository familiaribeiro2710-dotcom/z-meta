  ...
  return (
    <div className="min-h-screen">
      <header className="relative border-b-2 border-line bg-white/90 backdrop-blur sticky top-0 z-10 pt-[env(safe-area-inset-top)]">
        ...
        {tabs && (
          <nav className="max-w-5xl mx-auto px-3 sm:px-4 flex gap-4 sm:gap-6 -mb-px overflow-x-auto scrollbar-hide">
            {tabs.map((t) => (
              <button
                key={t.key}
                onClick={() => onTabChange(t.key)}
                className={`flex items-center gap-1.5 py-3 text-xs sm:text-sm font-bold tracking-wide border-b-[3px] transition-all shrink-0 whitespace-nowrap ${
                  activeTab === t.key
