              ...
              >
                {userAvatarUrl ? (
                  ...
                  <div
                    ...
                    style={{ background: "linear-gradient(135deg, #12203a, #1c2f52)" }}
                  >
                    {initials(userName)}
                  </div>
                )}
          ...
          <nav className="max-w-5xl mx-auto px-2 sm:px-4 flex gap-0.5 sm:gap-6 -mb-px overflow-x-auto">
            {tabs.map((t) => (
              <button
                ...
                className={`${t.hideOnMobile ? "hidden lg:flex" : "flex-1 sm:flex-none flex"} min-w-[62px] sm:min-w-0 shrink-0 flex-col sm:flex-row items-center justify-center gap-0.5 sm:gap-1.5 py-2 sm:py-3 text-[9px] sm:text-sm font-bold tracking-wide border-b-[3px] transition-all ${
                  activeTab === t.key
                ...
                style={
                  activeTab === t.key
                    ? { borderColor: "#c9a15a", borderBottomWidth: 3, borderBottomStyle: "solid" }
                    : undefined
                }
              >
