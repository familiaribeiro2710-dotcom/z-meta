  ...
  return (
    <div className="min-h-screen">
      <header className="relative border-b-2 border-line bg-white/90 backdrop-blur sticky top-0 z-10 pt-[env(safe-area-inset-top)]">
        ...
        {tabs && (
          <nav className="max-w-5xl mx-auto px-2 sm:px-4 flex gap-0.5 sm:gap-6 -mb-px">
            {tabs.map((t) => (
              <button
                key={t.key}
                onClick={() => onTabChange(t.key)}
                className={`flex-1 sm:flex-none min-w-0 flex flex-col sm:flex-row items-center justify-center gap-0.5 sm:gap-1.5 py-2 sm:py-3 text-[9px] sm:text-sm font-bold tracking-wide border-b-[3px] transition-all ${
                  activeTab === t.key
