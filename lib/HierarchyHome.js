              ...
              <>
                {tab === "atividades" && (() => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className={`relative grid grid-cols-2 sm:grid-cols-4 gap-x-3 gap-y-5 sm:gap-4 mt-6 pt-5 border-t ${borderCol}`}>
                ...
                {storeRanking.length > 0 && (
                  <div className="card animate-pop border-orange/20">
                    ...
                    <p className="text-[11px] text-muted mb-3">Todos os colaboradores da loja, ordenados por valor vendido no mês.</p>
                    <ul className="divide-y divide-line">
  ...
  return (
    <div className="card">
      ...
      {items.length === 0 ? (
        ...
        <ul className="divide-y divide-line">
          {items.map((it, idx) => (
            <li key={it.id} className="flex items-center justify-between gap-2 py-2 text-xs sm:text-sm">
              <span className="flex items-center gap-2 min-w-0">
                <span className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold shrink-0 ${idx === 0 ? "bg-orange text-white" : "bg-line text-muted"}`}>{idx + 1}</span>
                <span className="truncate">
                  <span className="font-medium text-navy">{it.name}</span>
                  <span className="text-muted"> · {it.lojaName}</span>
                </span>
              </span>
              <span className="font-semibold text-navy shrink-0 whitespace-nowrap">{formatValue(it.value)}</span>
            </li>
          ))}
        </ul>
      )}
