...
export default function HierarchyHome({ role, impersonate, viewerProfile, onExitImpersonation }) {
  ...
  const [heroConsorcio, setHeroConsorcio] = useState({ ligacoesHoje: 0, agendamentosHoje: 0, pendingToday: 0, soldLoja: 0, metaLoja: 0, leaderName: null });
  ...
  const loadHeroConsorcio = useCallback(async (lojaId, monthArg) => {
    if (!lojaId) {
      setHeroConsorcio({ ligacoesHoje: 0, agendamentosHoje: 0, pendingToday: 0, soldLoja: 0, metaLoja: 0, leaderName: null });
      return;
    }
    const { data: emps } = await supabase.from("profiles").select("id, full_name").eq("loja_id", lojaId).eq("role", "colaborador").eq("active", true);
    const empIds = (emps || []).map((e) => e.id);
    ...
    setHeroConsorcio({ ligacoesHoje, agendamentosHoje, pendingToday, soldLoja, metaLoja, leaderName });
  }, []);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (impersonate) {
        ...
        if (slug === "consorcio") {
          await loadHeroConsorcio(initialLoja, initialMonth);
        } else {
          await Promise.all([
    ...
    return () => { active = false; };
  }, [router, role, loadLojas, loadHero, loadHeroConsorcio, loadRankings, loadFaturamento, impersonate]);
  ...
  useEffect(() => {
    if (!didInit.current || categoriaSlug === "consorcio") return;
  ...
  if (viewingGerente) {
    const gerTabs = isConsorcio ? CONSORCIO_TABS : EMPRESA_TABS;
    const gerTab = tab === "metas" ? "metas" : "atividades";
    return (
      ...
      <div className="space-y-6">
        ...
        {lojas.length === 0 ? (
          ...
          <>
                ...
                {tab === "atividades" && isConsorcio && (
                  ...
                  >
                    ...
                    <div className="relative flex items-center gap-2 mb-3">
                      ...
                      <span className="text-xs font-bold uppercase tracking-wider text-white">Funil · {selectedLoja?.loja_name || "loja"}</span>
                    </div>
                    ...
                    <div className="relative grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-x-3 gap-y-5 sm:gap-4 mt-6 pt-5 border-t border-white/20">
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className={`relative grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-x-3 gap-y-5 sm:gap-4 mt-6 pt-5 border-t ${borderCol}`}>
                        ...
                        <div className="min-w-0">
                          <AutoFitText className={`text-lg sm:text-xl font-extrabold ${textMain}`}>{formatBRL(hero.prizesSoFar)}</AutoFitText>
            ...
            {tab === "rankings" && (
              isConsorcio ? (
                <div className="card"><p className="text-sm text-muted">Rankings cruzando lojas ainda não foi adaptado pra empresas de consórcio — em breve. Os números por loja já aparecem na aba Início.</p></div>
              ) : (
            ...
            {tab === "faturamento" && (
              isConsorcio ? (
                <div className="card"><p className="text-sm text-muted">Faturamento consolidado ainda não foi adaptado pra empresas de consórcio — em breve. Os números por loja já aparecem na aba Início.</p></div>
              ) : (
...
function LojaRankingCard({ byLoja, month }) {
  return (
    <div className="card animate-pop border-orange/20">
      <p className="label mb-3 flex items-center gap-1.5"><Store size={14} /> Ranking de lojas — {monthLabel(month)}</p>
      {byLoja.length === 0 ? (
        <p className="text-sm text-muted">Nenhum lançamento ainda.</p>
      ) : (
        <ul className="divide-y divide-line">
          {byLoja.map((l, idx) => (
            <li key={l.loja_id} className="flex items-center justify-between gap-2 py-2 text-xs sm:text-sm">
              <span className="flex items-center gap-2 min-w-0">
                <span
                  className={`w-6 h-6 rounded-full flex items-center justify-center text-[11px] font-bold shrink-0 ${
                    idx === 0 ? "bg-orange text-white" : "bg-line text-muted"
                ...
                <span className="font-medium text-navy truncate">{l.loja_name}</span>
              </span>
              <span className="font-semibold text-navy shrink-0 whitespace-nowrap">{formatBRL(l.total)}</span>
            </li>
          ))}
        </ul>
      )}
