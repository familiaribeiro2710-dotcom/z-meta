          ...
          <>
            <div className="flex items-start justify-between gap-3 flex-wrap">
              ...
              <div className="flex items-center gap-2 flex-wrap">
                <label className="flex items-center gap-1.5 bg-white border-2 border-purple/30 rounded-full pl-3 pr-2.5 py-1.5 shadow-soft hover:border-purple/60 transition-colors cursor-pointer whitespace-nowrap">
                  <Store size={14} className="text-purple shrink-0" />
                  <select
                    className="!border-0 !bg-transparent !p-0 !shadow-none !ring-0 text-xs font-bold text-navy focus:outline-none cursor-pointer max-w-[160px]"
            ...
            {(tab === "atividades" || tab === "metas") && (
              <>
                {tab === "atividades" && (() => {
                  ...
                  const textMain = isSocio ? "text-navy" : "text-white";
                  const textSub = isSocio ? "text-navy/70" : "text-white/80";
                  const circleBg = isSocio ? "bg-navy/10" : "bg-white/15";
                  const borderCol = isSocio ? "border-navy/15" : "border-white/20";
                  return (
                    <div
                    ...
                    >
                      ...
                      <div className={`relative grid grid-cols-2 sm:grid-cols-4 gap-4 mt-6 pt-5 border-t ${borderCol}`}>
                        <div>
                          <p className={`text-xl font-extrabold ${textMain}`}>{formatBRL(restoDaMeta)}</p>
                          <p className={`text-[11px] font-semibold ${textSub} mt-0.5 flex items-center gap-1`}><Rocket size={11} /> Falta pra meta do mês</p>
                        </div>
                        <div>
                          <p className={`text-xl font-extrabold ${textMain}`}>{remaining}</p>
                          <p className={`text-[11px] font-semibold ${textSub} mt-0.5 flex items-center gap-1`}><CalendarClock size={11} /> Dias restantes no mês</p>
                        </div>
                        <div>
                          <p className={`text-xl font-extrabold ${textMain}`}>{hero.pendingToday}</p>
                          <p className={`text-[11px] font-semibold ${textSub} mt-0.5 flex items-center gap-1`}><ListTodo size={11} /> Atividades pendentes</p>
                        </div>
                        <div>
                          <p className={`text-xl font-extrabold ${textMain}`}>{formatBRL(hero.prizesSoFar)}</p>
                          <p className={`text-[11px] font-semibold ${textSub} mt-0.5 flex items-center gap-1`}><Gift size={11} /> Premiações</p>
                        </div>
                      </div>
                    </div>
                  );
...
function SupervisoresTab({ empresaId, lojas }) {
  ...
  async function setAccessFor(supervisorId, lojaId, permission) {
    ...
    await fetch("/api/admin/hierarchy-access", {
      method: "POST",
      headers: { "Content-Type": "application/json", Authorization: `Bearer ${session.access_token}` },
      body: JSON.stringify({ action: "setAccess", supervisorId, lojaId, permission }),
    });
    setTogglingKey(null);
    await load();
  }
