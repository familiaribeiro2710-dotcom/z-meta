...
export default function HierarchyHome({ role, impersonate, viewerProfile, onExitImpersonation }) {
  ...
  useEffect(() => {
    if (tab !== "atividades" || impersonate || role !== "supervisor" || categoriaSlug === "consorcio" || !profile) return;
    let active = true;
    (async () => {
    ...
    return () => { active = false; };
  }, [tab, profile, impersonate, role, categoriaSlug]);
  ...
  return (
    ...
    >
      <div className="space-y-6">
        {impersonationBanner}
        {role === "supervisor" && !impersonate && categoriaSlug !== "consorcio" && (
          <MonthClosingAskModal
            open={showClosingAsk}
            role="supervisor"
            monthLabel={closingTargetMonthLabel()}
            onConfirm={async () => {
              const { error } = await supabase.rpc("confirm_month_closing_supervisor", { p_month: closingTargetMonth() });
