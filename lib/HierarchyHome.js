...
import { isCrmCategoria } from "./categoria";
...
import { needsClosingAsk, closingTargetMonth, closingTargetMonthLabel } from "./monthClosing";
import ConfirmModal from "./ConfirmModal";
import PendingActivitiesModal from "./PendingActivitiesModal";
import ReportOptionsModal from "./ReportOptionsModal";
import { buildMonthlyReportData, downloadMonthlyReportPdf } from "./monthlyReport";
import { calcIndividualPct, formatBRL, formatPct, currentGoalTarget } from "./scoring";
import { greeting, todayStr, firstDayOfMonth, nextMonth, remainingDaysInMonth, monthLabel, isTaskDueOn, daysElapsedInMonth } from "./date";
...
// 2026-08-08: "Pipeline" — precisa ficar em TODA lista de abas de colaborador consórcio (essa
// aqui, a de app/colaborador/page.js e a de app/admin/page.js/TABS_CONSORCIO_COLAB), senão some
// especificamente em "ver como colaborador" (bug real: só tinha sido adicionada na lista de
// app/colaborador/page.js, faltando aqui e no admin).
// 2026-08-12: "Leads" entrou aqui também (generalização pra todos os papéis, ver LeadsTab em
// ConsorcioDashboard.js) — mesma régua do Pipeline logo acima: sem ela, a aba some especificamente
// em "ver como colaborador".
...
export default function HierarchyHome({ role, impersonate, viewerProfile, onExitImpersonation }) {
  ...
  // 2026-08-31, pedido do Felipe: clicar no número de "Atividades pendentes" do herocard abre a
  // lista de verdade (PendingActivitiesModal), com opção de já marcar como feita ali mesmo —
  // escopada à loja selecionada (storeRanking em vestuário, consorcioLojaEmps em consórcio).
  const [pendingModalOpen, setPendingModalOpen] = useState(false);
  // 2026-08-31, pedido do Felipe: botão de baixar o relatório mensal (faturamento/comissão/
  // premiações) em PDF, direto do herocard — mockup aprovado antes de implementar
  // (MOCKUP_RELATORIO_MENSAL_PDF.html). 2026-08-31: antes de gerar, um modal (ReportOptionsModal)
  // deixa escolher quais seções entram no arquivo.
  const [reportLoading, setReportLoading] = useState(false);
  const [reportModalOpen, setReportModalOpen] = useState(false);

  // herocard simplificado pra lojas de CONSÓRCIO — só o essencial de "Início" pra não deixar a
  // tela vazia. Rankings/Faturamento cross-loja (loadRankingsConsorcio/loadFaturamentoConsorcio,
  // ver abaixo) já foram portados pra consórcio desde 2026-07-20 — não mostram mais aviso "em breve".
  const [heroConsorcio, setHeroConsorcio] = useState({
    ...
    nonMonetaryPrize: null,
  });

  // "Agenda de hoje" da loja de CONSÓRCIO selecionada — mesmo card que o gerente já tem (equipe
  // inteira), aqui agregado pra loja toda (todos os colaboradores/equipes), com filtro por
  // colaborador. Alimentado dentro de loadHeroConsorcio, reaproveitando a mesma query de
  // crm_leads (sem round-trip extra).
  const [todayMeetingsConsorcio, setTodayMeetingsConsorcio] = useState([]);
  const [consorcioLojaEmps, setConsorcioLojaEmps] = useState([]);
  const [agendaFiltroEmpConsorcio, setAgendaFiltroEmpConsorcio] = useState("todos");
  ...
  // Vendas — 2026-07-23, pedido do Felipe: sócio/supervisor precisam editar uma venda já
  // confirmada em qualquer loja que gerenciam (cross-loja, mesmo padrão de Rankings/Faturamento).
  // Aprovar/recusar continua exclusivo do administrativo. Só existe em consórcio (vestuário não
  // tem crm_leads).
  // Online — 2026-07-27, pedido do Felipe: módulo independente de vendas online, exclusivo de
  // vestuário (consórcio não tem, é conceito de loja física com metas/premiação por venda
  // avulsa). Entra logo depois de Metas, antes de Rankings/Faturamento.
  const tabs = [
    ...BASE_TABS.slice(0, 2),
    ...(!isCrmCategoria(categoriaSlug) ? [{ key: "online", label: "Online", Icon: Globe }] : []),
    // 2026-08-08 (pedido do Felipe): aba Pipeline, exclusiva de consórcio, escondida em telas
    // pequenas (hideOnMobile, ver AppShell.js) — mesmo componente/escopo (lojaId da loja
    // selecionada) do <ConsorcioDashboard tab="pipeline"> renderizado mais abaixo.
    // 2026-08-12 (pedido do Felipe): "Leads" generalizada pra sócio/supervisor — mesmo esquema
    // (lojaId da loja selecionada), cai no <ConsorcioDashboard tab="leads"> logo abaixo do Pipeline.
    ...(isCrmCategoria(categoriaSlug) ? [{ key: "pipeline", label: "Pipeline", Icon: Kanban, hideOnMobile: true }, { key: "leads", label: "Leads", Icon: Users }] : []),
    ...BASE_TABS.slice(2),
    ...(isCrmCategoria(categoriaSlug) ? [{ key: "vendas", label: "Vendas", Icon: Coins }] : []),
    ...(role === "socio" ? [{ key: "supervisores", label: isCrmCategoria(categoriaSlug) ? "Hierarquia" : "Supervisores", Icon: ShieldCheck }] : []),
  ];
  ...
  const loadHero = useCallback(async (lojaId, monthArg) => {
    ...
    // 2026-09-03: não filtra active=true aqui — soldLoja/ranking do mês não podem sumir com a
    // venda de quem foi desativado depois daquele mês. `pendingToday` (só relevante pra HOJE, não
    // pra mês fechado) usa `activeEmpIds` à parte, esse sim só time atual — não faz sentido contar
    // tarefa pendente hoje de quem já nem trabalha mais aqui.
    const { data: emps } = await supabase.from("profiles").select("id, full_name, avatar_url, active").eq("loja_id", lojaId).eq("role", "colaborador");
    const empIds = (emps || []).map((e) => e.id);
    const activeEmpIds = (emps || []).filter((e) => e.active).map((e) => e.id);
    ...
    let pendingToday = 0;
    if (monthArg === firstDayOfMonth(todayStr()) && activeEmpIds.length) {
      const { data: activeTasks } = await supabase.from("tasks").select("id").in("employee_id", activeEmpIds).eq("active", true);
    ...
    const { data: goalRows } = await supabase
      .from("sales_goals")
      .select("id, name, store_total, commission_pct_gerente, non_monetary_prize, pacing")
      .eq("loja_id", lojaId)
      .eq("month", monthArg)
    ...
    // ativo hoje OU vendeu algo no mês visualizado — senão todo ex-colaborador da loja (mesmo sem
    // nenhuma venda naquele mês) entraria no ranking com R$0.
    const ranking = (emps || [])
      .filter((emp) => emp.active || (soldByEmp[emp.id] || 0) > 0)
    ...
    // 2026-08-31: por loja/mês (não por empIds) de propósito — assim inclui também premiações
    // avulsas (employee_id null, ex.: estoquista sem cadastro), que são despesa real da loja mas
    // não pertencem a nenhum colaborador cadastrado pra filtrar por id.
    let prizesSoFar = 0;
    {
      const { data: prizeRows } = await supabase.from("employee_prizes").select("amount").eq("loja_id", lojaId).eq("month", monthArg);
  ...
  const loadHeroConsorcio = useCallback(async (lojaId, monthArg) => {
    if (!lojaId) {
      ...
      setTodayMeetingsConsorcio([]);
      setConsorcioLojaEmps([]);
      setAgendaFiltroEmpConsorcio("todos");
      return;
    }
    setAgendaFiltroEmpConsorcio("todos");
    // 2026-09-03: não filtra active=true aqui — soldLoja/conversão do mês não podem sumir com a
    // venda de quem foi desativado depois daquele mês. `pendingToday` usa `activeEmpIds` à parte.
    const { data: emps } = await supabase.from("profiles").select("id, full_name, active").eq("loja_id", lojaId).eq("role", "colaborador");
    const empIds = (emps || []).map((e) => e.id);
    const activeEmpIds = (emps || []).filter((e) => e.active).map((e) => e.id);
    setConsorcioLojaEmps(emps || []);
    ...
    if (empIds.length) {
      const { data: leadRows } = await supabase.from("crm_leads").select("id, employee_id, nome_completo, telefone, status, valor, data_ligacao, agendamento_at, vendido_at").in("employee_id", empIds);
      ...
      setTodayMeetingsConsorcio(rows.filter((l) => l.agendamento_at && l.agendamento_at.slice(0, 10) === todayNow));
    ...
    const { data: goalRows } = await supabase
      ...
      .select("store_total, non_monetary_prize")
      .eq("loja_id", lojaId)
      .eq("month", monthArg);
    const metaLoja = currentGoalTarget((goalRows || []).map((g) => g.store_total), soldLoja);
    ...
    setHeroConsorcio({
      ...
      nonMonetaryPrize: activeGoalRow?.non_monetary_prize || null,
    });
  }, []);
  ...
  const loadRankings = useCallback(async (lojaIds, monthArg) => {
    ...
    // 2026-09-03: NÃO filtra mais por active=true aqui (bug real corrigido — rankings de um mês
    // fechado sumiam depois de uma desativação posterior). Cada lista abaixo (sellers/prizes/bars/
    // commissions) já só entra quem teve valor > 0 naquele mês, então quem está inativo e não tem
    // nada no mês visualizado continua de fora naturalmente, sem precisar do filtro por status atual.
  ...
  const loadRankingsConsorcio = useCallback(async (lojaIds, monthArg) => {
    ...
    // 2026-09-03: mesmo ajuste de loadRankings acima — não filtra active=true aqui, cada lista já
    // só entra quem teve valor > 0 no mês visualizado.
    const { data: emps } = await supabase
      .from("profiles")
      .select("id, full_name, loja_id")
      .in("loja_id", lojaIds)
  ...
  useEffect(() => {
    ...
    if (isCrmCategoria(categoriaSlug)) {
  ...
  // Ritual de fechamento de mês — só pro supervisor de verdade, na própria sessão dele (nunca
  // via "ver como" do master admin, onde auth.uid() continua sendo o master e a RPC falharia
  // por não ser supervisor). Reavalia toda vez que a aba Início é reaberta, porque o componente
  // não desmonta ao trocar de aba — mesmo padrão de GerenteView.js.
  useEffect(() => {
    if (tab !== "atividades" || impersonate || role !== "supervisor" || !profile) return;
    let active = true;
    (async () => {
      const needsAsk = await needsClosingAsk(profile.id);
      if (active) setShowClosingAsk(needsAsk);
    })();
    return () => { active = false; };
  }, [tab, profile, impersonate, role]);
  ...
  async function handleDownloadReport(options) {
    if (reportLoading || !selectedLojaId) return;
    setReportLoading(true);
    try {
      const { data: empresaRow } = await supabase.from("empresas").select("name").eq("id", profile.empresa_id).single();
      const reportData = await buildMonthlyReportData({
        lojaId: selectedLojaId,
        isConsorcio,
        lojaName: selectedLoja?.loja_name || "",
        empresaName: empresaRow?.name || "",
        month,
      });
      await downloadMonthlyReportPdf(reportData, options);
  ...
  return (
    ...
    >
      <div className="space-y-6">
        {impersonationBanner}
        {role === "supervisor" && !impersonate && (
          <MonthClosingAskModal
            open={showClosingAsk}
            role="supervisor"
            monthLabel={closingTargetMonthLabel()}
            onConfirm={async () => {
              const fn = isCrmCategoria(categoriaSlug) ? "confirm_month_closing_supervisor_consorcio" : "confirm_month_closing_supervisor";
              const { error } = await supabase.rpc(fn, { p_month: closingTargetMonth() });
        ...
        <PendingActivitiesModal
          ...
          empIds={(isConsorcio ? consorcioLojaEmps : storeRanking).map((e) => e.id)}
          employeeNameById={Object.fromEntries((isConsorcio ? consorcioLojaEmps : storeRanking).map((e) => [e.id, e.full_name || e.name]))}
          onChanged={() => (isConsorcio ? loadHeroConsorcio(selectedLojaId, month) : loadHero(selectedLojaId, month))}
        />
          ...
          <>
            <div className="space-y-3">
              ...
              {/* seletor de loja só faz sentido nas abas que mostram dado de UMA loja por vez
                  (Início/Metas/Online/Pipeline) — Rankings e Faturamento agregam todas as lojas de
                  uma vez, e Supervisores nem usa loja ou mês, então nenhum dos dois aparece lá.
                  2026-08-09: Pipeline entrou aqui também (bug real corrigido — a aba ficava com o
                  cabeçalho ativo mas o conteúdo inteiro em branco, porque esse bloco nunca incluía
                  "pipeline" na lista de tabs válidas). Pipeline não usa MonthNav de propósito — o
                  quadro mostra tudo que está aberto agora, sem recorte por mês (pedido do Felipe).
                  2026-08-12: "leads" entrou pelo mesmo motivo do Pipeline (loja selecionada, sem
                  recorte por mês — a listagem é histórica, não mensal). */}
              {(tab === "atividades" || tab === "metas" || tab === "online" || tab === "rankings" || tab === "faturamento" || tab === "pipeline" || tab === "leads") && (
                <div className="flex items-center gap-2">
                  {(tab === "atividades" || tab === "metas" || tab === "online" || tab === "pipeline" || tab === "leads") && (
                    <SelectField
                      icon={Store}
                      className="flex-1"
                  ...
                  {tab !== "pipeline" && tab !== "leads" && (
                    <MonthNav
                      ...
                      // Metas/Online liberam ver o mês seguinte adiantado (planejar meta antes do
                      // mês virar de verdade, pedido do Felipe) — Início/Rankings/Faturamento
                      // continuam travados no mês corrente.
                      maxMonth={tab === "metas" || tab === "online" ? nextMonth(today) : firstDayOfMonth(today)}
                    />
                  )}
                  {tab === "atividades" && (
                    <button
                      ...
                      disabled={reportLoading || !selectedLojaId}
                      className="btn-outline !py-1.5 !text-xs flex items-center gap-1.5 whitespace-nowrap shrink-0 disabled:opacity-60"
                    >
            ...
            {/* 2026-08-09: "pipeline" entrou aqui — sem isso, o <ConsorcioDashboard>/<EmpresaDashboard>
                (que já sabe renderizar tab==="pipeline" por dentro) nunca chegava a montar, e a aba
                ficava com o cabeçalho ativo mas a tela inteira em branco. Os blocos internos (herocard,
                cards de Metas etc.) continuam corretamente restritos às próprias condições
                (tab === "atividades"/tab === "metas"), só o <ConsorcioDashboard> em si precisava
                ficar acessível também com tab === "pipeline". 2026-08-12: "leads" entrou pelo
                mesmo motivo (generalização da aba Leads, ver LeadsTab em ConsorcioDashboard.js). */}
            {(tab === "atividades" || tab === "metas" || tab === "pipeline" || tab === "leads") && (
              <>
                {tab === "atividades" && isConsorcio && (() => {
                  const isSocio = role === "socio";
                  const accent = isSocio ? "#94a3b8" : "#2563eb";
                  const accentLight = isSocio ? "#cbd5e1" : "#60a5fa";
                  return (
                    <div
                      className="relative overflow-hidden rounded-3xl p-6 sm:p-7 reveal-up border-2"
                      style={{ background: "linear-gradient(160deg, #1c2e4d 0%, #12203a 100%)", borderColor: accent, boxShadow: "0 10px 26px rgba(18,32,58,0.35)" }}
                    >
                      <div className="absolute -top-14 -right-10 w-48 h-48 rounded-full" style={{ background: `${accent}26` }} />
                      <div className="relative flex items-center gap-2 mb-3">
                        <Store size={18} style={{ color: accentLight }} />
                        <span className="text-xs font-bold uppercase tracking-wider" style={{ color: accentLight }}>Funil · {selectedLoja?.loja_name || "loja"}</span>
                      </div>
                      <AutoFitText className="relative text-4xl sm:text-5xl font-extrabold text-white leading-tight">{formatBRL(heroConsorcio.soldLoja)}</AutoFitText>
                      ...
                      <p className="relative text-xs font-semibold text-white/60 mt-1">Vendido no mês — {monthLabel(month)}</p>

                      <div className="relative grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-x-3 gap-y-5 sm:gap-4 mt-6 pt-5 border-t border-white/10">
                        <div className="min-w-0">
                          <AutoFitText className="text-lg sm:text-xl font-extrabold text-white">{heroConsorcio.ligacoesHoje}</AutoFitText>
                          <p className="text-[11px] font-semibold text-white/60 mt-0.5 flex items-center gap-1"><PhoneCall size={11} className="shrink-0" /> Ligações hoje</p>
                        </div>
                        <div className="min-w-0">
                          <AutoFitText className="text-lg sm:text-xl font-extrabold text-white">{heroConsorcio.agendamentosHoje}</AutoFitText>
                        ...
                        >
                          <AutoFitText className="text-lg sm:text-xl font-extrabold text-white">{heroConsorcio.pendingToday}</AutoFitText>
                          <p className="text-[11px] font-semibold text-white/60 mt-0.5 flex items-center gap-1"><ListTodo size={11} className="shrink-0" /> Atividades pendentes</p>
                        </button>
                        <div className="min-w-0">
                          <AutoFitText className="text-lg sm:text-xl font-extrabold text-white">{heroConsorcio.leaderName || "—"}</AutoFitText>
                          <p className="text-[11px] font-semibold text-white/60 mt-0.5 flex items-center gap-1"><Trophy size={11} className="shrink-0" /> Líder de vendas</p>
                        </div>
                      </div>
                    </div>
                  );
                ...
                {tab === "atividades" && !isConsorcio && (() => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="relative flex items-center gap-2 mb-3">
                        <Store size={18} style={{ color: accentLight }} />
                        <span className="text-xs font-bold uppercase tracking-wider" style={{ color: accentLight }}>Meta de hoje · {selectedLoja?.loja_name || "loja"}</span>
                      </div>
                              ...
                              <li key={g.id} className="py-2.5">
                                <div className="flex items-center justify-between gap-2 text-xs sm:text-sm">
                                  <span className="font-medium text-navy flex items-center gap-1.5 min-w-0">
                                    ...
                                    {activeGoal?.id === g.id && <span className="badge bg-gold/15 text-gold shrink-0">em jogo</span>}
                                  </span>
                          ...
                          {goalsList.map((g) => {
                            ...
                            return (
                              <li key={g.id} className="py-2.5">
                                ...
                                <div className="mt-1.5"><ProgressBar pct={goalPct} height="h-2" /></div>
                                {activeGoal?.id === g.id && g.non_monetary_prize && (
                ...
                {/* 2026-07-27: card de Conversão daqui foi removido — era duplicado byte-a-byte
                    do card "Conversão" que <ConsorcioDashboard> já renderiza na própria aba Funil
                    (mesmo escopo: loja selecionada, mesmo cohort por data_ligacao). O
                    `heroConsorcio.pct*`/`ligacoesMes`/`agendadosMes`/`vendidosMesCohort` continuam
                    computados em loadHeroConsorcio (não usados em outro lugar por enquanto, deixados
                    como dado morto inofensivo pra não arriscar quebrar algo mexendo nesse cálculo).
                    2026-07-27 (pedido do Felipe): Agenda de hoje precisa aparecer DENTRO da aba
                    Funil (depois da navegação de sub-abas, junto com o resto do conteúdo do
                    Funil/Placar) — não acima da barra de abas. Primeiro card dessa área. */}
                {tab === "atividades" && isConsorcio && (atSub === "funil" || atSub === "placar") && (
                  <div className="card animate-pop border-blue/20">
                    <div className="flex items-center justify-between gap-2 mb-3 flex-wrap">
                      <p className="label mb-0 flex items-center gap-1.5"><CalendarDays size={14} /> Agenda de hoje — {selectedLoja?.loja_name || "loja"}</p>
                      {consorcioLojaEmps.length > 1 && (
                        <SelectField icon={Users} className="w-full sm:w-56" value={agendaFiltroEmpConsorcio} onChange={(e) => setAgendaFiltroEmpConsorcio(e.target.value)}>
                          <option value="todos">Todos os colaboradores</option>
                          {consorcioLojaEmps.map((e) => <option key={e.id} value={e.id}>{e.full_name}</option>)}
                        </SelectField>
                      )}
                    </div>
                    {(() => {
                      const meetingsFiltradas = agendaFiltroEmpConsorcio === "todos"
                        ? todayMeetingsConsorcio
                        : todayMeetingsConsorcio.filter((l) => l.employee_id === agendaFiltroEmpConsorcio);
                      if (meetingsFiltradas.length === 0) {
                        return <p className="text-sm text-muted">{agendaFiltroEmpConsorcio === "todos" ? "Nenhum agendamento pra hoje nessa loja." : "Nenhum agendamento pra hoje desse colaborador."}</p>;
                      }
                      const empNameByIdConsorcio = {};
                      consorcioLojaEmps.forEach((e) => { empNameByIdConsorcio[e.id] = e.full_name; });
                      return (
                        <ul className="divide-y divide-line">
                          {meetingsFiltradas.map((l) => (
                            <li key={l.id} className="flex items-center justify-between gap-2 py-2 text-sm">
                              <span className="flex items-center gap-2 min-w-0">
                                <span className={`badge !text-[10px] shrink-0 ${VENDAS_HIER_STATUS_CHIP[l.status] || VENDAS_HIER_STATUS_CHIP.novo}`}>
                                  {l.agendamento_at ? new Date(l.agendamento_at).toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" }) : "—"}
                                </span>
                                <span className="min-w-0">
                                  <span className="block font-medium text-navy truncate">{l.nome_completo}</span>
                                  <span className="block text-[11px] text-muted truncate">{empNameByIdConsorcio[l.employee_id] || "—"}</span>
                                </span>
                              </span>
                ...
                {tab === "atividades" && isConsorcio && (atSub === "funil" || atSub === "placar") && heroConsorcio.metaLoja > 0 && (
                  <div className="card animate-pop border-orange/20">
                    ...
                    <ProgressBar pct={Math.min(100, (heroConsorcio.soldLoja / heroConsorcio.metaLoja) * 100)} />
                    <p className="text-xs text-muted mt-2">{formatBRL(heroConsorcio.soldLoja)} vendido de {formatBRL(heroConsorcio.metaLoja)} de meta.</p>
                  </div>
                )}
              </>
            )}

            {/* Online (2026-07-27) — módulo independente, exclusivo de vestuário, escopado à
                loja selecionada (mesmo padrão de Início/Metas). Reaproveita o EmpresaDashboard
                (que já carrega `employees` escopado certo por viewerRole), só passando
                tab="online" explicitamente — não precisa entrar no bloco gigante acima. */}
            {tab === "online" && !isConsorcio && (
              <EmpresaDashboard
                lojaId={selectedLojaId}
                empresaId={profile.empresa_id}
                viewerRole={canManageSelected ? role : "leitor"}
            ...
            {tab === "vendas" && isConsorcio && (
              <VendasTabHierarquia lojas={lojas} profile={profile} />
            )}
  ...
  return (
    <div className="card-dark animate-pop">
      ...
      {byLoja.length === 0 ? (
        ...
        <ul>
          {byLoja.map((l, idx) => (
            <li key={l.loja_id} className="row-card reveal-up" style={{ animationDelay: `${idx * 60}ms` }}>
              <span className={`rank-pos ${rankPosClass(idx)}`}>{idx + 1}</span>
              ...
              <span className="font-bold text-goldlight text-xs sm:text-sm shrink-0 whitespace-nowrap"><CountUp value={l.total} currency /></span>
            </li>
          ))}
        </ul>
      )}
...
function RankingCard({ title, Icon, items, format = "currency", emptyLabel }) {
  return (
    <div className="card-dark">
      ...
      {items.length === 0 ? (
        ...
        <ul>
          {items.map((it, idx) => (
            <li key={it.id} className="row-card reveal-up" style={{ animationDelay: `${idx * 60}ms` }}>
              <span className={`rank-pos ${rankPosClass(idx)}`}>{idx + 1}</span>
              ...
              <p className="font-bold text-goldlight text-xs sm:text-sm shrink-0 whitespace-nowrap">
                {format === "percent" ? <CountUp value={it.value} decimals={1} suffix="%" /> : <CountUp value={it.value} currency />}
              </p>
            </li>
          ))}
        </ul>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        <RankingCard title="Top vendedores" Icon={Trophy} items={rankings.sellers} format="currency" emptyLabel="Nenhuma venda lançada ainda." />
        <RankingCard title="Mais premiados" Icon={Gift} items={rankings.prizes} format="currency" emptyLabel="Nenhuma premiação lançada ainda." />
        <RankingCard title="Líder de tarefas concluídas" Icon={Medal} items={rankings.bars} format="percent" emptyLabel="Sem dados de tarefas ainda." />
        <RankingCard title="Mais comissionados" Icon={Award} items={rankings.commissions} format="currency" emptyLabel="Ninguém comissionado ainda." />
      </div>
    </div>
  );
}
...
const VENDAS_HIER_STATUS_LABEL = {
  novo: "Novo",
  agendado: "Agendado",
  follow_up: "Follow-up",
  perdido: "Perdido",
...
const VENDAS_HIER_PAGE_SIZE = 20;

// Aba Vendas de sócio/supervisor no segmento consórcio (2026-07-23) — cross-loja (todas as lojas
// que a pessoa gerencia/vê), mesmo padrão de Rankings/Faturamento. Aprovar/recusar uma venda
// pendente continua exclusivo do administrativo (can_confirm_vendas) — aqui é só leitura da fila
// pendente + edição de vendas já confirmadas ('vendido'), mesma UX de GerenteViewConsorcio.js.
// RLS (crm_leads_update: can_manage_loja(loja_id)) só libera UPDATE nas lojas em que a permission
// é 'gerenciar' — pro sócio isso é sempre todas (loadLojas força "gerenciar" fixo); pro supervisor
// depende de loja_access de verdade, então o botão Editar fica escondido nas lojas onde ele só tem
// 'ver' (senão o clique falharia silenciosamente contra a RLS).
function VendasTabHierarquia({ lojas, profile }) {
  ...
  const lojaIds = lojas.map((l) => l.loja_id);
  const lojaNameById = {};
  const manageableLojaIds = new Set();
  lojas.forEach((l) => {
    lojaNameById[l.loja_id] = l.loja_name;
    if (l.permission === "gerenciar") manageableLojaIds.add(l.loja_id);
  });

  const load = useCallback(async () => {
    if (!lojaIds.length) { setLeads([]); setLoading(false); return; }
    setLoading(true);
    const { data } = await supabase.from("crm_leads").select("*").in("loja_id", lojaIds).order("created_at", { ascending: false });
    setLeads(data || []);
    const empIds = [...new Set((data || []).map((l) => l.employee_id))];
    if (empIds.length) {
  ...
  const totalPages = Math.max(1, Math.ceil(vendasFiltradas.length / VENDAS_HIER_PAGE_SIZE));
  const pageClamped = Math.min(page, totalPages - 1);
  const vendasPage = vendasFiltradas.slice(pageClamped * VENDAS_HIER_PAGE_SIZE, pageClamped * VENDAS_HIER_PAGE_SIZE + VENDAS_HIER_PAGE_SIZE);

  const filtrosPendentes = draftFiltroStatus !== filtroStatus || draftFiltroLoja !== filtroLoja || draftBusca !== busca;
  const filtroAtivo = filtroStatus !== "vendido_pendente" || filtroLoja !== "todas" || !!busca.trim();
  ...
  function limparFiltros() {
    setDraftFiltroStatus("vendido_pendente"); setDraftFiltroLoja("todas"); setDraftBusca("");
    setFiltroStatus("vendido_pendente"); setFiltroLoja("todas"); setBusca("");
    setPage(0);
  }
                  ...
                  <div className="flex items-start justify-between gap-2 flex-wrap">
                    <div className="min-w-0">
                      <p className="text-sm font-bold text-navy flex items-center gap-1.5 flex-wrap">
                        ...
                        <span className={`badge !text-[10px] shrink-0 ${VENDAS_HIER_STATUS_CHIP[l.status] || VENDAS_HIER_STATUS_CHIP.novo}`}>{VENDAS_HIER_STATUS_LABEL[l.status] || l.status}</span>
                      </p>
                      ...
                      {l.status === "em_negociacao" && l.valor != null && <p className="text-sm font-bold mt-1 text-teal">Negociando {formatBRL(l.valor)}</p>}
                      {l.venda_motivo_recusa && <p className="text-[11px] text-danger mt-1">Recusada antes: {l.venda_motivo_recusa}</p>}
                    </div>
                    {l.status === "vendido" && manageableLojaIds.has(l.loja_id) && (
...
function SupervisoresTab({ empresaId, lojas, isConsorcio = false }) {
  ...
  async function createSupervisor(e) {
    ...
    setMsg(
      json.pending
        ? `Cadastro enviado! Usuário: ${json.username}${json.defaultPassword ? ` · senha padrão: ${json.defaultPassword}` : ""}. Aguardando aprovação do Master Admin antes de conseguir acessar.`
        : `${roleLabel} criado! Usuário: ${json.username}${json.defaultPassword ? ` · senha padrão: ${json.defaultPassword}` : ""}`
    );
                    ...
                    {[{ key: "supervisor", label: "Supervisor" }, { key: "administrativo", label: "Administrativo" }].map((opt) => (
                      <button
                        ...
                        className={`px-3 py-1.5 rounded-full text-xs font-bold transition-colors border ${
                          roleType === opt.key ? "bg-navyfixed text-white border-navyfixed" : "border-line text-muted hover:border-navy"
                        }`}
                      >
                        {opt.label}
                      </button>
                            ...
                            {["ver", "gerenciar"].map((opt) => (
                              <button
                                ...
                                className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wide border transition-colors ${
                                  perm === opt ? "bg-navyfixed text-white border-navyfixed" : "border-line text-muted hover:border-navy"
                                }`}
                              >
                                {opt}
                              </button>
                ...
                return (
                  <div key={sup.id} className="border-b border-line last:border-0 pb-2">
                    <button
                      ...
                      className="w-full text-sm flex items-center justify-between gap-2 py-1.5 hover:text-gold transition-colors"
                    >
                              ...
                              <span key={a.loja_id} className="inline-flex items-center gap-1">
                                <button
                                  ...
                                  className={`badge transition-colors active:scale-95 ${
                                    isManage ? "bg-gold/15 text-gold hover:bg-gold/25" : "bg-teal/10 text-teal hover:bg-teal/20"
