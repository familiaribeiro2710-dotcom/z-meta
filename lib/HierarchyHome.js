...
export default function HierarchyHome({ role, impersonate, viewerProfile, onExitImpersonation }) {
  ...
  const loadHero = useCallback(async (lojaId, monthArg) => {
    ...
    const { data: goalRows } = await supabase
      .from("sales_goals")
      .select("id, name, store_total, commission_pct_gerente, non_monetary_prize")
      .eq("loja_id", lojaId)
      .eq("month", monthArg)
              ...
              <>
                              ...
                              <li key={g.id} className="py-2.5">
                                ...
                                {activeGoal?.id === g.id && g.non_monetary_prize && (
                                  <p className="text-[11px] font-bold text-navy mt-1.5 flex items-center gap-1.5">
                                    <PartyPopper size={12} className="shrink-0" /> Prêmio da equipe: {g.non_monetary_prize}
                                  </p>
                                )}
                              </li>
                            );
                ...
                {tab === "atividades" && isConsorcio && (atSub === "funil" || atSub === "placar") && heroConsorcio.metaLoja > 0 && (
                  <div className="card animate-pop border-orange/20">
                    ...
                    {heroConsorcio.nonMonetaryPrize && (
                      <p className="text-xs font-bold text-navy bg-pink/10 rounded-lg py-2 px-3 mt-2 flex items-center gap-1.5">
                        <PartyPopper size={13} className="shrink-0" /> Prêmio da equipe: {heroConsorcio.nonMonetaryPrize}
                      </p>
                    )}
                  </div>
                )}
