...
import { RefreshCw, CalendarPlus, XCircle, Clock, Handshake, CheckCircle2, RotateCcw, PhoneOff, History, MessageSquarePlus } from "lucide-react";
...
import { useLeadListas, ListaFilterSelect, matchesListaFilter, ListaChip } from "./leadListas";
...
//
// 2026-09-05 (pedido do Felipe): "Sem contato" também virou destino de arraste manual — mas só
// a partir de "Novo" (mesma régua do botão "Não atendido" em ConsorcioDashboard.js/LeadsTab, que
// só aparece quando `l.status === "novo"`); de agendado/follow-up/em negociação em diante, "sem
// contato" continua não fazendo sentido (já houve contato/movimentação), então nem aparece como
// alvo de soltar o card ali. `openResolve(lead, "sem_contato")` grava o status final direto, sem
// passar pela pergunta "conseguiu contato" do modal de "Perdido" — não precisa perguntar de novo o
// que o próprio ato de soltar em "Sem contato" já responde.
//
...
function baseColumns() {
  ...
  // "Mostrar perdidos" (removido) — agora são colunas fixas, sempre visíveis (são desfechos
  // terminais negativos, não etapas de progresso do funil).
  // 2026-09-05 (pedido do Felipe): "Sem contato" passou por 2 posições no mesmo dia. Primeiro foi
  // pra logo depois de "Follow-up" (perto de onde ela de fato acontece no fluxo — só é alcançável
  // a partir de "Novo", ver `validDropTargets`). Ao revisar com o Felipe (ele perguntou "a ordem
  // está certa?"), ficou claro que isso quebrava a leitura de esquerda→direita = progresso: "Sem
  // contato" é um desfecho terminal negativo igual "Perdido" (nenhum dos dois avança pra mais
  // nada além de voltar pra "Agendado"), mas ficava emparedado no meio da sequência ativa
  // (Follow-up → Em negociação → Vendido). Mostrei 3 opções num mockup e o Felipe escolheu
  // agrupar os dois desfechos negativos no fim — sequência ativa contígua (Novo → Agendado →
  // Follow-up → Em negociação → Aguardando confirmação → Vendido), com os 2 terminais negativos
  // (Sem contato, Perdido) juntos por último.
  return [
    { key: "novo", label: "Novo" },
...
function validDropTargets(lead) {
  ...
  const targets = ["novo", "agendado", "follow_up", "em_negociacao", "vendido_pendente", "perdido"];
  // "Sem contato" só faz sentido vindo de "Novo" — mesma régua do botão "Não atendido" em
  // ConsorcioDashboard.js/LeadsTab, que só aparece quando `l.status === "novo"`.
  if (lead.status === "novo") targets.push("sem_contato");
  return targets;
}
...
export default function Pipeline({ employeeIds, lojaId, employees = [], empresaId, canManage = false, showResponsavel = true }) {
  ...
  // 2026-09-24 (pedido do Felipe): filtro por lista de origem do lead (ver lib/leadListas.js).
  const [filterListaId, setFilterListaId] = useState("");
  const { listas } = useLeadListas(empresaId);
  const listaNameById = {};
  listas.forEach((l) => { listaNameById[l.id] = l.nome; });
  const [refreshing, setRefreshing] = useState(false);
  const [justRefreshed, setJustRefreshed] = useState(false);
  const refreshFlashRef = useRef(null);
  const empScopeKey = (employeeIds || []).join(",");
  // 2026-09-04 (pedido do Felipe): ícone de histórico/observações em cada card — mesmo painel
  // compartilhado da aba Leads (ver LeadHistoryPanel.js).
  ...
  // 2026-09-05 (pedido do Felipe): observação em massa por etapa — em vez de abrir lead por lead
  // no histórico (LeadHistoryPanel.js) pra escrever a mesma coisa em cada um, um ícone no
  // cabeçalho da coluna deixa escrever uma vez só e aplicar em todos os leads QUE ESTÃO VISÍVEIS
  // naquela coluna no momento (respeita o filtro de colaborador já existente no topo do quadro —
  // decisão explícita: se o filtro está escondendo leads de outro colaborador, a observação não
  // deve vazar pra eles). `bulkNoteLeads` guarda a lista (id/empresa_id/loja_id) capturada na hora
  // de ABRIR o modal — se o quadro recarregar sozinho (auto-refresh) enquanto o modal ainda está
  // aberto, a observação continua valendo pro grupo que o usuário efetivamente viu e confirmou,
  // não por um grupo diferente que possa ter mudado por baixo. Grava em `crm_lead_events` com
  // `event_type: 'nota'` — mesma tabela/evento do "Adicionar observação" de um lead só
  // (LeadHistoryPanel.js), só que um insert em lote em vez de um por vez.
  const [bulkNoteCol, setBulkNoteCol] = useState(null); // { key, label }
  const [bulkNoteLeads, setBulkNoteLeads] = useState([]);
  const [bulkNoteText, setBulkNoteText] = useState("");
  const [bulkNoteSaving, setBulkNoteSaving] = useState(false);
  const [bulkNoteError, setBulkNoteError] = useState("");

  function openBulkNote(col, colLeads) {
    setBulkNoteCol(col);
    setBulkNoteLeads(colLeads);
    setBulkNoteText("");
    setBulkNoteError("");
  }

  async function confirmBulkNote() {
    if (!bulkNoteCol || !bulkNoteText.trim() || !bulkNoteLeads.length) return;
    setBulkNoteSaving(true);
    setBulkNoteError("");
    const { data: { session } } = await supabase.auth.getSession();
    if (!session) {
      setBulkNoteSaving(false);
      setBulkNoteError("Sessão expirada — recarregue a página e tente de novo.");
      return;
    }
    const rows = bulkNoteLeads.map((lead) => ({
      lead_id: lead.id,
      empresa_id: lead.empresa_id,
      loja_id: lead.loja_id,
      event_type: "nota",
      observacoes: bulkNoteText.trim(),
      actor_id: session.user.id,
    }));
    const { error } = await supabase.from("crm_lead_events").insert(rows);
    setBulkNoteSaving(false);
    if (!error) {
      notifySaved(`Observação adicionada em ${rows.length} lead(s).`);
      setBulkNoteCol(null);
      setBulkNoteLeads([]);
      setBulkNoteText("");
    } else {
      setBulkNoteError("Não foi possível salvar. Tente novamente em alguns instantes.");
    }
  }
  ...
  // 2026-09-04 (pedido do Felipe): no iPad, tocar num card já armava o arrasto na hora (touchstart),
  // e o card ficava com touch-action:none o tempo todo enquanto arrastável — isso capturava
  // qualquer deslizar de dedo sobre um card como tentativa de arrastar, travando a rolagem normal
  // da coluna. Fix: press-and-hold — só arma o arrasto depois de ~1s parado no mesmo lugar; se a
  // pessoa soltar ou mover o dedo antes disso, cancela e deixa o navegador rolar normalmente (a
  // classe touch-none só entra DEPOIS que o arrasto arma de verdade, ver isBeingDragged no card).
  ...
  function handleTouchMove(e) {
    // Ainda dentro do 1s de espera: se moveu o dedo além do limiar, é rolagem — cancela o
    // arrasto pendente e não mexe em mais nada (sem preventDefault em nenhum momento, o
    // navegador continua livre pra rolar a coluna normalmente).
    if (longPressTimerRef.current) {
      const t = e.touches[0];
  ...
  const visibleLeads = leads.filter((l) => (!filterEmployeeId || l.employee_id === filterEmployeeId) && matchesListaFilter(l, filterListaId));
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between gap-2 flex-wrap">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <ListaFilterSelect listas={listas} value={filterListaId} onChange={setFilterListaId} selectClassName="!text-[11px]" />
        </div>
        <button
          type="button"
          ...
          return (
            <div key={col.key} className="min-w-[220px] flex-1">
              <div className="flex items-center gap-2 mb-2 px-2 py-1.5 rounded-lg" style={{ background: `${color}1f` }}>
                ...
                <button
                  type="button"
                  title={cards.length ? `Adicionar observação em todos os ${cards.length} lead(s) de "${col.label}"` : "Nenhum lead nessa etapa"}
                  aria-label="Adicionar observação em massa nessa etapa"
                  disabled={!cards.length}
                  onClick={() => openBulkNote(col, cards)}
                  className="shrink-0 p-1 -m-1 rounded-lg text-muted hover:text-navy hover:bg-surface/60 transition-colors disabled:opacity-30 disabled:hover:bg-transparent disabled:cursor-not-allowed"
                >
                ...
                {cards.map((lead) => {
                  ...
                  return (
                    <div
                      ...
                      className={`card-dark !p-2.5 !rounded-xl transition-shadow ${draggable ? "cursor-grab active:cursor-grabbing" : ""} ${isBeingDragged ? "touch-none opacity-90 shadow-xl scale-[1.03]" : ""}`}
                      style={{
                    ...
                    >
                      <div className="flex items-center gap-2">
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          {lead.lista_id && <ListaChip nome={listaNameById[lead.lista_id]} />}
                        </div>
                        <button
                          type="button"
                          title="Histórico e observações"
                          aria-label="Histórico e observações"
                          onClick={(e) => { e.stopPropagation(); setHistoryLead(lead); }}
                          onTouchStart={(e) => e.stopPropagation()}
                          onMouseDown={(e) => e.stopPropagation()}
                          className="shrink-0 p-1 -m-1 rounded-lg text-white/50 hover:text-white hover:bg-surface/10 transition-colors"
                        >
                          <History size={14} />
                        </button>
                      </div>
      ...
      {bulkNoteCol && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-navyfixed/70 p-6">
          <div className="card max-w-sm w-full animate-bounce-in">
            <h2 className="text-lg font-extrabold text-navy flex items-center gap-2">
              <MessageSquarePlus className="text-blue" size={20} /> Observação em massa
            </h2>
            <p className="text-xs text-muted mt-1">
              Vai ser adicionada no histórico de {bulkNoteLeads.length} lead(s) da etapa &ldquo;{bulkNoteCol.label}&rdquo;{filterEmployeeId || filterListaId ? " (respeitando os filtros atuais)" : ""}.
            </p>
            <div className="mt-4">
              ...
              <textarea
                ...
                placeholder="Ex: todos aguardando retorno da campanha de outubro…"
                autoFocus
              />
            </div>
            {bulkNoteError && <p className="text-xs font-bold text-danger mt-3">{bulkNoteError}</p>}
            <div className="flex gap-2 mt-5">
              <button className="btn-outline flex-1" onClick={() => setBulkNoteCol(null)} disabled={bulkNoteSaving}>Cancelar</button>
              <button className="btn flex-1" disabled={bulkNoteSaving || !bulkNoteText.trim()} onClick={confirmBulkNote}>
                {bulkNoteSaving ? "Salvando…" : `Adicionar em ${bulkNoteLeads.length}`}
              </button>
            </div>
          </div>
        </div>
      ...
      <LeadHistoryPanel lead={historyLead} onClose={() => setHistoryLead(null)} canManage={canManage} produtoCategorias={produtoCategorias} onChanged={loadLeads} />
    </div>
  );
}
