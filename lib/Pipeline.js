...
import { RefreshCw, CalendarPlus, XCircle, Clock, Handshake, CheckCircle2, Eye, EyeOff, RotateCcw, PhoneOff } from "lucide-react";
...
export default function Pipeline({ employeeIds, lojaId, employees = [], empresaId, canManage = false, showResponsavel = true }) {
  ...
  function openResolve(lead, type) {
    // "novo" -> "perdido" vira "sem_contato" automaticamente (ver comentário no topo do arquivo) —
    // decidido aqui, no ponto de entrada único de todo mundo que chama openResolve (drag e o que
    // mais existir), pra já abrir o modal com o título/ícone certos desde o primeiro instante.
  ...
  if (showPerdido) columns.splice(4, 0, { key: "perdido", label: "Perdido" }, { key: "sem_contato", label: "Sem contato" });
      ...
      <div className="flex items-center justify-between gap-2 flex-wrap">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          >
            ...
            {showPerdido ? "Ocultar perdidos" : "Mostrar perdidos"}
          </button>
        ...
        <button
          ...
          className={`inline-flex items-center justify-center w-7 h-7 rounded-full border transition-all duration-200 disabled:opacity-70 shrink-0 hover:scale-110 active:scale-90 ${
            justRefreshed ? "border-success/40 bg-success/10 text-success" : "border-line bg-white text-muted hover:text-navy hover:border-purple/50 hover:bg-paper"
          }`}
        >
