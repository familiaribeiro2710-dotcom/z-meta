...
import { RefreshCw, CalendarPlus, XCircle, Clock, Handshake, CheckCircle2, Eye, EyeOff, Move } from "lucide-react";
...
// arrastar com mouse, trackpad ou Apple Pencil — dedo não funciona. Por isso todo card arrastável
// também tem `onClick` abrindo um seletor de destino (`moveModal`, mesma decisão de "pra onde
// mover" que soltar o card faria) — funciona por toque em qualquer iPad, com ou sem trackpad. As
// duas vias (arrastar e tocar) convergem pra mesma função `moveLeadTo`, que abre o mesmo modal de
// confirmação de sempre (data/hora, feedback, valor) — nada muda de status sem esse passo.
//
...
export default function Pipeline({ employeeIds, lojaId, employees = [], empresaId, canManage = false, showResponsavel = true }) {
  ...
  // ---- drag and drop nativo (desktop/tablet com mouse-trackpad-Pencil, sem lib nova) ----
  const [draggingId, setDraggingId] = useState(null);
  const [dragOverCol, setDragOverCol] = useState(null);
  const draggingLead = leads.find((l) => l.id === draggingId) || null;

  // ---- seletor de destino por toque (iPad sem trackpad — ver comentário no topo do arquivo) ----
  const [moveModal, setMoveModal] = useState(null); // lead sendo movido
                ...
                {cards.map((lead) => {
                  ...
                  return (
                    <div
                      ...
                      onClick={() => { if (draggable) setMoveModal(lead); }}
                    ...
                    >
                      <div className="flex items-center gap-2">
                        ...
                        {draggable && <Move size={12} className="text-white/40 shrink-0" />}
                      </div>
      ...
      {moveModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-navy/70 p-6">
          <div className="card max-w-sm w-full animate-bounce-in border-purple/30">
            <h2 className="text-lg font-extrabold text-navy flex items-center gap-2"><Move className="text-purple" size={18} /> Mover para</h2>
            <p className="text-xs text-muted mt-1">{moveModal.nome_completo}</p>
            <div className="mt-4 space-y-2">
              {validDropTargets(moveModal).map((colKey) => (
                <button
                  key={colKey}
                  type="button"
                  onClick={() => { const lead = moveModal; setMoveModal(null); moveLeadTo(lead, colKey); }}
                  className="w-full flex items-center gap-2.5 text-left border border-line rounded-xl px-3 py-2.5 hover:border-purple/50 hover:bg-paper transition-colors"
                >
                  <span className="w-2 h-2 rounded-full shrink-0" style={{ background: COLUMN_COLOR[colKey] || "#7d7a6f" }} />
                  <span className="text-xs font-bold text-navy">{STATUS_LABEL[colKey]}</span>
                </button>
              ))}
            </div>
            <button type="button" className="btn-outline w-full mt-4" onClick={() => setMoveModal(null)}>Cancelar</button>
          </div>
        </div>
      )}
