...
export default function ChangePassword({ force = false, onDone }) {
  ...
  async function handleSave(e) {
    ...
    setOpen(false);
    notifySaved("Senha alterada com sucesso.");
  }
    ...
    return (
      <main className="min-h-screen flex items-center justify-center p-6">
        <div className="card max-w-sm w-full animate-pop">
          <p className="label mb-1 flex items-center gap-1.5"><Lock size={14} /> Defina sua nova senha</p>
  ...
  return (
    <div className="mt-8 border-t border-line pt-4">
      <button
        onClick={() => setOpen((v) => !v)}
        className="flex items-center gap-1.5 text-xs uppercase tracking-wider text-muted hover:text-purple transition-colors"
      >
        <KeyRound size={13} /> {open ? "Fechar" : "Alterar senha"}
      </button>
      {open && (
