...
export default function ColaboradorView({ profile, tab, viewedByManager = false, onBack }) {
  ...
  async function toggleTask(taskId) {
    // Ativação Online nunca pode ser marcada manualmente pelo colaborador — só o gerente (via
    // Tarefas do painel dele) ou o próprio sistema quando a contagem de contatos bate a meta
    // (check_online_activation_completion, no banco). Mesmo padrão de defesa em profundidade
    // usado em lib/ColaboradorViewConsorcio.js pro task_type "contatos".
  ...
  async function showYesterdayContacts() {
    ...
    const { data } = await supabase
      ...
      .eq("contact_date", yStr)
      .order("created_at", { ascending: false });
    setYesterdayContacts(data || []);
    setActDashDate(yStr);
  }

  function showTodayContacts() {
    setActDashDate(todayStr());
  }

  const actDashContacts = actDashDate === todayStr() ? todayContacts : yesterdayContacts;
  ...
  return (
    <div className="space-y-6">
      ...
      {!showCongrats && goalCelebrations.length === 0 && onlineCelebrations.length === 0 && closingWinners.length > 0 && (
              ...
              <div className="mt-5 pt-4 border-t border-line">
                <div className="flex items-center justify-between gap-2 flex-wrap mb-3">
                  ...
                  <div className="flex gap-1.5">
                    <button
                      type="button"
                      onClick={showTodayContacts}
                      className={`text-xs font-bold px-3 py-1 rounded-full border-2 transition-all ${
                        actDashDate === todayStr() ? "border-purple text-purple" : "border-line text-muted hover:border-purple/50"
                      }`}
                    >
                      Hoje
                    </button>
                ...
                {actDashContacts.length === 0 ? (
                  <p className="text-sm text-muted">Nenhum contato lançado {actDashDate === todayStr() ? "hoje" : "ontem"}.</p>
                ) : (
                  <ul className="divide-y divide-line">
