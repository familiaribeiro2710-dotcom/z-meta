            ...
            <div className="card-dark animate-pop">
              ...
              <ul>
                {storeRanking.map((r, idx) => {
                  ...
                  return (
                    <li key={r.id} className={`row-card ${isMe ? "bg-goldlight/10 -mx-2 px-2 rounded-xl" : ""}`}>
                      ...
                      <span className={`font-bold text-xs sm:text-sm shrink-0 whitespace-nowrap ${isMe ? "text-goldlight" : "text-white"}`}>{formatBRL(r.sold)}</span>
                    </li>
                  );
                })}
              </ul>
