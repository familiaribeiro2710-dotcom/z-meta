...
import { todayStr, firstDayOfMonth, remainingDaysInMonth, monthLabel, greeting } from "./date";
        ...
        <div className="space-y-6">
          ...
          >
            ...
            <div className="relative grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-x-3 gap-y-5 sm:gap-4 mt-6 pt-5 border-t border-navy/15">
              ...
              <div className="min-w-0">
                <p className="text-lg sm:text-xl font-extrabold text-navy">{pendingCount}</p>
              ...
              <div className="min-w-0">
                <p className="text-lg sm:text-xl font-extrabold text-navy break-words">{formatBRL(commissionSoFar)}</p>
              ...
              <div className="min-w-0">
                <p className="text-lg sm:text-xl font-extrabold text-navy break-words">{formatBRL(prizesSoFar)}</p>
              ...
              <div className="min-w-0">
                <p className="text-lg sm:text-xl font-extrabold text-navy break-words">
                  {myRank ? `${myRank}º` : "—"}
                  {storeRanking.length > 0 && <span className="text-xs font-semibold text-navy/60"> /{storeRanking.length}</span>}
                </p>
                    ...
                    <li key={goal.id} className="py-2.5">
                      <div className="flex items-center justify-between gap-2 text-sm">
                        <span className="font-medium text-navy flex items-center gap-1.5">
                          {goal.name}
                          {activeGoalItem?.goal.id === goal.id && <span className="badge bg-purple/15 text-purple">em jogo</span>}
                        </span>
          ...
          {storeRanking.length > 0 && (
            <div className="card animate-pop border-orange/20">
              <p className="label mb-1 flex items-center gap-1.5"><Trophy size={14} /> Ranking de vendas — {monthLabel(month)}</p>
              <p className="text-[11px] text-muted mb-3">Como você está em relação à sua equipe na loja este mês.</p>
              <ul className="divide-y divide-line">
                {storeRanking.map((r, idx) => {
                  ...
                  return (
                    <li
                      key={r.id}
                      className={`flex items-center justify-between gap-2 py-2 text-sm ${isMe ? "bg-purple/5 -mx-2 px-2 rounded-xl" : ""}`}
                    >
                      ...
                      <span className={`font-semibold shrink-0 ${isMe ? "text-purple" : "text-navy"}`}>{formatBRL(r.sold)}</span>
                    </li>
                  );
                })}
              </ul>
                  ...
                  <div key={goal.id} className={`card animate-pop ${borders[i % borders.length]}`}>
                    ...
                    <div className="mt-3 pt-3 border-t border-line grid grid-cols-2 gap-2 sm:gap-3">
                      <div className="min-w-0">
                        ...
                        <p className="text-lg sm:text-xl font-extrabold gradient-text mt-0.5 break-words">{formatBRL(goalDailyGoal)}</p>
                      </div>
                      <div className="min-w-0">
                        <p className="text-[11px] text-muted uppercase tracking-wider font-bold">Falta pra bater</p>
                        <p className="text-lg sm:text-xl font-extrabold text-navy mt-0.5 break-words">{formatBRL(rest)}</p>
                      </div>
                    </div>
                  </div>
                );
            ...
            <div className="card">
              ...
              <form onSubmit={saveEntry} className="flex flex-col sm:flex-row gap-3 items-end">
                <div className="flex-1 w-full">
                  ...
                  <div className="input">
                    <DateNav date={entryDate} onChange={setEntryDate} maxDate={today} />
                  </div>
                </div>
                <div className="flex-1 w-full">
