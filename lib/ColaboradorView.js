...
export default function ColaboradorView({ profile, tab, viewedByManager = false, onBack }) {
  ...
  // (pedido do Felipe: não só o gerente), com a mesma flag/key usada em GerenteView.js
  // (zmeta_storegoalhit_${pessoa}_${mes}_${meta}) — como a chave é por profile.id, não colide
  // entre pessoas diferentes vendo a mesma meta batida.
  useEffect(() => {
    if (viewedByManager) return;
    if (!isCurrentMonth) return;
    if (!storeGoalsList.length) return;
    const newlyAchieved = [];
    storeGoalsList.forEach((g) => {
      if (storeSoldLoja >= Number(g.store_total || 0)) {
        const flagKey = `zmeta_storegoalhit_${profile.id}_${month}_${g.id}`;
        if (typeof window !== "undefined" && !window.localStorage.getItem(flagKey)) {
          window.localStorage.setItem(flagKey, "1");
    ...
    if (newlyAchieved.length) {
      setStoreGoalCelebrations((q) => [...q, ...newlyAchieved]);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
