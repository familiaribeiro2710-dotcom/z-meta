...
import { todayStr, firstDayOfMonth, remainingDaysInMonth, monthLabel, yesterdayStr, greeting } from "./date";
    ...
    <div className="space-y-6">
      ...
      {tab === "atividades" && (
        <div className="space-y-6">
          ...
          {activeGoalItem && (
            <div className="card animate-pop border-purple/20">
              <p className="label flex items-center gap-1.5"><Target size={14} /> Barra da meta em jogo — {activeGoalItem.goal.name}</p>
              <ProgressBar pct={Number(activeGoalItem.allocation.amount) > 0 ? Math.min(100, (soldSoFar / Number(activeGoalItem.allocation.amount)) * 100) : 0} />
              <p className="text-xs text-muted mt-2">{formatBRL(soldSoFar)} vendido de {formatBRL(activeGoalItem.allocation.amount)} de meta individual.</p>
            </div>
          )}
