...
export default function ColaboradorView({ profile, tab, viewedByManager = false, onBack }) {
  ...
  // meta que carrega o prêmio (pedido do Felipe, achado num caso real na ArmyBR).
  const extraPrizeGoal = storeGoalsList.find((g) => g.non_monetary_prize && storeSoldLoja < Number(g.store_total)) || storeGoalsList.find((g) => g.non_monetary_prize) || null;
