...
import { todayStr, firstDayOfMonth, remainingPeriodsInMonth, PACING_HERO_LABEL, PACING_UNIT, monthLabel, greeting, isTaskDueOn, isPrizePeriodActive, prizeWindowLabel } from "./date";
...
export default function ColaboradorView({ profile, tab, viewedByManager = false, onBack }) {
  ...
  const [prizeCelebrations, setPrizeCelebrations] = useState([]); // premiação em dinheiro lançada pelo gestor (employee_prizes) — fila de linhas ainda não vistas (seen_at is null)

  const [selectedMonth, setSelectedMonth] = useState(firstDayOfMonth(todayStr()));
  const didInit = useRef(false);

  const [goals, setGoals] = useState([]);
  const [storeGoalsList, setStoreGoalsList] = useState([]); // metas raw da loja, pra barra "geral"
  const [storeGoalCelebrations, setStoreGoalCelebrations] = useState([]); // "a loja bateu a meta" — fila de níveis, mesmo padrão de GerenteView.js/ColaboradorViewConsorcio.js
  ...
  const loadAll = useCallback(async (uid, lojaId, monthArg) => {
    ...
    setGoals(combined);
    // Metas RAW da loja (todas, não só as que têm alocação pra este colaborador) — usadas pra
    // calcular a barra "geral da loja" (mesmo padrão de ColaboradorViewConsorcio.js/storeSoldLoja),
    // já que a meta geral compara o total vendido pela loja inteira, não a alocação individual.
    setStoreGoalsList(goalRows || []);
  ...
  // Premiação em dinheiro lançada pelo gestor: além do push (já existia), o colaborador vê um
  // modal de "você ganhou uma premiação de R$X" na próxima vez que abrir o app — qualquer linha de
  // employee_prizes ainda não vista (seen_at is null), de qualquer mês, não só o selecionado.
  // Nunca roda em "ver como" (gerente vendo a tela do colaborador não deve marcar como visto).
  useEffect(() => {
  ...
  async function saveEntry(e) {
    ...
    // como concluídas automaticamente (não faz sentido cobrar tarefas de um dia de folga). 2026-08-18
    // (pedido do Felipe): pergunta também quando a data é hoje — antes só perguntava pra dias
    // passados, e R$0 hoje salvava direto sem avisar.
  ...
  async function doSaveEntry(isFolga) {
    ...
    if (!error && isFolga) {
      ...
      // — venda R$0 confirmada aqui (2026-08-18: inclusive no dia de hoje). Por isso ela entra
      // igual às outras tarefas na lista de tarefas marcadas automaticamente quando confirma que
      // foi folga.
  ...
  const willRelease = individualPct >= Number(settings.team_threshold_pct);
  ...
  // Ritmo da meta ativa (dia/semana/quinzena/estágio, 2026-08-05) — generaliza o "falta pra meta de
  // hoje" pra qualquer régua escolhida na meta: remaining vira quantos períodos daquela régua ainda
  // restam (não só dias), e o que faltou num período já batido soma automaticamente no próximo
  // (remainingPeriodsInMonth cuida disso sozinho, olhando só quanto falta de hoje em diante).
  const activePacing = activeGoalItem?.goal?.pacing || "dia";
  const remaining = isCurrentMonth ? remainingPeriodsInMonth(month, today, activePacing) : 0;
  const dailyGoal = remaining > 0 ? restoDaMeta / remaining : 0;

  // Meta geral da loja (todos os colaboradores, não só a alocação individual) — storeRanking já
  // vem de get_store_sales_ranking com TODOS os colaboradores ativos da loja, então somar dá o
  // total da loja no mês, mesma base de sales_goals.store_total (mesmo padrão de
  // ColaboradorViewConsorcio.js/storeSoldLoja, mas ali via query direta em crm_leads).
  const storeSoldLoja = storeRanking.reduce((s, r) => s + Number(r.sold || 0), 0);
  const metaLojaGeral = currentGoalTarget(storeGoalsList.map((g) => g.store_total), storeSoldLoja);
  // Meta geral em jogo (o item, não só o número) — o prêmio não-monetário é da loja inteira, então
  // tem que vir da camada da meta GERAL, não da individual (mesmo achado do Felipe já corrigido em
  // ColaboradorViewConsorcio.js: as duas podem divergir).
  const activeGoalGeralItem = storeGoalsList.find((g) => storeSoldLoja < Number(g.store_total)) || (storeGoalsList.length ? storeGoalsList[storeGoalsList.length - 1] : null);
  // Prêmio extra: fixo o mês inteiro assim que QUALQUER meta da loja tiver premiação extra
  // cadastrada, não só a que está "em jogo" — senão o aviso some assim que a loja ultrapassa a
  // meta que carrega o prêmio (pedido do Felipe, achado num caso real na ArmyBR). Com 2+ metas
  // premiadas: mostra a de menor valor ainda não batida (mais próxima); se TODAS já foram batidas,
  // cai pra de MAIOR valor entre elas (a conquista mais recente/relevante), não a primeira da lista.
  const prizeGoalsAll = storeGoalsList.filter((g) => g.non_monetary_prize && isPrizePeriodActive(g));
  ...
  // (marcado em goal_celebration_seen, no banco — pra não repetir em NENHUM aparelho, não só o
  // que já viu antes). Só roda pra quem está de fato logado como o colaborador (não quando o
  // gerente está "vendo como") e só pro mês corrente — bater uma meta de um mês fechado não deve
  // gerar pop-up.
  useEffect(() => {
    if (viewedByManager) return;
    ...
    let active = true;
    (async () => {
      const achieved = goals.filter(({ allocation }) => soldSoFar >= Number(allocation.amount || 0)).map(({ goal }) => goal);
      if (!achieved.length) return;
      const newly = await filterNewlyAchievedGoals({ profileId: profile.id, month, kind: "individual", achievedGoals: achieved });
      if (active && newly.length) setGoalCelebrations((q) => [...q, ...newly]);
    })();
    return () => { active = false; };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [goals, soldSoFar, month, isCurrentMonth, viewedByManager]);

  // Modal de "a loja bateu a meta" — mesmo padrão acima, só que pela camada GERAL da loja
  // (storeSoldLoja vs storeGoalsList), não a alocação individual. Todo colaborador da loja vê
  // (pedido do Felipe: não só o gerente); flag em goal_celebration_seen é por profile_id, então
  // não colide entre pessoas diferentes vendo a mesma meta batida.
  useEffect(() => {
    if (viewedByManager) return;
    if (!isCurrentMonth) return;
    if (!storeGoalsList.length) return;
    let active = true;
    (async () => {
      const achieved = storeGoalsList.filter((g) => storeSoldLoja >= Number(g.store_total || 0));
      if (!achieved.length) return;
      const newly = await filterNewlyAchievedGoals({ profileId: profile.id, month, kind: "loja", achievedGoals: achieved });
      if (active && newly.length) setStoreGoalCelebrations((q) => [...q, ...newly]);
    })();
    return () => { active = false; };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [storeGoalsList, storeSoldLoja, month, isCurrentMonth, viewedByManager]);
  ...
  // kind própria em goal_celebration_seen (não reaproveita a mesma linha, são conquistas diferentes).
  useEffect(() => {
    if (viewedByManager) return;
    if (!isCurrentMonth) return;
    ...
    let active = true;
    (async () => {
      const achieved = onlineGoals.filter((g) => onlineSoldSoFar >= Number(g.target_amount || 0));
      if (!achieved.length) return;
      const newly = await filterNewlyAchievedGoals({ profileId: profile.id, month, kind: "online", achievedGoals: achieved });
      if (active && newly.length) setOnlineCelebrations((q) => [...q, ...newly]);
    })();
    return () => { active = false; };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  return (
    <div className="space-y-6">
      ...
      {prizeCelebrations.length === 0 && !showCongrats && goalCelebrations.length > 0 && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-navyfixed/70 p-6">
          <Confetti />
          <div className="card max-w-sm w-full text-center animate-bounce-in border-purple/30">
            <div className="flex justify-center mb-3 animate-wiggle">
            ...
            <h2 className="text-xl font-extrabold gradient-text">Parabéns, {profile.full_name.split(" ")[0]}!</h2>
            <p className="text-sm text-muted mt-2 flex items-center justify-center gap-1.5 flex-wrap">
              Você bateu a <span className="font-bold text-navy">{goalCelebrations[0].name}</span> de {monthLabel(month)}! Continue faturando! <PartyPopper size={15} className="text-orange" />
            </p>
      ...
      {prizeCelebrations.length === 0 && !showCongrats && goalCelebrations.length === 0 && onlineCelebrations.length > 0 && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-navyfixed/70 p-6">
          <Confetti />
          <div className="card max-w-sm w-full text-center animate-bounce-in border-purple/30">
            <div className="flex justify-center mb-3 animate-wiggle">
            ...
            <h2 className="text-xl font-extrabold gradient-text">Parabéns, {profile.full_name.split(" ")[0]}!</h2>
            <p className="text-sm text-muted mt-2 flex items-center justify-center gap-1.5 flex-wrap">
              Você bateu a <span className="font-bold text-navy">{onlineCelebrations[0].name}</span> Online de {monthLabel(month)} — prêmio de{" "}
      ...
      {prizeCelebrations.length === 0 && !showCongrats && goalCelebrations.length === 0 && onlineCelebrations.length === 0 && ativacaoCelebration && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-navyfixed/70 p-6">
          <Confetti />
          <div className="card max-w-sm w-full text-center animate-bounce-in border-purple/30">
            <div className="flex justify-center mb-3 animate-wiggle">
      ...
      {prizeCelebrations.length === 0 && !showCongrats && goalCelebrations.length === 0 && onlineCelebrations.length === 0 && !ativacaoCelebration && storeGoalCelebrations.length > 0 && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-navyfixed/70 p-6">
          <Confetti />
          <div className="card max-w-sm w-full text-center animate-bounce-in border-purple/30">
            <div className="flex justify-center mb-3 animate-wiggle">
            ...
            <h2 className="text-xl font-extrabold gradient-text">Parabéns, {profile.full_name.split(" ")[0]}!</h2>
            <p className="text-sm text-muted mt-2 flex items-center justify-center gap-1.5 flex-wrap">
              A loja bateu a <span className="font-bold text-navy">{storeGoalCelebrations[0].name}</span> de {monthLabel(month)}! <PartyPopper size={15} className="text-orange" />
            </p>
      ...
      {prizeCelebrations.length === 0 && !showCongrats && goalCelebrations.length === 0 && onlineCelebrations.length === 0 && !ativacaoCelebration && storeGoalCelebrations.length === 0 && closingWinners.length > 0 && (
      ...
      <ConfirmModal
        ...
        danger
        onConfirm={async () => { await deleteEntry(confirmDeleteEntry.id); setConfirmDeleteEntry(null); }}
        ...
        <div className="space-y-6">
          ...
          >
            ...
            <div className="relative flex items-center gap-2 mb-3">
              ...
              <span className="text-xs font-bold uppercase tracking-wider text-[#a78bfa]">{PACING_HERO_LABEL[activePacing]}</span>
            </div>
            <AutoFitText className="relative text-4xl sm:text-5xl font-extrabold text-white leading-tight">{formatBRL(dailyGoal)}</AutoFitText>
            <p className="relative text-xs font-semibold text-white/60 mt-1">
              {isCurrentMonth ? `pra bater a meta do mês em ${remaining} ${PACING_UNIT[activePacing]}${remaining !== 1 ? "s" : ""} restante${remaining !== 1 ? "s" : ""}` : `mês fechado — ${monthLabel(month)}`}
            </p>
            <p className="relative text-xs font-semibold text-white/60 mt-1">{isCurrentMonth ? "Vendido até hoje" : "Vendido no mês"}: {formatBRL(soldSoFar)}</p>

            <div className="relative grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-x-3 gap-y-5 sm:gap-4 mt-6 pt-5 border-t border-white/10">
              <div className="min-w-0">
                <AutoFitText className="text-lg sm:text-xl font-extrabold text-white">{formatBRL(restoDaMeta)}</AutoFitText>
                <p className="text-[11px] font-semibold text-white/60 mt-0.5 flex items-center gap-1"><Rocket size={11} className="shrink-0" /> Falta pra meta do mês</p>
              </div>
              <div className="min-w-0">
                <AutoFitText className="text-lg sm:text-xl font-extrabold text-white">{remaining}</AutoFitText>
                <p className="text-[11px] font-semibold text-white/60 mt-0.5 flex items-center gap-1"><CalendarClock size={11} className="shrink-0" /> Dias restantes no mês</p>
              </div>
              <div className="min-w-0">
                <AutoFitText className="text-lg sm:text-xl font-extrabold text-white">{pendingCount}</AutoFitText>
              ...
              <div className="min-w-0">
                <AutoFitText className="text-lg sm:text-xl font-extrabold text-white">
                  {myRank ? `${myRank}º` : "—"}
                  {storeRanking.length > 0 ? ` /${storeRanking.length}` : ""}
                </AutoFitText>
                <p className="text-[11px] font-semibold text-white/60 mt-0.5 flex items-center gap-1"><Medal size={11} className="shrink-0" /> Posição no ranking</p>
              </div>
            </div>
          </div>
          ...
          <div className="card animate-pop border-gold/20">
            ...
            <ProgressBar pct={individualPct} threshold={settings.team_threshold_pct} />
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <div className="card animate-pop border-teal/20">
              ...
              <p className="text-xs font-semibold text-navy mt-1">Meta: {formatBRL(metaDoMes)}</p>
              <ProgressBar pct={metaDoMes > 0 ? Math.min(100, (soldSoFar / metaDoMes) * 100) : 0} />
              <p className="text-xs text-muted mt-2">Vendido: {formatBRL(soldSoFar)}</p>
            </div>
                    ...
                    <li key={goal.id} className="py-2.5">
                      <div className="flex items-center justify-between gap-2 text-xs sm:text-sm">
                        <span className="font-medium text-navy flex items-center gap-1.5 min-w-0">
                          ...
                          {activeGoalItem?.goal.id === goal.id && <span className="badge bg-gold/15 text-gold shrink-0">em jogo</span>}
                        </span>
          ...
          <div className="card">
            ...
            {warnings.length === 0 ? (
              ...
              <ul className="space-y-2">
                {warnings.map((w) => (
                  <li key={w.id} className="text-sm flex items-start justify-between gap-2 border-b border-line pb-2">
                    <span className="flex-1 min-w-0 break-words">{w.reason}</span>
                    {/* 2026-08-29, pedido do Felipe: colaborador não vê mais o desconto em % de
                        cada advertência (só data) — o impacto real na nota/comissão continua
                        sendo calculado normalmente por trás (app_settings.warning_penalty_points),
                        só deixou de aparecer nessa lista. */}
                    <span className="text-muted text-xs shrink-0 whitespace-nowrap">{w.warning_date}</span>
                  </li>
                ))}
              </ul>
            )}
          ...
          {goals.length === 0 ? (
            ...
            <div className={`grid gap-4 ${goals.length > 1 ? "sm:grid-cols-2" : ""}`}>
              {goals.map(({ goal, allocation }, i) => {
                ...
                const goalPacing = goal.pacing || "dia";
                const goalRemaining = isCurrentMonth ? remainingPeriodsInMonth(month, today, goalPacing) : 0;
                const goalDailyGoal = goalRemaining > 0 ? rest / goalRemaining : 0;
                ...
                return (
                  <div key={goal.id} className={`card animate-pop ${borders[i % borders.length]}`}>
                    ...
                    <div className="mt-3 pt-3 border-t border-line grid grid-cols-2 gap-2 sm:gap-3">
                      <div className="min-w-0">
                        <p className="text-[11px] text-muted uppercase tracking-wider font-bold">{PACING_HERO_LABEL[goalPacing]}</p>
              ...
              <table className="w-full text-xs sm:text-sm">
                <thead>
                  <tr className="text-left text-xs uppercase tracking-wider text-white/50 border-b border-white/10">
                    ...
                    {isCurrentMonth && <th className="pb-2"></th>}
                  </tr>
                </thead>
                <tbody>
                    ...
                    <tr key={en.id} className="border-b border-white/10 last:border-0">
                      ...
                      {isCurrentMonth && (
                        <td className="py-2 text-right">
                          <button type="button" title="Excluir" aria-label="Excluir" className="p-1.5 rounded-lg text-white/50 hover:text-danger hover:bg-surface/10 transition-colors" onClick={() => setConfirmDeleteEntry(en)}>
                            <Trash2 size={13} />
                          </button>
                        </td>
                      )}
      ...
      {tab === "online" && (
        <div className="space-y-6">
          ...
          <div
            className="relative overflow-hidden rounded-3xl p-6 sm:p-7 reveal-up border-2"
            style={{ background: "linear-gradient(160deg, #1c2e4d 0%, #12203a 100%)", borderColor: "#f97316", boxShadow: "0 10px 26px rgba(18,32,58,0.35)" }}
          >
            <div className="absolute -top-14 -right-10 w-48 h-48 rounded-full bg-[#f97316]/15" />
            <div className="relative flex items-center gap-2 mb-3">
              <Globe size={18} className="text-[#f97316]" />
              <span className="text-xs font-bold uppercase tracking-wider text-[#f97316]">Online · {monthLabel(month)}</span>
            </div>
            <AutoFitText className="relative text-4xl sm:text-5xl font-extrabold text-white leading-tight">{formatBRL(onlineSoldSoFar)}</AutoFitText>
            <p className="relative text-xs font-semibold text-white/60 mt-1">
              {onlineTierInPlay
            ...
            <div className="relative grid grid-cols-2 sm:grid-cols-4 gap-x-3 gap-y-5 sm:gap-4 mt-6 pt-5 border-t border-white/10">
              <div className="min-w-0">
                <AutoFitText className="text-lg sm:text-xl font-extrabold text-white">{onlineSales.length}</AutoFitText>
                <p className="text-[11px] font-semibold text-white/60 mt-0.5 flex items-center gap-1"><Receipt size={11} className="shrink-0" /> Vendas lançadas</p>
              </div>
              <div className="min-w-0">
                <AutoFitText className="text-lg sm:text-xl font-extrabold text-white">{formatBRL(onlinePrizeInPlay)}</AutoFitText>
                <p className="text-[11px] font-semibold text-white/60 mt-0.5 flex items-center gap-1"><Gift size={11} className="shrink-0" /> Prêmio em jogo</p>
              </div>
              <div className="min-w-0">
                <AutoFitText className="text-lg sm:text-xl font-extrabold text-white">{formatBRL(onlinePrizeGuaranteed)}</AutoFitText>
                <p className="text-[11px] font-semibold text-white/60 mt-0.5 flex items-center gap-1"><CheckCircle2 size={11} className="shrink-0" /> Prêmio garantido</p>
              </div>
              <div className="min-w-0">
                <AutoFitText className="text-lg sm:text-xl font-extrabold text-white">{formatPct(onlineProgressPct)}</AutoFitText>
                <p className="text-[11px] font-semibold text-white/60 mt-0.5 flex items-center gap-1"><Trophy size={11} className="shrink-0" /> {onlineTierInPlay ? onlineTierInPlay.name : "meta"}</p>
              </div>
            </div>
          </div>
                    ...
                    <li key={g.id} className="py-3">
                      <div className="flex items-center justify-between gap-2">
                        <span className="font-bold text-sm text-navy flex items-center gap-1.5 min-w-0">
                          ...
                          {!achieved && isInPlay && <span className="badge bg-gold/15 text-gold shrink-0">em jogo</span>}
                        </span>
                  ...
                  {onlineSales.map((s) => (
                    <tr key={s.id} className="border-b border-white/10 last:border-0">
                      ...
                      <td className="py-2 text-right whitespace-nowrap">
                        <button type="button" title="Editar" aria-label="Editar" className="p-1.5 rounded-lg text-white/50 hover:text-white hover:bg-surface/10 transition-colors" onClick={() => openOnlineEdit(s)}>
                          <Pencil size={13} />
                        </button>
                        <button type="button" title="Excluir" aria-label="Excluir" className="p-1.5 rounded-lg text-white/50 hover:text-danger hover:bg-surface/10 transition-colors" onClick={() => setOConfirmDelete(s)}>
                          <Trash2 size={13} />
                        </button>
                      </td>
                    </tr>
