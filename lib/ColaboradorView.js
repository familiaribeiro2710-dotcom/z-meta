    ...
    <div className="space-y-6">
      ...
      {tab === "atividades" && (
        <div className="space-y-6">
                  ...
                  return (
                    <li key={goal.id} className="py-2.5">
                      <div className="flex items-center justify-between gap-2 text-sm">
                        <span className="font-medium text-navy">{goal.name}</span>
          ...
          {goals.length > 0 && (
            <div className="card animate-pop border-purple/20">
              <p className="label flex items-center gap-1.5"><Target size={14} /> Barra da meta — {goals[0].goal.name}</p>
              <ProgressBar pct={Number(goals[0].allocation.amount) > 0 ? Math.min(100, (soldSoFar / Number(goals[0].allocation.amount)) * 100) : 0} />
              <p className="text-xs text-muted mt-2">{formatBRL(soldSoFar)} vendido de {formatBRL(goals[0].allocation.amount)} de meta individual.</p>
            </div>
          )}
