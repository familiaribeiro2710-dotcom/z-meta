  ...
  if (gerenteId) {
    ...
    employees = (emps || [])
      .map((e) => {
        ...
        return { name: e.full_name, sold, commissionPct: storeTier.colaboradorPct, commission: sold * (storeTier.colaboradorPct / 100), prizes: validatedEmployeeIds.has(e.id) ? (prizesByEmp[e.id] || 0) : 0, warningsCount: warningsByEmp[e.id] || 0 };
      })
    ...
    employees = (emps || [])
      .map((e) => {
        ...
        return { name: e.full_name, sold, commissionPct: teamTier.colaboradorPct, commission: sold * (teamTier.colaboradorPct / 100), prizes: validatedEmployeeIds.has(e.id) ? (prizesByEmp[e.id] || 0) : 0, warningsCount: warningsByEmp[e.id] || 0 };
      })
      .sort((a, b) => b.sold - a.sold);
