...
export default function NotificationBell({ userId }) {
  ...
  // (limitação da Web Notifications API), só avisar onde resolver.
  if (permission === "denied" && !subscribed) {
    return (
      <button
        ...
        className="p-2 rounded-full text-muted/50 cursor-not-allowed shrink-0"
        onClick={() => {}}
      >
        <BellOff size={16} />
      </button>
  ...
  return (
    <button
      ...
      className={`p-2 rounded-full transition-colors shrink-0 disabled:opacity-50 ${
        subscribed ? "text-gold hover:text-gold hover:bg-gold/10" : "text-muted hover:text-gold hover:bg-line/60"
      }`}
    >
