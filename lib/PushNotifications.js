...
// Tempo relativo simples (há Xm/Xh/Xd) — só pro histórico do sino, não precisa de precisão além
// disso nem de virar util global ainda.
...
const ROLE_OPTIONS = {
  ...
  gerente: [
    ...
    { key: "meta_individual", label: "Colaborador da equipe bateu a meta" },
    { key: "tarefas_completas", label: "Colaborador concluiu todas as tarefas do dia" },
  ],
  socio: [
    { key: "venda_loja", label: "Venda registrada na loja" },
    { key: "meta_individual", label: "Colaborador bateu a meta individual" },
    { key: "meta_batida", label: "Meta da loja batida" },
    { key: "tarefas_completas", label: "Colaborador concluiu todas as tarefas do dia" },
  ],
  supervisor: [
    { key: "venda_loja", label: "Venda registrada na loja" },
  ...
  // Papel exclusivo de consórcio (2026-07-21) — a única notificação própria do administrativo é
  // "venda aguardando confirmação" (`push_notify_administrativos_loja`, pref_key
  // `venda_pendente_confirmacao`). "Venda confirmada"/"venda recusada" são preferências do
  // COLABORADOR dono da venda, não do administrativo que confirma/recusa.
  administrativo: [
    { key: "venda_pendente_confirmacao", label: "Venda aguardando confirmação" },
  ],
};
...
// 2026-07-21 — pedido do Felipe: sócio/supervisor/gerente também podem optar por receber o
// mesmo aviso de "venda aguardando confirmação" que hoje só o administrativo tinha (gerente
// ganhou a mesma autonomia de aprovar/recusar essa venda na aba Vendas). Só existe no
// consórcio (é o fluxo de confirmação do crm_leads, não existe em vestuário).
const VENDA_PENDENTE_OPTION = { key: "venda_pendente_confirmacao", label: "Venda aguardando confirmação" };
// 2026-08-24 — pedido do Felipe: gerente/supervisor/sócio também podem ser avisados quando um
// lead do funil é agendado (status vira "agendado"), mesmo padrão de venda/meta — só existe em
// consórcio (funil de crm_leads não existe em vestuário).
const AGENDAMENTO_OPTION = { key: "agendamento", label: "Novo agendamento marcado" };
...
function optionsForRole(role, isConsorcio) {
  ...
  if ((role === "gerente" || role === "socio" || role === "supervisor") && isConsorcio) {
    return [...base, VENDA_PENDENTE_OPTION, AGENDAMENTO_OPTION];
  }
  return base;
}
...
export default function NotificationBell({ userId }) {
  ...
  // 2026-07-21: histórico das últimas 48h (pedido do Felipe) — tabela `notifications`, gravada
  // dentro de `push_notify()` no banco (todo evento que manda push passa por ali, então cobre
  // tudo de uma vez, sem precisar duplicar lógica por tipo de evento aqui no front).
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (prof) {
        setRole(prof.role);
        // 2026-07-21: antes só rodava pro colaborador (únicos toggles que dependiam de
        // categoria eram lead_novo/nova_meta/nova_tarefa) — agora gerente/sócio/supervisor
        // também têm um toggle exclusivo de consórcio (venda_pendente_confirmacao), então a
        // detecção de categoria precisa rodar pros 4 papéis.
        const rolesThatNeedCategoria = ["colaborador", "gerente", "socio", "supervisor"];
        if (rolesThatNeedCategoria.includes(prof.role) && prof.empresa_id) {
          ...
          if (empresaRow?.categoria_id) {
            ...
            setIsConsorcio(isCrmCategoria(categoriaRow?.slug));
          }
        }
      }
      setRoleLoaded(true);
  ...
  return (
    <>
      ...
      {panelOpen && mounted && createPortal(
        ...
        <div
          className="fixed inset-0 z-50 overflow-y-auto bg-navyfixed/70 p-4 sm:p-6 pt-[calc(env(safe-area-inset-top)+1rem)]"
        ...
        >
          <div className="min-h-full flex items-start sm:items-center justify-center">
            <div className="card max-w-sm w-full my-8 sm:my-0" onClick={(e) => e.stopPropagation()}>
              ...
              <p className="text-[11px] uppercase tracking-wider text-muted font-bold mb-1.5 mt-4">Preferências</p>
              ...
              {/* 2026-07-21: histórico movido pra baixo das preferências (pedido do Felipe) — a
                  configuração vem primeiro, o histórico é só consulta. */}
              <div className="mt-4">
                <p className="text-[11px] uppercase tracking-wider text-muted font-bold mb-1.5">Últimas 48h</p>
                <div className="max-h-48 overflow-y-auto space-y-2 pr-1">
                  {historyLoading ? (
                    <p className="text-xs text-muted py-2">carregando…</p>
                  ) : history.length === 0 ? (
                    <p className="text-xs text-muted py-2">Nenhuma notificação nas últimas 48h.</p>
                  ) : (
                    history.map((n) => (
                      <div key={n.id} className="rounded-xl bg-paper border border-line p-2.5">
                        <div className="flex items-center justify-between gap-2">
                          <p className="text-xs font-bold text-navy truncate">{n.title}</p>
                          <span className="text-[10px] text-muted shrink-0 whitespace-nowrap">{relativeTime(n.created_at)}</span>
                        </div>
