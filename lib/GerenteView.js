...
import { greeting, todayStr, firstDayOfMonth, nextMonth, remainingPeriodsInMonth, PACING_HERO_LABEL, PACING_UNIT, monthLabel, isTaskDueOn, daysElapsedInMonth } from "./date";
...
export default function GerenteView({ profile, tab, viewedBySupervisor = false, onBack }) {
  ...
  const loadStats = useCallback(async (prof, monthArg) => {
    ...
    const { data: goalRows } = await supabase
      .from("sales_goals")
      .select("id, name, store_total, commission_pct_colaborador, commission_pct_gerente, non_monetary_prize, pacing")
  ...
  const prizeGoalsAll = goalsList.filter((g) => g.non_monetary_prize);
