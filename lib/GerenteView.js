        ...
        <>
          ...
          {activeGoal && (
            <div className="card animate-pop border-purple/20">
              <p className="label flex items-center gap-1.5"><Target size={14} /> Barra da meta em jogo — {activeGoal.name}</p>
              <ProgressBar pct={Number(activeGoal.store_total) > 0 ? Math.min(100, (hero.soldLoja / Number(activeGoal.store_total)) * 100) : 0} />
              <p className="text-xs text-muted mt-2">{formatBRL(hero.soldLoja)} vendido de {formatBRL(activeGoal.store_total)} de meta.</p>
            </div>
          )}
