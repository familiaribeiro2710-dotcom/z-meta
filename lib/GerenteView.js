...
import { greeting, todayStr, firstDayOfMonth, remainingDaysInMonth, monthLabel, isTaskDueOn, daysInMonth } from "./date";
...
export default function GerenteView({ profile, tab, viewedBySupervisor = false, onBack }) {
  ...
  const loadStats = useCallback(async (prof, monthArg) => {
    ...
    if (empIds.length) {
      ...
      const rangeEnd = monthArg === firstDayOfMonth(todayNow) ? todayNow : `${monthArg.slice(0, 7)}-${String(daysInMonth(monthArg)).padStart(2, "0")}`;
      const daysRange = [];
      {
