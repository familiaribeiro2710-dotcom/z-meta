...
export default function GerenteView({ profile, tab, viewedBySupervisor = false, onBack }) {
  ...
  // ultrapassa a meta que carrega o prêmio (pedido do Felipe, achado num caso real na ArmyBR).
  const extraPrizeGoal = goalsList.find((g) => g.non_monetary_prize && hero.soldLoja < Number(g.store_total)) || goalsList.find((g) => g.non_monetary_prize) || null;
