...
import { Store, Rocket, CalendarClock, ListTodo, Coins, Gift, Eye, ArrowLeft, CheckSquare, Check, AlertTriangle, Target } from "lucide-react";
...
export default function GerenteView({ profile, tab, viewedBySupervisor = false, onBack }) {
  ...
  const [baseGoalAllocs, setBaseGoalAllocs] = useState([]);
  ...
  const loadStats = useCallback(async (prof, monthArg) => {
    ...
    // distribuição da meta base (a primeira/menor) entre os colaboradores da equipe — só leitura pro gerente
    if (goalRows && goalRows.length && empIds.length) {
      const { data: baseAllocRows } = await supabase
        .from("sales_goal_allocations")
        .select("*")
        .eq("goal_id", goalRows[0].id)
        .in("employee_id", empIds);
      setBaseGoalAllocs(baseAllocRows || []);
    } else {
      setBaseGoalAllocs([]);
    }
        ...
        <>
              ...
              <ul className="divide-y divide-line">
                {goalsList.map((g, idx) => (
                  <li key={g.id} className="flex items-center justify-between gap-2 py-2.5 text-sm">
                    <span className="font-medium text-navy">Meta {idx + 1} — {g.name}</span>
                    <span className="text-muted">{formatBRL(g.store_total)} · {Number(g.commission_pct_colaborador) || 0}% colab. · {Number(g.commission_pct_gerente) || 0}% gerente</span>
                  </li>
                ))}
              </ul>
            </div>
          ...
          {goalsList.length > 0 && (
            <div className="card animate-pop border-orange/20">
              <p className="label mb-3 flex items-center gap-1.5"><Target size={14} /> Distribuição da meta — {goalsList[0].name}</p>
              {teamEmps.length === 0 ? (
                <p className="text-sm text-muted">Nenhum colaborador na equipe ainda.</p>
              ) : (
                <ul className="divide-y divide-line">
                  {teamEmps.map((emp) => {
                    const alloc = baseGoalAllocs.find((a) => a.employee_id === emp.id);
                    return (
                      <li key={emp.id} className="flex items-center justify-between gap-2 py-2 text-sm">
                        <span className="text-navy font-medium">{emp.full_name}</span>
                        <span className="text-muted">{alloc ? formatBRL(alloc.amount) : "sem distribuição"}</span>
                      </li>
                    );
                  })}
                </ul>
          ...
          {goalsList.length > 0 && (
            <div className="card animate-pop border-purple/20">
              <p className="label flex items-center gap-1.5"><Target size={14} /> Barra da meta — {goalsList[0].name}</p>
              <ProgressBar pct={Number(goalsList[0].store_total) > 0 ? Math.min(100, (hero.soldLoja / Number(goalsList[0].store_total)) * 100) : 0} />
              <p className="text-xs text-muted mt-2">{formatBRL(hero.soldLoja)} vendido de {formatBRL(goalsList[0].store_total)} de meta.</p>
            </div>
          )}

          {(personalTasks.length > 0 || personalWarnings.length > 0 || personalPrizes.length > 0) && (
