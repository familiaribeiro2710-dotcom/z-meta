...
export function motivationalMessage(pct) {
  if (pct >= 100) return "Perfeito! Dia impecável. 🏆";
  if (pct >= 80) return "Mandando bem! Continua assim. 🔥";
  if (pct >= 50) return "Na média, dá pra melhorar hoje. 💪";
  if (pct > 0) return "Bora acelerar, o dia ainda não acabou. ⚡";
  return "Ainda não começou hoje — vamos! 🚀";
}
