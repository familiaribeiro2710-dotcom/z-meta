...
function Metas({ goals, allocations, commissionSettings, activeEmpIds, employees, soldLoja, storeMetaTotal, vendasMes, produtoCategorias, month, empresaId, lojaId, canManage, settings, onChanged }) {
  const notifySaved = useSavedNotice();
              ...
              <div key={g.id} className={`card animate-pop ${borders[i % borders.length]}`}>
                ...
                {editingPrizeId === g.id ? (
                  ...
                  >
                    ...
                    {canManage && <Pencil size={11} className="text-muted shrink-0 ml-auto" />}
                  </button>
                ) : (
                  canManage && (
