...
function Funil({ leads, leadsThisMonth, funilBoard, employees, month, produtoCategorias, empresaId, canManageCategorias, onChanged }) {
  const today = todayStr();
  ...
  async function exportExcel() {
    ...
    try {
      ...
      const resumoRows = funilBoard.map((b) => ({
        Colaborador: b.employee.full_name,
        Ligações: b.total,
        Agendados: b.agendados,
        "Agendados que venderam": b.agendadosVendidos,
        "Agendados perdidos": b.agendadosPerdidos,
        "Follow-up": b.followUp,
        Perdidos: b.perdidos,
        Vendidos: b.vendidosCount,
        "Valor vendido": b.valorVendido,
      }));
      ...
      XLSX.writeFile(wb, `leads-consorcio-${month}.xlsx`);
    } finally {
      setExporting(false);
    }
  }
