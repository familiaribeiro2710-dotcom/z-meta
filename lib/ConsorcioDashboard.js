...
function Metas({ goals, allocations, commissionSettings, activeEmpIds, employees, soldLoja, storeMetaTotal, vendasMes, produtoCategorias, month, empresaId, lojaId, canManage, viewerRole, settings, onChanged }) {
  const notifySaved = useSavedNotice();
  // Prêmio não-monetário (2026-08-04, pedido do Felipe): gerente não gerencia meta (valor/comissão/
  // exclusão continuam só de sócio/supervisor/master), mas PRECISA poder escolher em qual meta cadastrada
  // vai valer o prêmio não-monetário da equipe — permissão só pra esse campo específico.
  ...
  // Prêmio não-monetário (2026-08-04, pedido do Felipe): meta que não é paga em comissão, e sim
  // em algo tipo festa/jantar/viagem — texto livre, opcional, só aparece se preenchido.
  const [gPremioNaoMonetario, setGPremioNaoMonetario] = useState("");
  ...
  async function addGoal(e) {
    ...
    const { data: goal, error } = await supabase
      ...
      .insert({
        ...
        non_monetary_prize: gPremioNaoMonetario.trim() || null,
      })
      .select()
      .single();
    ...
    setSaving(false);
    setGNome(""); setGValor(""); setGPctColab(""); setGPctGerente(""); setGPremioNaoMonetario("");
    setFormOpen(false);
    notifySaved();
    await onChanged();
  }
  ...
  // Edição rápida só do prêmio não-monetário (meta em si não tem edição geral ainda — Felipe
  // confirmou que por ora só esse campo precisa, 2026-08-04).
  ...
  async function savePrizeText(goalId) {
    ...
    await supabase.from("consorcio_goals").update({ non_monetary_prize: editPrizeText.trim() || null }).eq("id", goalId);
            ...
            return (
              <div key={g.id} className={`card animate-pop ${borders[i % borders.length]}`}>
                <div className="flex items-start justify-between gap-2">
                  ...
                  {canManage && (
                    <button type="button" onClick={() => setConfirmDeleteGoal(g)} title="Excluir" aria-label="Excluir" className="p-1 rounded-lg text-muted hover:text-danger hover:bg-danger/10 transition-colors shrink-0">
                      <Trash2 size={14} />
                    </button>
                  )}
                </div>
          ...
          {formOpen && (
            <form onSubmit={addGoal} className="grid sm:grid-cols-2 gap-3 mt-3">
              ...
              <div className="sm:col-span-2">
                ...
                <input className="input" placeholder="ex: jantar de confraternização, viagem, festa da equipe" value={gPremioNaoMonetario} onChange={(e) => setGPremioNaoMonetario(e.target.value)} maxLength={120} />
