...
function Funil({ leads, leadsThisMonth, funilBoard, employees, viewerRole, month, produtoCategorias, empresaId, canManageCategorias, onChanged }) {
  ...
  function toggleExportStatus(key) {
    setExportStatuses((prev) => ({ ...prev, [key]: !prev[key] }));
  }

  async function exportExcel() {
    const anyStatusChecked = EXPORT_STATUS_OPTIONS.some((s) => exportStatuses[s.key]);
    if (!anyStatusChecked) return;
    setExporting(true);
    try {
      // filtro por "data da ligação" (é a data que todo lead sempre tem, diferente de agendamento/
      // venda que só existem em parte do funil) + status marcados no modal.
      const filtered = leads.filter((l) => {
        if (exportDataIni && (!l.data_ligacao || l.data_ligacao < exportDataIni)) return false;
        if (exportDataFim && (!l.data_ligacao || l.data_ligacao > exportDataFim)) return false;
        if (!exportStatuses[l.status]) return false;
        return true;
      });
      ...
      const resumoRows = employees
        .filter((e) => e.active)
        .map((emp) => {
          const empLeads = filtered.filter((l) => l.employee_id === emp.id);
          return {
      ...
      const suffix = exportDataIni || exportDataFim ? `${exportDataIni || "inicio"}_a_${exportDataFim || "hoje"}` : month;
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <div className="card animate-pop border-blue/20 min-w-0">
          ...
          <p className="text-[11px] text-muted mt-0.5 flex items-center gap-1"><CalendarClock size={11} className="shrink-0" /> Agendamentos hoje</p>
        </div>
        <div className="card animate-pop border-blue/20 min-w-0">
      ...
      <div className="card overflow-x-auto">
        <div className="flex items-center justify-between gap-2 mb-3">
          <p className="label mb-0 flex items-center gap-1.5"><Trophy size={14} /> Funil por colaborador</p>
          {canExport && (
            <button
              type="button"
      ...
      {canManageCategorias && (
        <div className="card">
          <button type="button" onClick={() => setCatOpen((v) => !v)} className="w-full flex items-center justify-between gap-2">
            <p className="label mb-0 flex items-center gap-1.5"><Tag size={14} /> Categorias de produto (consórcio)</p>
                ...
                <ul className="divide-y divide-line">
                  {produtoCategorias.map((c) => (
                    <li key={c.id} className="flex items-center justify-between gap-2 py-2 text-sm">
                      ...
                      <button type="button" onClick={() => toggleCategoria(c)} className="text-[11px] font-bold uppercase tracking-wider text-muted hover:text-purple shrink-0">
      ...
      {exportModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-navy/70 p-6">
          <div className="card max-w-sm w-full max-h-[85vh] overflow-y-auto animate-bounce-in">
            <p className="label mb-3 flex items-center gap-1.5"><Download size={14} /> Exportar leads</p>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="label">De</label>
                <input type="date" className="input date-input" value={exportDataIni} onChange={(e) => setExportDataIni(e.target.value)} max={exportDataFim || undefined} />
              </div>
              <div>
                <label className="label">Até</label>
                <input type="date" className="input date-input" value={exportDataFim} onChange={(e) => setExportDataFim(e.target.value)} min={exportDataIni || undefined} />
              </div>
            </div>
            <p className="text-[11px] text-muted mt-1">Filtro por data da ligação. Deixe em branco pra não filtrar por período.</p>

            <p className="label mt-4 mb-2">Status</p>
            <div className="grid grid-cols-2 gap-2">
              {EXPORT_STATUS_OPTIONS.map((s) => (
                <label key={s.key} className="flex items-center gap-2 text-sm cursor-pointer">
                  <input type="checkbox" checked={!!exportStatuses[s.key]} onChange={() => toggleExportStatus(s.key)} />
                  {s.label}
                </label>
              ))}
            </div>
            ...
            <div className="flex gap-2 mt-5">
              <button type="button" className="btn-outline flex-1" onClick={() => setExportModalOpen(false)}>Cancelar</button>
              <button type="button" className="btn flex-1" onClick={exportExcel} disabled={exporting}>
