...
const STATUS_LABEL = { novo: "Novo", agendado: "Agendado", follow_up: "Follow-up", perdido: "Perdido", vendido: "Vendido", vendido_pendente: "Aguardando confirmação" };
...
function Funil({ leads, leadsThisMonth, funilBoard, employees, viewerRole, month, produtoCategorias, empresaId, canManageCategorias, onChanged }) {
  ...
  const filtroAtivo = !!(filtroDataIni || filtroDataFim || filtroCategoriaId || filtroColaboradorId || busca.trim() || EXPORT_STATUS_OPTIONS.some((s) => !filtroStatuses[s.key]));
  ...
  const abertos = leads.filter((l) => l.status !== "vendido" && l.status !== "perdido" && l.status !== "vendido_pendente").length;
  ...
  const agendadosEmAberto = leadsThisMonth.filter((l) => l.agendamento_at && l.status !== "vendido" && l.status !== "perdido" && l.status !== "vendido_pendente").length;
  ...
  function limparFiltros() {
    ...
    setDraftDataIni(""); setDraftDataFim(""); setDraftCategoriaId(""); setDraftColaboradorId(""); setDraftBusca(""); setDraftStatuses(all);
    setFiltroDataIni(""); setFiltroDataFim(""); setFiltroCategoriaId(""); setFiltroColaboradorId(""); setBusca(""); setFiltroStatuses(all);
    setPage(1);
  }
