...
import { useLeadListas, ListaFilterSelect, ListaPicker, matchesListaFilter, LeadListasManager } from "./leadListas";
import { useSavedNotice } from "./SavedNotice";

// Mesmas chaves/posição de EMPRESA_TABS (vestuário) — Início/Metas — pra manter o AppShell
// idêntico entre os dois segmentos, só o conteúdo interno muda. 2026-08-08: "Pipeline" entrou como
// 3ª aba, escondida em telas pequenas (hideOnMobile, ver AppShell.js) — pedido do Felipe.
// 2026-08-12 (pedido do Felipe): "Leads" generaliza pra TODOS os papéis (respeitando hierarquia) a
// aba que antes só o gerente tinha (GERENTE_TABS, GerenteViewConsorcio.js) — listagem de leads +
// cadastro de cliente novo. Entra aqui, em vez de ficar só em GERENTE_TABS, porque o componente
// LeadsTab (definido no fim deste arquivo) já reaproveita o `employees` que este componente carrega
// em `loadAll` — escopado por equipe quando viewerRole="gerente" ou pela loja inteira quando
// sócio/supervisor/master (mesmo mecanismo de escopo já usado pelo Pipeline logo acima). Isso
// automaticamente propaga a aba pra sócio/supervisor (HierarchyHome.js) e master (admin/page.js),
// que montam este componente via CONSORCIO_TABS — só precisou também entrar em GERENTE_TABS
// (GerenteViewConsorcio.js) pro gerente, que usa uma lista de abas própria.
...
export default function ConsorcioDashboard({ lojaId, empresaId, viewerRole = "master_admin", viewerId, tab = "atividades", month: monthProp, onOpenEmployee, onOpenGerente, atSub: atSubProp, onAtSubChange }) {
  ...
  // Mesma régua do Funil (canManageLeads) — só "leitor" (supervisor com acesso só de visualização
  // numa loja) fica sem poder arrastar/agir no Pipeline. 2026-08-12: reaproveitada também pra
  // LeadsTab (cadastrar cliente/transferir) — mesmo limite de "só visualização" faz sentido nos dois.
  const canManagePipeline = viewerRole !== "leitor";
  ...
  return (
    <div className="space-y-6">
      ...
      {tab === "pipeline" && (
        <Pipeline
          // 2026-08-08: este mesmo componente/rota é reaproveitado tanto pelo Início do
          // supervisor/sócio/master (CONSORCIO_TABS, escopo = loja inteira) quanto pelo Pipeline do
          // GERENTE_TABS via GerenteViewConsorcio.js (que delega qualquer tab != calendario/leads/
          // vendas pra cá com viewerRole="gerente") — quando é gerente, `employees` já vem filtrado
          // só pela própria equipe (ver loadAll acima), então o escopo tem que ser por employeeIds,
          // não pela loja toda, senão o gerente veria leads de outras equipes da mesma loja.
          employeeIds={viewerRole === "gerente" ? employees.map((e) => e.id) : undefined}
          lojaId={viewerRole === "gerente" ? undefined : lojaId}
          employees={employees}
          empresaId={empresaId}
...
const STATUS_CHIP_DARK = {
  ...
  // 2026-08-10: lead que estava "Novo" (nunca teve nenhuma movimentação) e foi marcado como
  // perdido — separado de "Perdido" de verdade (que já teve alguma tentativa/negociação).
  sem_contato: "bg-[rgba(148,163,184,0.22)] text-[#cbd5e1]",
};

// Mesma paleta semântica de STATUS_CHIP_DARK, só que pensada pra fundo claro (.card branco) — usada
// pela tabela de LeadsTab (fim do arquivo), que fica num card claro, diferente do resto do Funil
// (fundo escuro). Duplicada de propósito (mesmo padrão de isolar risco já usado em todo o módulo de
// consórcio) em vez de tentar unificar as duas paletas numa função de uma cor só.
const STATUS_CHIP = {
...
// Todos os status marcados — usado como default do filtro de status na aba Leads (LeadsTab),
// diferente do default do Funil (só "em_negociacao"): a aba Leads sempre mostrou todo mundo, então
// o filtro novo não pode mudar o que já aparecia antes de existir.
...
function Funil({ leads, leadsThisMonth, funilBoard, employees, viewerRole, month, produtoCategorias, empresaId, canManageCategorias, onChanged }) {
  ...
  // um furo — colaborador pode ligar, CONSEGUIR contato, cliente não demonstrar interesse, e o lead
  // nunca sair de "novo" (não existe etapa intermediária pra "contato feito, ainda decidindo").
  ...
  // antes se o gestor não mexer no campo).
  ...
  async function confirmResolve() {
    ...
    if (type === "vendido") {
      ...
      // gestor confirma que houve contato, fica "perdido" mesmo (ver pergunta no modal).
  ...
  async function exportExcel() {
    ...
    try {
      ...
      // 2026-09-03: não filtra mais por `e.active` aqui — quem foi desativado mas tem ligação
      // dentro do período filtrado precisa continuar aparecendo no resumo (o `.filter(r =>
      // r.Ligações > 0)` no final já cuida de tirar quem não tem nada no período, ativo ou não).
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="card">
        <button type="button" onClick={() => setFiltrosOpen((v) => !v)} className="w-full flex items-center justify-between gap-2">
          <p className="label mb-0 flex items-center gap-1.5">
            <Filter size={14} /> Filtros{filtroAtivo && <span className="badge bg-gold/15 text-gold !text-[10px]">ativo</span>}
          </p>
          ...
          <form onSubmit={aplicarFiltros} className="mt-3 space-y-3">
              ...
              <div className="flex items-center justify-between gap-2 mb-1.5">
                ...
                {canExport && (
                  <button
                    ...
                    className="text-[11px] font-bold uppercase tracking-wider text-muted hover:text-gold disabled:opacity-50 flex items-center gap-1"
                  >
            ...
            <div className="flex items-center gap-3 pt-1 flex-wrap">
              ...
              {(filtroAtivo || filtrosPendentes) && (
                <button type="button" onClick={limparFiltros} className="text-[11px] font-bold uppercase tracking-wider text-muted hover:text-gold">
                  Limpar filtros
                </button>
              )}
              ...
              <div className="mt-4 rounded-xl bg-paper border border-line p-3">
                ...
                <div className="space-y-2">
                  <label className="flex items-center gap-2 text-xs text-navy cursor-pointer">
                    <input type="radio" name="funilContatoFeito" checked={!resolveContatoFeito} onChange={() => setResolveContatoFeito(false)} />
                    Não, nunca consegui contato
                  </label>
                  <label className="flex items-center gap-2 text-xs text-navy cursor-pointer">
                    <input type="radio" name="funilContatoFeito" checked={resolveContatoFeito} onChange={() => setResolveContatoFeito(true)} />
                      ...
                      {editingCatId === c.id ? (
                        ...
                        <>
                          ...
                          <span className="flex items-center gap-1 shrink-0">
                            ...
                            <button type="button" onClick={() => setConfirmToggleCat(c)} className="text-[11px] font-bold uppercase tracking-wider text-muted hover:text-gold px-1">
      ...
      {viewerRole !== "leitor" && empresaId && <LeadListasManager empresaId={empresaId} />}
...
// Usada só por LeadsTab (fim do arquivo) — mesma função que já existia duplicada em
// GerenteViewConsorcio.js, movida pra cá junto com o componente que a usa.
  ...
  return (
    <li className="row-card items-start flex-wrap gap-y-2">
      ...
      <div className="min-w-0 flex-1 cursor-pointer" onClick={() => setExpanded((v) => !v)}>
        ...
        {expanded && (
          <div className="mt-2 grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-2 bg-surface/5 border border-dashed border-white/15 rounded-xl p-3" onClick={(e) => e.stopPropagation()}>
            <div>
      ...
      {(canManage && (canAgendar || canResolve)) || lead.status === "vendido" || lead.status === "vendido_pendente" || (lead.status === "em_negociacao" && lead.valor != null) ? (
        // 2026-08-11 (bug real reportado pelo Felipe, print da visão do sócio): valor e ícones de
        // ação eram dois irmãos flex separados na mesma linha do nome — num celular estreito, o
        // flex-wrap espremia a coluna do nome (flex-1/min-w-0) pra caber os dois ao lado dela em vez
        // de simplesmente quebrar de linha, e o telefone/responsável (sem truncate) acabavam
        // quebrando letra a letra. Agrupar os dois num único wrapper `w-full sm:w-auto` resolve: no
        // mobile ele vira uma segunda linha inteira própria (não compete por espaço com o nome), no
        // desktop volta a ficar ao lado, igual antes.
        <div className="flex items-center gap-2 flex-wrap w-full sm:w-auto justify-between sm:justify-end shrink-0">
          ...
          {canManage && (canAgendar || canResolve) && (
            <div className="flex items-center gap-1 shrink-0" onClick={(e) => e.stopPropagation()}>
              {canAgendar && (
                <button type="button" onClick={() => onAgendar(lead)} title="Agendar / reagendar" aria-label="Agendar" className="p-1.5 rounded-lg text-white/60 hover:text-bluelight hover:bg-surface/10 transition-colors">
                  <CalendarPlus size={14} />
                </button>
              )}
              {canResolve && (
                <>
                  <button type="button" onClick={() => onResolve(lead, "em_negociacao")} title="Em negociação" aria-label="Em negociação" className="p-1.5 rounded-lg text-white/60 hover:text-teal hover:bg-surface/10 transition-colors">
                    <Handshake size={14} />
                  </button>
                  <button type="button" onClick={() => onResolve(lead, "perdido")} title="Perdido" aria-label="Perdido" className="p-1.5 rounded-lg text-white/60 hover:text-danger hover:bg-surface/10 transition-colors">
                    <XCircle size={14} />
                  </button>
                  <button type="button" onClick={() => onResolve(lead, "follow_up")} title="Follow-up" aria-label="Follow-up" className="p-1.5 rounded-lg text-white/60 hover:text-warn hover:bg-surface/10 transition-colors">
                    <Clock size={14} />
                  </button>
                  <button type="button" onClick={() => onResolve(lead, "vendido")} title="Vendido" aria-label="Vendido" className="p-1.5 rounded-lg text-white/60 hover:text-success hover:bg-surface/10 transition-colors">
...
// Pills de "Igual pra todos / Valor manual / Por percentual" + os campos por colaborador — mesmo
// componente/motivo de EmpresaDashboard.js (vestuário). Precisa viver FORA de Metas (não como
// função aninhada no corpo dela): componente redefinido a cada render do pai é uma identidade nova
// pro React, que desmonta e remonta o <input> inteiro — no iPad/mobile isso fecha o teclado a cada
// dígito digitado (bug real reportado pelo Felipe, 2026-09-04, inclusive travando o cadastro de
// meta manual pra loja com 1 colaborador só). Mesmo motivo por trás de PacingFields logo abaixo.
function DistributionFields({ mode, setMode, vals, setVals, storeTotalForCalc, error, activeEmps }) {
  const sum = mode === "percent" ? activeEmps.reduce((s, emp) => s + (Number(vals[emp.id]) || 0), 0) : 0;
  return (
...
function Metas({ goals, allocations, commissionSettings, activeEmpIds, employees, soldLoja, storeMetaTotal, vendasMes, produtoCategorias, month, empresaId, lojaId, canManage, viewerRole, settings, onChanged }) {
  ...
  async function addGoal(e) {
    ...
    if (!error && goal) {
      ...
      if (rows.length) {
        // BUG REAL (Felipe, 2026-09-04): esse upsert silenciosamente não salvava nada
        // (consorcio_goal_allocations não tinha o índice único que onConflict exige — corrigido no
        // banco) e o erro nunca era checado, então a tela mostrava "salvo" mesmo sem distribuição
        // nenhuma. Checar o erro aqui é defesa em profundidade — não deixa esse tipo de falha
        // passar em silêncio de novo, seja qual for a causa.
        const { error: allocError } = await supabase.from("consorcio_goal_allocations").upsert(rows, { onConflict: "goal_id,employee_id" });
        if (allocError) {
          setSaving(false);
          setCreateError("Meta criada, mas a distribuição não foi salva: " + allocError.message);
          await onChanged();
          return;
        }
      }
    ...
    if (goalAllocs.length === 0 || isEvenSplit(goalAllocs)) {
      ...
      if (rows.length) {
        ...
        if (allocError) {
          ...
          notifySaved("Valores da meta salvos, mas a redistribuição igual falhou: " + allocError.message);
          await onChanged();
          return;
        }
      }
  ...
  async function saveDistribution(goal) {
    ...
    if (rows.length) {
      const { error: allocError } = await supabase.from("consorcio_goal_allocations").upsert(rows, { onConflict: "goal_id,employee_id" });
      if (allocError) {
        setDistError("Não foi possível salvar a distribuição: " + allocError.message);
        return;
      }
    }
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-lg sm:text-xl font-bold text-navy flex items-center gap-2"><Wallet size={20} className="text-gold" /> Metas — {monthLabel(month)}</h1>
              ...
              <div key={g.id} className={`card animate-pop ${borders[i % borders.length]}`}>
                <div className="flex items-start justify-between gap-2">
                  <p className="font-bold text-xs sm:text-sm text-navy flex items-center gap-1.5 flex-wrap">
                    ...
                    {g.pacing && g.pacing !== "dia" && <span className="badge bg-gold/15 text-gold">ritmo: {PACING_LABELS[g.pacing]?.toLowerCase()}</span>}
                  </p>
...
// 2026-08-12 (pedido do Felipe): "Leads" generalizado pra todos os papéis com visão de loja/equipe
// (gerente/supervisor/sócio/master) — antes vivia só em GerenteViewConsorcio.js (LeadsTab,
// exclusivo do gerente, prop `teamEmps`). Movido pra cá e generalizado `teamEmps` -> `employees`
// (já vem escopado certo por quem monta <ConsorcioDashboard> — equipe pro gerente, loja inteira pro
// sócio/supervisor/master, ver comentário em CONSORCIO_TABS acima e loadAll). Também importado
// direto por ColaboradorViewConsorcio.js, passando employees=[o próprio perfil] — nesse caso a
// listagem cruzada/seleção/transferência em lote não fazem sentido (só 1 pessoa no escopo) e ficam
// escondidas sozinhas: todo gate de UI abaixo usa `multiEmp` (employees.length > 1).
// "Cadastrar cliente": pedido do Felipe, cadastro rápido direto desta aba — 2026-08-12: campos
// EXATAMENTE iguais ao "Cadastrar ligação" que o colaborador já tem em ColaboradorViewConsorcio.js
// (nome/telefone/endereço/email/data da ligação/agendar opcional/feedback), incluindo o mesmo
// comportamento de status (entra em 'novo', ou já em 'agendado' se a data+hora de agendamento
// forem preenchidas). Quando há mais de um colaborador no escopo, um seletor "Atribuir a" decide o
// dono do lead (obrigatório) — único campo a mais, porque o form do colaborador nunca precisa disso
// (é sempre ele mesmo). RLS de INSERT (`crm_leads_insert`) já cobre os 4 papéis: dono
// (employee_id = auth.uid()), gerente da equipe (is_my_team_member), sócio/supervisor com
// permissão de gerenciar a loja (can_manage_loja) e master_admin — nenhuma migração precisou mudar.
// 2026-09-04: o painel de histórico/observações do lead (antes só nesta aba) virou componente
// compartilhado — ver lib/LeadHistoryPanel.js (também usado pelo Pipeline, mesmo painel nos dois
// lugares em vez de duas versões divergentes). leadEventIcon/leadEventTitle moveram pra lá.

// Filtro de período (De/Até) da aba Leads — 2026-09-04 (pedido do Felipe): os dois campos de data
// sempre visíveis viraram um único botão com ícone de calendário; clicar abre um painel com os
// mesmos inputs De/Até (input nativo, mesmo comportamento de sempre — abre o calendário do
// sistema ao tocar), fechado por padrão pra não ocupar espaço quando ninguém está filtrando por
// período. Top-level de propósito (não aninhado dentro de LeadsTab): função-componente redefinida
// a cada render do pai perde a identidade pro React e remonta o campo inteiro — mesmo bug corrigido
// em DistributionFields (fechava o teclado do iPad a cada dígito, ver seção "BUG REAL" no
// CONTEXTO_PROJETO.md).
function PeriodoFilterField({ dataIni, setDataIni, dataFim, setDataFim }) {
  const [open, setOpen] = useState(false);
  const hasRange = !!(dataIni || dataFim);
  const label = hasRange
    ? `${dataIni ? fmtDataLigacao(dataIni) : "…"} – ${dataFim ? fmtDataLigacao(dataFim) : "…"}`
    : "Todo o período";
  return (
    <div className="relative">
      ...
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        className={`input w-full flex items-center gap-2 ${hasRange ? "text-navy" : "text-muted"}`}
      >
        <Calendar size={15} className="shrink-0 text-muted" />
        <span className="truncate">{label}</span>
      </button>
      {open && (
        <div className="absolute z-20 mt-1.5 p-3 rounded-xl border border-line bg-surface shadow-lg space-y-3 w-full min-w-[15rem]">
          <div>
            <label className="label">De</label>
            <input type="date" className="input date-input" value={dataIni} onChange={(e) => setDataIni(e.target.value)} max={dataFim || undefined} />
          </div>
          <div>
            <label className="label">Até</label>
            <input type="date" className="input date-input" value={dataFim} onChange={(e) => setDataFim(e.target.value)} min={dataIni || undefined} />
          </div>
          <button type="button" onClick={() => setOpen(false)} className="btn-outline w-full !py-1.5 !text-xs">
            Fechar
          </button>
        </div>
      )}
...
export function LeadsTab({ employees, lojaId, empresaId, viewerId, canManage = true }) {
  const notifySaved = useSavedNotice();
  const [leads, setLeads] = useState([]);
  const [loading, setLoading] = useState(true);
  const [page, setPage] = useState(0);
  // Rascunho do filtro só passa a valer ao clicar "Aplicar filtros" (mesmo padrão do resto do
  // consórcio — nenhum filtro recalcula a cada tecla digitada).
  const [draftFilterEmp, setDraftFilterEmp] = useState("todos");
  const [draftSearch, setDraftSearch] = useState("");
  // 2026-08-12 (pedido do Felipe): filtro por período — mesmo padrão De/Até já usado no Funil
  // (ColaboradorViewConsorcio.js), filtrando por data_ligacao.
  const [draftDataIni, setDraftDataIni] = useState("");
  const [draftDataFim, setDraftDataFim] = useState("");
  // 2026-09-01 (pedido do Felipe): filtro por status — mesmo padrão de checkboxes já usado no
  // Funil (EXPORT_STATUS_OPTIONS), só que aqui todos vêm marcados por padrão (a aba Leads sempre
  // mostrou todo mundo; o Funil por padrão só mostra "em_negociacao").
  const [draftStatuses, setDraftStatuses] = useState(allLeadStatusesOn);
  const [filterEmp, setFilterEmp] = useState("todos");
  const [search, setSearch] = useState("");
  const [dataIni, setDataIni] = useState("");
  const [dataFim, setDataFim] = useState("");
  const [filterStatuses, setFilterStatuses] = useState(allLeadStatusesOn);
  // 2026-09-24 (pedido do Felipe): filtro por lista de origem (ver lib/leadListas.js).
  const [draftFilterLista, setDraftFilterLista] = useState("");
  const [filterLista, setFilterLista] = useState("");
  const { listas, reload: reloadListas } = useLeadListas(empresaId);
  const listaNameById = {};
  listas.forEach((x) => { listaNameById[x.id] = x.nome; });
  ...
  // 2026-08-12 (pedido do Felipe): histórico do lead — clique na linha abre um painel com a linha
  // do tempo completa. 2026-09-04: virou LeadHistoryPanel compartilhado (ver import no topo do
  // arquivo) — aqui só guarda QUAL lead está aberto, o resto (fetch/render/adicionar observação)
  // é responsabilidade do componente.
  const [detailLead, setDetailLead] = useState(null);

  const multiEmp = employees.length > 1;
  const empScopeKey = employees.map((e) => e.id).join(",");
  const empNameById = {};
  employees.forEach((e) => { empNameById[e.id] = e.full_name; });
  ...
  const load = useCallback(async () => {
    ...
    const { data } = await supabase
      .from("crm_leads")
      .select("id, employee_id, empresa_id, loja_id, lista_id, nome_completo, telefone, status, data_ligacao, agendamento_at, valor, categoria_produto_id, feedback, observacoes, created_at")
  ...
  // 2026-08-12 (pedido do Felipe): "a aba Leads precisa ter as mesmas funções do dashboard Leads"
  // — agendar/marcar em negociação/perdido/follow-up/vendido direto daqui, mesma ação que já existe
  // no Funil (ManagerLeadRow) e no Pipeline. Categorias de produto carregadas aqui mesmo (mesmo
  // padrão 100% autocontido do Pipeline) porque LeadsTab não recebe esse dado do componente pai.
  const [produtoCategorias, setProdutoCategorias] = useState([]);
  useEffect(() => {
    if (!empresaId) return;
    (async () => {
  ...
  // ---- agendar / resolver lead direto da lista (mesma lógica de Funil/Pipeline, duplicada de
  // propósito — mesmo padrão de isolar risco entre árvores de componente já usado no resto do
  // consórcio) ----
  const [agendarModal, setAgendarModal] = useState(null); // lead
  const [agendarData, setAgendarData] = useState("");
  const [agendarHora, setAgendarHora] = useState("");
  ...
  const [resolveSaving, setResolveSaving] = useState(false);
  const [resolveError, setResolveError] = useState("");
  // "conseguiu contato?" — mesma régua criada pro Pipeline/Funil (ver comentário lá): só pergunta
  // quando é ambíguo (lead ainda em "novo" ao marcar perdido), senão continua automático.
  const [resolveContatoFeito, setResolveContatoFeito] = useState(false);
  ...
  async function confirmResolve() {
    ...
    if (type === "vendido") {
      ...
      // valor tem que ir null junto (CHECK crm_leads_vendido_shape_chk) — mesmo bug real já
      // corrigido em Pipeline.js/Funil (Fernando Santos de Souza/Canaã).
      const finalStatus = type === "perdido" && lead.status === "novo" ? (resolveContatoFeito ? "perdido" : "sem_contato") : type;
      ({ error } = await supabase
        .from("crm_leads")
        .update({ status: finalStatus, valor: null, feedback: resolveFeedback.trim() || null, updated_at: new Date().toISOString() })
  ...
  const filtered = leads.filter((l) => {
    ...
    if (dataIni && (!l.data_ligacao || l.data_ligacao < dataIni)) return false;
    if (dataFim && (!l.data_ligacao || l.data_ligacao > dataFim)) return false;
    if (!filterStatuses[l.status]) return false;
    if (!matchesListaFilter(l, filterLista)) return false;
    if (search.trim()) {
  ...
  const filtroAtivo = filterEmp !== "todos" || !!search.trim() || !!dataIni || !!dataFim || !!filterLista
    || EXPORT_STATUS_OPTIONS.some((s) => !filterStatuses[s.key]);
  const filtrosPendentes = draftFilterEmp !== filterEmp || draftSearch !== search || draftDataIni !== dataIni || draftDataFim !== dataFim || draftFilterLista !== filterLista
    || EXPORT_STATUS_OPTIONS.some((s) => !!draftStatuses[s.key] !== !!filterStatuses[s.key]);
  ...
  function aplicarFiltros(e) {
    ...
    setFilterStatuses(draftStatuses);
    setFilterLista(draftFilterLista);
    setPage(0);
  }

  function limparFiltros() {
    const all = allLeadStatusesOn();
  ...
  // 2026-09-15 (pedido do Felipe): "Exportar Excel" direto da aba Leads — mesmo padrão do botão
  // já existente no Funil (ConsorcioDashboard.js, exportExcel acima), exportando exatamente o que
  // está filtrado na tela (`filtered`), não o total geral sem filtro nenhum.
  async function exportExcel() {
    setExporting(true);
    try {
      const XLSX = await import("xlsx");
      ...
      const leadRows = filtered.map((l) => ({
        ...
        "Nome do cliente": l.nome_completo,
        Telefone: l.telefone,
        Lista: l.lista_id ? listaNameById[l.lista_id] || "" : "",
        "Data da ligação": l.data_ligacao || "",
        Agendamento: l.agendamento_at ? new Date(l.agendamento_at).toLocaleString("pt-BR") : "",
        Status: STATUS_LABEL[l.status] || l.status,
        Feedback: l.feedback || "",
      ...
      const wb = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(leadRows), "Leads");
      const suffix = dataIni || dataFim ? `${dataIni || "inicio"}_a_${dataFim || "hoje"}` : todayStr();
      XLSX.writeFile(wb, `leads-${suffix}.xlsx`);
    } finally {
      setExporting(false);
    }
  }

  // ---- cadastro rápido de cliente — 2026-08-12 (pedido do Felipe): campos EXATAMENTE iguais ao
  // "Cadastrar ligação" que o colaborador já tem em ColaboradorViewConsorcio.js (nome/telefone/
  // endereço/email/data da ligação/agendar opcional/feedback), pra não ter dois formulários com
  // pegadinhas diferentes. Única diferença real é o seletor "Atribuir a" (só aparece com mais de 1
  // colaborador no escopo — o form do colaborador nunca precisa disso, é sempre ele mesmo). ----
  const [formOpen, setFormOpen] = useState(false);
  const [fNome, setFNome] = useState("");
  const [fTelefone, setFTelefone] = useState("");
  const [fEndereco, setFEndereco] = useState("");
  const [fEmail, setFEmail] = useState("");
  const [fDataLigacao, setFDataLigacao] = useState(todayStr());
  ...
  function resetForm() {
    ...
    setFDataLigacao(todayStr()); setFAgendaData(""); setFAgendaHora(""); setFFeedback(""); setFAssignTo(""); setFListaId(""); setFormMsg("");
  }
  ...
  async function submitLead(e) {
    ...
    // lead novo só pode ser atribuído a colaborador ATIVO — quem foi desativado continua aparecendo
    // no resto da aba (histórico/filtro/coluna), só não pode receber lead novo.
    const assignableEmps = employees.filter((e) => e.active);
    const assignedTo = multiEmp ? fAssignTo : assignableEmps[0]?.id;
    if (!assignedTo) {
      setFormMsg(assignableEmps.length ? "Selecione o colaborador responsável." : "Nenhum colaborador ativo pra atribuir esse lead.");
      return;
    }
    setSaving(true);
    ...
    const { error } = await supabase.from("crm_leads").insert({
      ...
      data_ligacao: fDataLigacao || todayStr(),
    ...
    resetForm();
    setFormOpen(false);
    notifySaved("Cliente cadastrado com sucesso.");
    await load();
  }
  ...
  return (
    <div className="space-y-4">
      {canManage && (
        <div className="card">
          <button type="button" onClick={() => setFormOpen((v) => !v)} className="w-full flex items-center justify-between gap-2">
            <p className="label mb-0 flex items-center gap-1.5"><Plus size={14} /> Cadastrar lead</p>
            {formOpen ? <ChevronUp size={15} className="text-muted" /> : <ChevronDown size={15} className="text-muted" />}
          </button>
          {formOpen && (
            <form onSubmit={submitLead} className="grid sm:grid-cols-2 gap-3 mt-3">
              ...
              <div className="sm:col-span-2 grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div>
                  ...
                  <input type="date" className="input date-input" value={fDataLigacao} max={todayStr()} onChange={(e) => setFDataLigacao(e.target.value)} required />
                </div>
                <div>
              ...
              {multiEmp && (
                <div>
                  ...
                  <SelectField className="w-full" value={fAssignTo} onChange={(e) => setFAssignTo(e.target.value)}>
              ...
              <div>
                ...
                <ListaPicker empresaId={empresaId} listas={listas} value={fListaId} onChange={setFListaId} onCreated={reloadListas} canCreate={canManage} />
              </div>
              <div className="sm:col-span-2">
              ...
              <div className="sm:col-span-2">
                <button className="btn" type="submit" disabled={saving}>{saving ? "Salvando…" : "Cadastrar lead"}</button>
              </div>
              {formMsg && <p className="sm:col-span-2 text-xs text-danger font-bold">{formMsg}</p>}
            </form>
          )}
        </div>
      )}
      ...
      <div className="card">
        {/* 2026-08-12 (bug real reportado pelo Felipe, print da aba Leads): o botão "Aplicar
            filtros" ficava numa linha ao lado dos campos com label (Buscar/Colaborador/De/Até),
            só que sem label própria acima — como cada campo rotulado é mais alto que o botão
            sozinho, o botão nunca ficava de fato alinhado com a base dos inputs em nenhuma
            largura de tela (mobile, iPad ou desktop). Trocado pro mesmo padrão de grade usada em
            todo o resto do app (AdministrativoView.js/HierarchyHome.js/GerenteViewConsorcio.js,
            aba Vendas): cada campo na própria célula do grid, e o botão numa linha PRÓPRIA
            (sm:col-span-2 lg:col-span-4) — nunca compete de altura com um campo vizinho, em
            nenhum breakpoint. */}
        <form onSubmit={aplicarFiltros} className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3">
          ...
          <PeriodoFilterField dataIni={draftDataIni} setDataIni={setDraftDataIni} dataFim={draftDataFim} setDataFim={setDraftDataFim} />
          {listas.length > 0 && (
            <div>
              <label className="label">Lista</label>
              <ListaFilterSelect listas={listas} value={draftFilterLista} onChange={setDraftFilterLista} className="w-full" />
            </div>
          )}
      ...
      {canManage && multiEmp && selectedCount > 0 && (
      ...
      <div className="card overflow-x-auto">
        <div className="flex items-center justify-between gap-2 mb-3 flex-wrap">
          <div>
            <p className="label mb-0">Leads {multiEmp ? "da equipe" : "cadastrados"} ({filtered.length})</p>
            {filtroAtivo && (
              <p className="text-[11px] text-muted mt-0.5">Total cadastrado (sem filtro): {leads.length}</p>
            )}
          </div>
          <div className="flex items-center gap-3 flex-wrap">
            {filtered.length > 0 && (
              <button
                ...
                onClick={exportExcel}
                disabled={exporting}
                className="text-xs font-bold text-blue hover:underline disabled:opacity-50 flex items-center gap-1"
              >
                <Download size={13} /> {exporting ? "Gerando…" : "Exportar Excel"}
              </button>
            )}
            {canManage && multiEmp && filtered.length > 0 && (
            ...
            <table className="w-full text-xs sm:text-sm">
              <thead>
                <tr className="text-left text-[10px] sm:text-xs uppercase tracking-wider text-muted border-b border-line">
                  ...
                  <th className="pb-2 pr-3 whitespace-nowrap">Ligação</th>
                  {canManage && <th className="pb-2 whitespace-nowrap">Ações</th>}
                </tr>
              </thead>
              <tbody>
                {pageItems.map((l) => {
                  // 2026-08-12 (pedido do Felipe): "a aba Leads precisa ter as mesmas funções do
                  // dashboard Leads" — mesmas guardas/ações do ManagerLeadRow (Funil): agendar,
                  // marcar em negociação/perdido/follow-up/vendido direto da lista, sem precisar
                  // abrir o Funil separado.
                  const canAgendarRow = l.status !== "vendido" && l.status !== "vendido_pendente" && l.status !== "cancelado";
                  const canResolveRow = canAgendarRow && l.status !== "perdido";
                  return (
                    <tr
                      key={l.id}
                    ...
                    >
                      {canManage && multiEmp && (
                        <td className="py-2.5 pr-2" onClick={(e) => e.stopPropagation()}>
                          <input type="checkbox" checked={!!selectedIds[l.id]} onChange={() => toggleOne(l.id)} />
                        </td>
                      )}
                      <td className="py-2.5 pr-3">
                        <p className="font-medium text-navy whitespace-nowrap">{l.nome_completo}</p>
                        <p className="text-[11px] text-muted whitespace-nowrap flex items-center gap-1"><Phone size={10} /> {l.telefone}{l.lista_id && listaNameById[l.lista_id] ? ` · ${listaNameById[l.lista_id]}` : ""}</p>
                      </td>
                      {multiEmp && <td className="py-2.5 pr-3 text-navy whitespace-nowrap">{empNameById[l.employee_id] || "—"}</td>}
                      <td className="py-2.5 pr-3 whitespace-nowrap">
                        <span className={`badge !text-[10px] ${STATUS_CHIP[l.status] || STATUS_CHIP.novo}`}>{STATUS_LABEL[l.status] || l.status}</span>
                      </td>
                      <td className="py-2.5 pr-3 text-muted whitespace-nowrap">{l.data_ligacao ? fmtLongDate(l.data_ligacao) : "—"}</td>
                      {canManage && (
                        <td className="py-2.5 whitespace-nowrap" onClick={(e) => e.stopPropagation()}>
                          {(canAgendarRow || canResolveRow) && (
                            <div className="flex items-center gap-1">
                              {canAgendarRow && (
                                <button type="button" onClick={() => openAgendar(l)} title="Agendar / reagendar" aria-label="Agendar" className="p-1.5 rounded-lg text-muted hover:text-blue hover:bg-paper transition-colors">
                                  <CalendarPlus size={14} />
                                </button>
                              )}
                              {canResolveRow && l.status === "novo" && (
                                <button type="button" onClick={() => openResolve(l, "sem_contato")} title="Não atendido" aria-label="Não atendido" className="p-1.5 rounded-lg text-muted hover:text-[#475569] hover:bg-paper transition-colors">
                                  <PhoneOff size={14} />
                                </button>
                              )}
                              {canResolveRow && (
                                <>
                                  <button type="button" onClick={() => openResolve(l, "em_negociacao")} title="Em negociação" aria-label="Em negociação" className="p-1.5 rounded-lg text-muted hover:text-teal hover:bg-paper transition-colors">
                                    <Handshake size={14} />
                                  </button>
                                  <button type="button" onClick={() => openResolve(l, "perdido")} title="Perdido" aria-label="Perdido" className="p-1.5 rounded-lg text-muted hover:text-danger hover:bg-paper transition-colors">
                                    <XCircle size={14} />
                                  </button>
                                  <button type="button" onClick={() => openResolve(l, "follow_up")} title="Follow-up" aria-label="Follow-up" className="p-1.5 rounded-lg text-muted hover:text-warn hover:bg-paper transition-colors">
                                    <Clock size={14} />
                                  </button>
                                  <button type="button" onClick={() => openResolve(l, "vendido")} title="Vendido" aria-label="Vendido" className="p-1.5 rounded-lg text-muted hover:text-success hover:bg-paper transition-colors">
                                    <CheckCircle2 size={14} />
                                  </button>
                                </>
                              )}
      ...
      <ConfirmModal
        ...
        message={`Esses leads passam a pertencer a ${empNameById[targetEmployee] || employees.find((e) => e.id === targetEmployee)?.full_name || "outro colaborador"}. O histórico (ligações, agendamentos, feedback) continua intacto — só o dono do lead muda.`}
          ...
          <div className="card max-w-sm w-full animate-bounce-in border-gold/30">
            <h2 className="text-lg font-extrabold text-navy flex items-center gap-2">
              {resolveModal.type === "perdido" ? <XCircle className="text-danger" size={20} />
                : resolveModal.type === "sem_contato" ? <PhoneOff className="text-muted" size={20} />
                : <Clock className="text-warn" size={20} />}
              Marcar como {STATUS_LABEL[resolveModal.type]?.toLowerCase() || resolveModal.type}
            </h2>
            <p className="text-xs text-muted mt-1">{resolveModal.lead.nome_completo}</p>
            {resolveModal.type === "perdido" && resolveModal.lead.status === "novo" && (
              <div className="mt-4 rounded-xl bg-paper border border-line p-3">
                ...
                <div className="space-y-2">
                  <label className="flex items-center gap-2 text-xs text-navy cursor-pointer">
                    <input type="radio" name="leadsTabContatoFeito" checked={!resolveContatoFeito} onChange={() => setResolveContatoFeito(false)} />
                    Não, nunca consegui contato
                  </label>
                  <label className="flex items-center gap-2 text-xs text-navy cursor-pointer">
                    <input type="radio" name="leadsTabContatoFeito" checked={resolveContatoFeito} onChange={() => setResolveContatoFeito(true)} />
                    Sim, falei mas não teve interesse
                  </label>
                </div>
              </div>
      ...
      <LeadHistoryPanel lead={detailLead} onClose={() => setDetailLead(null)} canManage={canManage} produtoCategorias={produtoCategorias} onChanged={load} />
    </div>
  );
}
