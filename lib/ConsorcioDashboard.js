    ...
    <div className="space-y-6">
      ...
      <div className="card-dark">
        <div className="flex items-center justify-between gap-2 mb-3">
          <p className="label-dark mb-0 flex items-center gap-1.5"><Trophy size={14} className="text-goldlight" /> Funil por colaborador{filtroAtivo && " (filtrado)"}</p>
          {canExport && (
            <button
              type="button"
              onClick={exportExcel}
        ...
        {filteredBoard.length === 0 ? (
          ...
          <ul>
            {filteredBoard.map((b) => (
              <li key={b.employee.id} className="row-card items-start flex-wrap gap-y-2">
                <Avatar name={b.employee.full_name} size={32} />
                <div className="min-w-0 flex-1">
                  <p className="font-medium text-white text-xs sm:text-sm truncate">{b.employee.full_name}</p>
                  <div className="flex flex-wrap items-center gap-1.5 mt-1.5">
                    <span className="text-[10px] font-bold px-2.5 py-1 rounded-full bg-white/10 text-white/70 shrink-0">{b.total} ligações</span>
                    <span
                      className="text-[10px] font-bold px-2.5 py-1 rounded-full bg-white/10 text-white/70 shrink-0"
                      title={`${b.agendadosVendidos} venderam · ${b.agendadosPerdidos} perdidos`}
                    >
                      {b.agendados} agendados
                    </span>
                    {b.followUp > 0 && <span className="chip-warn">{b.followUp} follow-up</span>}
                    {b.perdidos > 0 && <span className="chip-danger">{b.perdidos} perdidos</span>}
                    {b.vendidosCount > 0 && <span className="chip-ok">{b.vendidosCount} vendidos</span>}
                  </div>
                </div>
                <p className="font-bold text-goldlight text-xs sm:text-sm shrink-0 whitespace-nowrap">{formatBRL(b.valorVendido)}</p>
              </li>
            ))}
          </ul>
        )}
