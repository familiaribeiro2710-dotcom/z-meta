...
function Funil({ leads, leadsThisMonth, funilBoard, employees, viewerRole, month, produtoCategorias, empresaId, canManageCategorias, onChanged }) {
  ...
  // Filtro de colaborador — select dentro do próprio card de Filtros (não chip separado), aplica
  // na hora (não passa pelo rascunho/"Aplicar" dos outros campos, é uma escolha única e direta).
        ...
        {filtrosOpen && (
          <form onSubmit={aplicarFiltros} className="mt-3 space-y-3">
            ...
            <div>
              ...
              <SelectField className="w-full" value={filtroColaboradorId} onChange={(e) => { setFiltroColaboradorId(e.target.value); setPage(1); }}>
