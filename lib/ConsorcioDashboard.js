...
function Funil({ leads, leadsThisMonth, funilBoard, employees, viewerRole, month, produtoCategorias, empresaId, canManageCategorias, onChanged }) {
  ...
  // Filtro de colaborador (via chip clicável, não um select) + busca livre por nome/telefone —
  // pedido do Felipe: "o usuário tem que conseguir filtrar os leads e todas as informações dele",
  // hoje só existia o agregado por colaborador, sem forma de abrir o lead individual na tela.
  // Os chips vivem dentro do próprio card de Filtros (junto dos outros filtros), não numa seção
  // à parte — outro pedido do Felipe, pra tudo que filtra a lista de leads ficar num só lugar.
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="mt-3 space-y-3">
            <div>
              ...
              <div className="relative">
                ...
                <input className="input !pl-10" placeholder="ex: Carlos, (11) 9…" value={busca} onChange={(e) => setBusca(e.target.value)} />
              </div>
            </div>
            ...
            <div className="card-dark !p-4">
              <div className="flex items-center justify-between gap-2 mb-3">
                <p className="label-dark mb-0 flex items-center gap-1.5"><Users size={14} className="text-goldlight" /> Colaborador</p>
                {canExport && (
                  <button
                    type="button"
                    onClick={exportExcel}
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  {filteredBoard.map((b) => (
                    <button
                      type="button"
                      key={b.employee.id}
                      onClick={() => setFiltroColaboradorId((cur) => (cur === b.employee.id ? "" : b.employee.id))}
                      className={`flex items-center gap-2 rounded-full pl-1 pr-3 py-1 border transition-colors ${
      ...
      <div className="card-dark">
        <p className="label-dark mb-3 flex items-center gap-1.5"><PhoneCall size={14} className="text-goldlight" /> Leads{filtroAtivo ? " (filtrado)" : ""}</p>
