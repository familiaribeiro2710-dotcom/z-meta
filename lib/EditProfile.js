...
export default function EditProfile({ userId, currentName, currentUsername, currentAvatarUrl, onNameChange, onAvatarChange, onClose }) {
  ...
  return (
    <>
      ...
      <div className="absolute right-0 top-full mt-2 z-40 w-80 max-w-[calc(100vw-1.5rem)] max-h-[80vh] overflow-y-auto card animate-pop border-gold/20">
        ...
        <div className="flex items-center gap-3 mb-3 pb-3 border-b border-line">
          <div
            className="relative w-14 h-14 rounded-full shrink-0 overflow-hidden cursor-pointer group border-2 border-gold/20"
          ...
          <div>
            ...
            <button
              type="button"
              onClick={() => fileInputRef.current?.click()}
              className="text-[11px] uppercase tracking-wider font-bold text-gold hover:text-goldlight transition-colors"
            >
              Clique pra alterar
            </button>
            {avatarMsg && (
