...
// já está em maxDate — não dá pra navegar além dele (nem sempre é hoje: pode ser o
// último dia de um mês fechado sendo revisado, por exemplo).
...
// `todayDate`: dia de hoje de verdade, só pro rótulo "· hoje" — se omitido, cai no
// comportamento antigo de comparar contra maxDate (mantém quem já usa sem essa prop).
export default function DateNav({ date, onChange, maxDate, dark = false, todayDate }) {
  const realToday = todayDate ?? maxDate;
  const isToday = realToday && date === realToday;
