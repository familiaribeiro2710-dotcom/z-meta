...
export default function ConfirmModal({ open, title, message, confirmLabel = "Confirmar", danger = false, confirmText, onConfirm, onCancel }) {
  ...
  // Case/espaço-insensível de propósito — nome de loja/empresa pode ter maiúsculas específicas
  // (ex.: "ArmyBR"), e forçar o case exato é frágil (foi exatamente o que gerou o bug do botão
  // não desbloquear, antes do fix acima no rótulo).
  const canConfirm = !confirmText || typed.trim().toLowerCase() === confirmText.trim().toLowerCase();
  ...
  return createPortal(
    <div className="fixed inset-0 z-50 overflow-y-auto bg-navy/70 p-4 sm:p-6 pt-[calc(env(safe-area-inset-top)+1rem)]" onClick={handleCancel}>
      <div className="min-h-full flex items-start sm:items-center justify-center">
        <div
          className={`card max-w-sm w-full my-8 sm:my-0 animate-bounce-in ${danger ? "border-danger/30" : "border-gold/30"}`}
        ...
        >
          ...
          {confirmText && (
            <div className="mt-3">
              {/* .label é uppercase por CSS — sem o `normal-case` aqui, o nome exigido aparecia
                  TODO EM CAIXA ALTA na tela, mas a comparação embaixo é case-sensitive contra o
                  nome de verdade (ex.: "ArmyBR Iburapuera"). Resultado: o usuário digitava
                  exatamente o que via na tela (maiúsculo) e o botão nunca desbloqueava. */}
              <label className="label">
                Digite <span className="normal-case">&quot;{confirmText}&quot;</span> para confirmar
              </label>
