  ...
  return (
    <div className="space-y-6">
      ...
      <div className="card">
        ...
        <p className="text-[11px] text-muted mb-3">Só o colaborador marca as tarefas — aqui é só acompanhamento.</p>
        <ul className="divide-y divide-line">
          {myActiveTasks.map((t) => {
            const done = !!dayCompletions[t.id]?.completed;
            return (
              <li key={t.id} className="flex items-center gap-3 py-2.5 text-sm">
                <span className={`w-6 h-6 rounded-lg flex items-center justify-center shrink-0 font-bold text-white ${done ? "" : "bg-line text-muted"}`} style={done ? { background: "linear-gradient(135deg, #84cc16, #0d9488)" } : undefined}>
