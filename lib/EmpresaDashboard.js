...
export default function EmpresaDashboard({ lojaId, empresaId, viewerRole = "master_admin", tab = "atividades", month: monthProp }) {
    ...
    <div className="space-y-6">
      {tab === "atividades" && (
        <div className="space-y-6">
          ...
          {atSub === "colaboradores" && (
            <Colaboradores employees={employees} empresaId={empresaId} lojaId={lojaId} onChanged={refreshTeam} />
          )}
          {atSub === "tarefas" && (
          ...
          {atSub === "premiacoes" && (
            <Premiacoes employees={employees} prizes={prizes} month={month} empresaId={empresaId} lojaId={lojaId} onChanged={refresh} />
          )}
        </div>
      )}
...
function Colaboradores({ employees, empresaId, lojaId, onChanged }) {
  ...
  async function handleCreate(e) {
    ...
    const res = await fetch("/api/admin/create-employee", {
      ...
      body: JSON.stringify({ fullName, username: username.trim() || undefined, password, empresaId, lojaId }),
    });
  ...
  return (
    <div className="space-y-6">
      <div className="card">
        ...
        <p className="text-[11px] text-muted mt-2">Se o usuário não for informado, ele é gerado automaticamente a partir do nome. O colaborador poderá trocar a senha no primeiro acesso.</p>
        {msg && (
...
function Premiacoes({ employees, prizes, month, empresaId, lojaId, onChanged }) {
  ...
  useEffect(() => {
    ...
    activeEmps.forEach((emp) => {
      const p = prizes.find((pr) => pr.employee_id === emp.id);
      v[emp.id] = p ? String(p.amount) : "";
    });
    setVals(v);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [prizes, employees]);
  ...
  async function saveAll(e) {
    ...
    if (rows.length) {
      await supabase.from("employee_prizes").upsert(rows, { onConflict: "employee_id,month" });
    }
    setSaving(false);
    onChanged();
  }
  ...
  return (
    <div className="card">
      <p className="label mb-2 flex items-center gap-1.5"><Gift size={14} /> Premiações — {monthLabel(month)}</p>
      <p className="text-xs text-muted mb-4">O valor lançado aqui aparece no herocard de cada colaborador (e somado no seu, aqui no dashboard do gerente).</p>
      <form onSubmit={saveAll} className="space-y-3">
        ...
        {activeEmps.length === 0 ? (
          ...
          <button className="btn" type="submit" disabled={saving}>{saving ? "Salvando…" : "Salvar premiações"}</button>
        )}
      </form>
    </div>
  );
