...
export default function EmpresaDashboard({ lojaId, empresaId, viewerRole = "master_admin" }) {
      ...
      <div className="flex gap-2 flex-wrap">
        {TABS.map((t) => (
          <button
            ...
            className={`flex items-center gap-1.5 text-sm font-bold px-4 py-2 rounded-full border-2 transition-all ${
              tab === t.key ? "text-white border-transparent shadow-pop scale-105" : "border-line text-muted hover:border-purple hover:text-purple"
            }`}
            style={tab === t.key ? { background: "linear-gradient(135deg, #7c3aed, #ec4899)" } : undefined}
          >
...
function Metas({ employees, goals, allocations, month, empresaId, lojaId, onChanged, viewerRole = "master_admin" }) {
  const canCreateGoal = viewerRole !== "gerente";
  ...
  async function createGoal(e) {
    ...
    setName(""); setTotal(""); setCommissionPct("");
    onChanged();
  }
  ...
  function openEditor(goal, goalAllocs) {
    ...
    employees.filter((e) => e.active).forEach((emp) => {
      ...
      existingComm[emp.id] = a ? String(a.commission_pct ?? goal.commission_pct ?? 0) : String(goal.commission_pct ?? 0);
    });
    setCustomVals(existing);
    setCustomCommission(existingComm);
  ...
  async function applyEqual(goal) {
    ...
    const rows = activeEmps.map((emp) => ({
      ...
      commission_pct: Number(goal.commission_pct) || 0,
      empresa_id: empresaId,
      loja_id: lojaId,
    }));
  ...
  async function saveCustom(goalId) {
    const rows = Object.entries(customVals)
      ...
      .map(([employee_id, v]) => ({
        ...
        employee_id,
        amount: Number(v),
        commission_pct: Number(customCommission[employee_id]) || 0,
        empresa_id: empresaId,
        loja_id: lojaId,
      }));
    if (rows.length) {
        ...
        <div className="card">
          ...
          <form onSubmit={createGoal} className="grid sm:grid-cols-4 gap-3 items-end">
            ...
            <div>
              ...
              <input type="number" step="0.1" min="0" className="input" placeholder="0" value={commissionPct} onChange={(e) => setCommissionPct(e.target.value)} />
            </div>
            <div>
              <button className="btn w-full" type="submit">Criar meta</button>
            </div>
          </form>
          <p className="text-[11px] text-muted mt-2">O gerente da loja é quem define como essa meta é dividida entre os colaboradores.</p>
        </div>
      ) : (
        <div className="card">
          <p className="text-sm text-navy flex items-center gap-1.5"><Target size={14} className="text-purple" /> Metas definidas pelo sócio/supervisor.</p>
          <p className="text-xs text-muted mt-1">Você organiza como cada meta é dividida entre os colaboradores da loja.</p>
        </div>
      )}
          ...
          return (
            <div key={g.id} className="card">
              <div className="flex justify-between items-start flex-wrap gap-2">
                <div>
                  <p className="font-semibold text-sm text-navy flex items-center gap-1.5"><Target size={14} className="text-purple" /> {g.name}</p>
                  <p className="text-xs text-muted">
                    {formatBRL(g.store_total)}
                    {Number(g.commission_pct) > 0 && ` · ${Number(g.commission_pct)}% de comissão`}
              ...
              {isEditing ? (
                <div className="mt-4 space-y-3">
                  ...
                  {distMode === "equal" ? (
                    <div>
                      <p className="text-xs text-muted">
                        {employees.filter((e) => e.active).length} colaborador(es) ativo(s) · {formatBRL(employees.filter((e) => e.active).length ? Number(g.store_total) / employees.filter((e) => e.active).length : 0)} cada, {Number(g.commission_pct) || 0}% de comissão.
                      </p>
                        ...
                        <div key={emp.id} className="flex items-center gap-3">
                          ...
                          <div className="w-24">
                            <input
                              ...
                              className="input"
                              placeholder="% com."
                              value={customCommission[emp.id] ?? ""}
                              onChange={(e) => setCustomCommission((c) => ({ ...c, [emp.id]: e.target.value }))}
                            />
                          </div>
                        </div>
                      ))}
                  ...
                  {goalAllocs.map((a) => {
                    ...
                    return (
                      <li key={a.id} className="flex justify-between py-1.5">
                        ...
                        <span>{formatBRL(a.amount)} <span className="text-muted">· {Number(a.commission_pct || 0)}% com.</span></span>
                      </li>
                    );
                  })}
