...
function Placar({ scoreboard, teamPct, settings, month, viewerRole = "master_admin", goals = [], storeMetaTotal = 0, storeSoldTotal = 0, entries = [] }) {
    ...
    <div className="space-y-6">
      {(viewerRole === "gerente" || isSupervisorView) && (
        <div className="card-dark animate-pop">
          ...
          <p className="text-xs text-white/50 mt-1">Soma das vendas de todos os colaboradores da loja nesse dia.</p>
        </div>
      )}
              ...
              <li key={g.id} className="row-card justify-between flex-wrap">
                {editingGerenteId === g.id ? (
                  <div className="flex items-center gap-2 flex-1 flex-wrap">
                    ...
                    <button onClick={() => saveEditGerente(g)} disabled={savingGerenteEdit} title="Salvar" aria-label="Salvar" className="p-1.5 rounded-xl text-success hover:bg-success/15 transition-colors disabled:opacity-40">
                      <Check size={16} strokeWidth={2.5} />
                    </button>
                    <button onClick={() => { setEditingGerenteId(null); setEditGerenteMsg(""); }} title="Cancelar" aria-label="Cancelar" className="p-1.5 rounded-xl text-white/60 hover:text-white hover:bg-white/10 transition-colors">
                      <X size={16} />
                    </button>
                ...
                {editingGerenteId !== g.id && (
                  <div className="flex items-center gap-1 shrink-0">
                    <button onClick={() => startEditGerente(g)} title="Editar" aria-label="Editar" className="p-1.5 rounded-xl text-white/60 hover:text-purplelight hover:bg-white/10 transition-colors">
                      <Pencil size={14} />
                    </button>
                    <button
                      ...
                      className="p-1.5 rounded-xl text-white/60 hover:text-warn hover:bg-white/10 transition-colors"
                    >
                      <KeyRound size={14} />
                    </button>
                    <button
                      ...
                      className={`p-1.5 rounded-xl hover:bg-white/10 transition-colors ${g.active ? "text-white/60 hover:text-white" : "text-danger"}`}
                    >
                      <Power size={14} />
                    </button>
                    <button onClick={() => requestDelete(g)} title="Excluir" aria-label="Excluir" className="p-1.5 rounded-xl text-white/60 hover:text-danger hover:bg-white/10 transition-colors">
                      <Trash2 size={14} />
                    </button>
                  </div>
                )}
            ...
            <li key={emp.id} className="row-card justify-between flex-wrap">
              {editingId === emp.id ? (
                <div className="flex items-center gap-2 flex-1 flex-wrap">
                  ...
                  <button onClick={() => saveEdit(emp)} disabled={savingEdit} title="Salvar" aria-label="Salvar" className="p-1.5 rounded-xl text-success hover:bg-success/15 transition-colors disabled:opacity-40">
                    <Check size={16} strokeWidth={2.5} />
                  </button>
                  <button onClick={() => { setEditingId(null); setEditMsg(""); }} title="Cancelar" aria-label="Cancelar" className="p-1.5 rounded-xl text-white/60 hover:text-white hover:bg-white/10 transition-colors">
                    <X size={16} />
                  </button>
              ...
              {editingId !== emp.id && canEdit && (
                <div className="flex items-center gap-1 shrink-0">
                  <button onClick={() => startEdit(emp)} title="Editar" aria-label="Editar" className="p-1.5 rounded-xl text-white/60 hover:text-purplelight hover:bg-white/10 transition-colors">
                    <Pencil size={14} />
                  </button>
                  <button
                  ...
                  <button
                    ...
                    className={`p-1.5 rounded-xl hover:bg-white/10 transition-colors ${emp.active ? "text-white/60 hover:text-white" : "text-danger"}`}
                  >
                    <Power size={14} />
                  </button>
                  <button onClick={() => requestDelete(emp)} title="Excluir" aria-label="Excluir" className="p-1.5 rounded-xl text-white/60 hover:text-danger hover:bg-white/10 transition-colors">
                    <Trash2 size={14} />
                  </button>
                </div>
              )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-navy/70 p-6" onClick={() => setSupportBlockOpen(false)}>
          <div className="card max-w-sm w-full animate-bounce-in" onClick={(e) => e.stopPropagation()}>
            <h2 className="text-lg font-extrabold text-navy flex items-center gap-2">
              <ShieldCheck className="text-purple shrink-0" size={20} /> Exclusão só pelo Master Admin
            </h2>
              ...
              <div className="flex gap-2 flex-wrap">
                <button
                  ...
                  className={`text-xs font-bold px-3 py-1.5 rounded-full border-2 transition-all ${
                    taskType === "checklist" ? "border-purple text-purple" : "border-line text-muted hover:border-purple/50"
                  }`}
                >
                  Checklist manual
                </button>
              ...
              {editingTaskId === t.id ? (
                <div className="flex items-center gap-2 flex-1 flex-wrap">
                  <input
                    ...
                    autoFocus
                  />
                  <button onClick={() => saveTaskEdit(t)} disabled={savingTaskEdit} title="Salvar" aria-label="Salvar" className="p-1.5 rounded-xl text-success hover:bg-success/10 transition-colors disabled:opacity-40">
                    <Check size={16} strokeWidth={2.5} />
                  </button>
                  <button onClick={() => setEditingTaskId(null)} title="Cancelar" aria-label="Cancelar" className="p-1.5 rounded-xl text-muted hover:text-navy hover:bg-line/60 transition-colors">
                    <X size={16} />
                  </button>
                </div>
              ) : (
                <>
                  ...
                  <div className="flex gap-1 shrink-0">
                    <button onClick={() => startEditTask(t)} title="Editar tarefa" aria-label="Editar tarefa" className="p-1.5 rounded-xl text-muted hover:text-navy hover:bg-line/60 transition-colors">
                      <Pencil size={14} />
                    </button>
                    <button onClick={() => setConfirmDeleteTask(t)} title="Excluir tarefa" aria-label="Excluir tarefa" className="p-1.5 rounded-xl text-muted hover:text-danger hover:bg-line/60 transition-colors">
                      <Trash2 size={14} />
                    </button>
                  </div>
                  )}
          ...
          {dueTasks.map((t) => {
            ...
            return (
              <li key={t.id} className="flex items-center gap-3 py-2.5 text-sm">
                ...
                <span
                  className={`w-6 h-6 rounded-xl flex items-center justify-center shrink-0 font-bold text-white transition-all ${done ? "" : "bg-line text-muted"}`}
                  style={done ? { background: "linear-gradient(135deg, #84cc16, #0d9488)" } : undefined}
                >
                  {done ? <Check size={13} strokeWidth={3} /> : <X size={13} />}
                </span>
...
export function Premiacoes({ employees, gerentes = [], viewerRole = "master_admin", prizes, month, empresaId, lojaId, settings, onChanged, goals = [], soldTotal = 0, onSaveExtraPrize }) {
  ...
  async function addPrize(e) {
    ...
    const qtdLancamentos = replicateAll ? activeEmps.length : 1;
      ...
      {canEdit && goals.length > 0 && (
        <div className="card">
          ...
          {activeExtraPrizeGoal && (
            <div className="rounded-xl py-2 px-3 bg-pink/10 flex items-center gap-2 mb-4">
      ...
      <div className="card">
        <div className="flex items-center justify-between gap-2 mb-3 flex-wrap">
          ...
          <span className="text-right shrink-0">
            <span className="block text-sm font-bold text-purple whitespace-nowrap">Total: {formatBRL(totalLancado)}</span>
                ...
                {empPrizes.map((p) => (
                  <li key={p.id} className="flex items-center justify-between gap-2 py-1.5 text-xs sm:text-sm">
                    ...
                    <span className="flex items-center gap-3 shrink-0">
                      ...
                      {canEdit && (
                      <button onClick={() => setConfirmDeletePrize(p)} title="Excluir" aria-label="Excluir" className="p-1 rounded-xl text-muted hover:text-danger hover:bg-line/60 transition-colors">
                        <Trash2 size={13} />
                      </button>
                      )}
                    </span>
          ...
          return (
            <div key={g.id} className="card">
              <div className="flex justify-between items-start flex-wrap gap-2">
                <div>
                  <p className="font-semibold text-xs sm:text-sm text-navy flex items-center gap-1.5 flex-wrap"><Target size={14} className={GOAL_ICON_COLORS[idx % GOAL_ICON_COLORS.length]} /> {g.name} <span className="badge bg-line text-muted">Meta {idx + 1}</span>{g.pacing && g.pacing !== "dia" && <span className="badge bg-purple/15 text-purple">ritmo: {PACING_LABELS[g.pacing]?.toLowerCase()}</span>}</p>
                ...
                {canManage && !isEditing && editingValuesId !== g.id && (
                  <div className="flex items-center gap-1 shrink-0">
                    <button
                      className="p-2 rounded-xl text-muted hover:text-navy hover:bg-line/60 transition-colors"
                ...
                {editingGoal?.id === g.id ? (
                  ...
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <div className="flex items-center gap-1 shrink-0">
                      ...
                      {canEdit && (
                        <>
                          <button type="button" title="Editar" aria-label="Editar" className="p-1.5 rounded-xl text-muted hover:text-navy hover:bg-line/60 transition-colors" onClick={() => openEditGoal(g)}>
                            <Pencil size={14} />
                          </button>
                          <button type="button" title="Excluir" aria-label="Excluir" className="p-1.5 rounded-xl text-muted hover:text-danger hover:bg-line/60 transition-colors" onClick={() => setConfirmDeleteGoal(g)}>
                            <Trash2 size={14} />
                          </button>
                        </>
                      )}
...
function Lancamentos({ employees, entries, month, empresaId, lojaId, onChanged, viewerRole = "master_admin" }) {
  ...
  // onde é o gerente escolhendo por qual colaborador lançar. E a pergunta de folga (que marca as
  // tarefas do dia como concluídas) só faz sentido pra um dia que já passou — hoje ainda está em
  // andamento, então R$0 hoje só salva normal, sem perguntar/mexer no checklist.
  async function save(e) {
    e.preventDefault();
    if (!selected || value === "") return;
    if (Number(value) === 0 && date !== today) {
  ...
  async function doSave(isFolga) {
    ...
    if (isFolga && date !== today) {
      ...
      // Ativação Online (contagem real, gerente manual, ou folga confirmada num dia passado).
              ...
              editingEntryId === en.id ? (
                <li key={en.id} className="row-card justify-between gap-2 flex-wrap">
                  ...
                  <div className="flex items-center gap-2 flex-1 min-w-[160px]">
                    ...
                    <button type="button" title="Cancelar" aria-label="Cancelar" className="p-1.5 rounded-xl text-white/50 hover:text-white transition-colors shrink-0" onClick={() => setEditingEntryId(null)}>
                      <X size={16} />
                    </button>
                  </div>
                </li>
                ...
                <li key={en.id} className="row-card justify-between">
                  ...
                  <span className="flex items-center gap-2 shrink-0">
                    ...
                    {canEdit && (
                      <>
                        <button type="button" title="Editar" aria-label="Editar" className="p-1.5 rounded-xl text-white/50 hover:text-white hover:bg-white/10 transition-colors" onClick={() => startEdit(en)}>
                          <Pencil size={13} />
                        </button>
                        <button type="button" title="Excluir" aria-label="Excluir" className="p-1.5 rounded-xl text-white/50 hover:text-danger hover:bg-white/10 transition-colors" onClick={() => setConfirmDeleteEntry(en)}>
                          <Trash2 size={13} />
                        </button>
                      </>
                    )}
