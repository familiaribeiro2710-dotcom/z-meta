...
function Placar({ scoreboard, settings, month, viewerRole = "master_admin", goals = [], storeMetaTotal = 0, storeSoldTotal = 0, entries = [], prizes = [], empresaId, lojaId, onChanged }) {
  ...
  // settings.team_threshold_pct) deixa de ser só informativo — o gestor agora valida (cria a
  // premiação de verdade, valor congelado no momento do clique) ou descarta (remove) por
  // colaborador/mês. `origin: "atividades_mes"` distingue essa premiação automática de qualquer
  // outra lançada manualmente na aba Premiações — uma vez criada, é uma linha normal de
  // employee_prizes, então já entra sozinha em relatório/verba/notificação do colaborador.
  ...
  function atividadesPrizeFor(employeeId) {
    return prizes.find((p) => p.employee_id === employeeId && p.origin === "atividades_mes") || null;
  }
  ...
  async function runPrizeAction() {
    ...
    if (type === "validate") {
      ...
      await supabase.from("employee_prizes").insert({
        ...
        month,
        amount: Number(settings.monthly_prize) || 0,
        description: "Prêmio do mês — atividades",
        origin: "atividades_mes",
        empresa_id: empresaId,
        loja_id: lojaId,
      ...
      const existing = atividadesPrizeFor(employee.id);
      if (existing) await supabase.from("employee_prizes").delete().eq("id", existing.id);
    }
    setSavingPrizeAction(false);
    setConfirmPrizeAction(null);
    notifySaved(type === "validate" ? "Premiação validada com sucesso." : "Premiação descartada com sucesso.");
    onChanged && onChanged();
  }
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="card animate-pop">
          ...
          <ul className="divide-y divide-line">
            {scoreboard.map((b) => {
              ...
              const validated = atividadesPrizeFor(b.employee.id);
              return (
                  ...
                  {canValidatePrize && (
                    <div className="flex items-center justify-between gap-2 mt-2">
                      <span className={`text-[11px] font-bold flex items-center gap-1 ${validated ? "text-gold" : "text-muted"}`}>
                        {validated ? <><Gift size={12} className="shrink-0" /> Validada — {formatBRL(validated.amount)}</> : "Ainda não validada"}
                      </span>
                      <button
                        type="button"
                        disabled={savingPrizeAction}
                        onClick={() => setConfirmPrizeAction({ type: validated ? "discard" : "validate", employee: b.employee })}
                        className={`shrink-0 text-[11px] font-bold px-2.5 py-1 rounded-full border transition disabled:opacity-60 ${
                          validated ? "border-danger/30 text-danger hover:bg-danger/10" : "border-gold/40 text-gold hover:bg-gold/10"
                        }`}
                      >
                        {validated ? "Descartar" : "Validar premiação"}
                      </button>
                    </div>
                  )}
                </li>
      ...
      <ConfirmModal
        ...
        title={confirmPrizeAction?.type === "validate" ? "Validar premiação do mês?" : "Descartar premiação do mês?"}
        message={
          confirmPrizeAction
            ? confirmPrizeAction.type === "validate"
              ? `${confirmPrizeAction.employee.full_name} vai receber uma premiação de ${formatBRL(settings.monthly_prize)} referente a ${monthLabel(month)}.`
              : `A premiação do mês de ${confirmPrizeAction.employee.full_name} (${monthLabel(month)}) será removida — essa ação não pode ser desfeita.`
            : ""
        }
