...
export default function EmpresaDashboard({ lojaId, empresaId, viewerRole = "master_admin", viewerId, tab = "atividades", month: monthProp, onOpenEmployee, onOpenGerente }) {
  ...
  const storeMetaTotal = goals.reduce((s, g) => s + Number(g.store_total || 0), 0);
...
function Placar({ scoreboard, teamPct, settings, month, viewerRole = "master_admin", storeMetaTotal = 0, storeSoldTotal = 0, onSaveSettings }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="card overflow-x-auto">
        <p className="label mb-3">Placar individual — ranqueado por valor vendido</p>
          ...
          return (
            <div key={g.id} className="card">
              <div className="flex justify-between items-start flex-wrap gap-2">
                <div>
                  <p className="font-semibold text-sm text-navy flex items-center gap-1.5"><Target size={14} className={GOAL_ICON_COLORS[idx % GOAL_ICON_COLORS.length]} /> {g.name} <span className="badge bg-line text-muted">Meta {idx + 1}</span></p>
