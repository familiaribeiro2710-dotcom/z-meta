...
import { todayStr, firstDayOfMonth, monthLabel, yesterdayStr, isTaskDueOn, WEEKDAY_LABELS, daysElapsedInMonth } from "./date";
...
function Placar({ scoreboard, teamPct, settings, month, viewerRole = "master_admin", goals = [], storeMetaTotal = 0, storeSoldTotal = 0 }) {
              ...
              <li key={g.id} className="flex items-center justify-between gap-3 py-2.5 text-sm flex-wrap">
                {editingGerenteId === g.id ? (
                  <div className="flex items-center gap-2 flex-1 flex-wrap">
                    ...
                    <button onClick={() => saveEditGerente(g)} disabled={savingGerenteEdit} title="Salvar" aria-label="Salvar" className="p-1.5 rounded-lg text-success hover:bg-success/10 transition-colors disabled:opacity-40">
                      <Check size={16} strokeWidth={2.5} />
                    </button>
                    <button onClick={() => { setEditingGerenteId(null); setEditGerenteMsg(""); }} title="Cancelar" aria-label="Cancelar" className="p-1.5 rounded-lg text-muted hover:text-navy hover:bg-line/60 transition-colors">
                      <X size={16} />
                    </button>
                ...
                {editingGerenteId !== g.id && (
                  <div className="flex items-center gap-1 shrink-0">
                    <button onClick={() => startEditGerente(g)} title="Editar" aria-label="Editar" className="p-1.5 rounded-lg text-muted hover:text-purple hover:bg-line/60 transition-colors">
                      <Pencil size={14} />
                    </button>
                    <button
                      ...
                      className="p-1.5 rounded-lg text-muted hover:text-warn hover:bg-line/60 transition-colors"
                    >
                      <KeyRound size={14} />
                    </button>
                    <button
                      ...
                      className={`p-1.5 rounded-lg hover:bg-line/60 transition-colors ${g.active ? "text-muted hover:text-navy" : "text-danger"}`}
                    >
                      <Power size={14} />
                    </button>
                    <button onClick={() => removeEmployee(g)} title="Excluir" aria-label="Excluir" className="p-1.5 rounded-lg text-muted hover:text-danger hover:bg-line/60 transition-colors">
                      <Trash2 size={14} />
                    </button>
                  </div>
                )}
            ...
            <li key={emp.id} className="flex items-center justify-between gap-3 py-2.5 text-sm flex-wrap">
              {editingId === emp.id ? (
                <div className="flex items-center gap-2 flex-1 flex-wrap">
                  ...
                  <button onClick={() => saveEdit(emp)} disabled={savingEdit} title="Salvar" aria-label="Salvar" className="p-1.5 rounded-lg text-success hover:bg-success/10 transition-colors disabled:opacity-40">
                    <Check size={16} strokeWidth={2.5} />
                  </button>
                  <button onClick={() => { setEditingId(null); setEditMsg(""); }} title="Cancelar" aria-label="Cancelar" className="p-1.5 rounded-lg text-muted hover:text-navy hover:bg-line/60 transition-colors">
                    <X size={16} />
                  </button>
              ...
              {editingId !== emp.id && canEdit && (
                <div className="flex items-center gap-1 shrink-0">
                  <button onClick={() => startEdit(emp)} title="Editar" aria-label="Editar" className="p-1.5 rounded-lg text-muted hover:text-purple hover:bg-line/60 transition-colors">
                    <Pencil size={14} />
                  </button>
                  <button
                  ...
                  <button
                    ...
                    className={`p-1.5 rounded-lg hover:bg-line/60 transition-colors ${emp.active ? "text-muted hover:text-navy" : "text-danger"}`}
                  >
                    <Power size={14} />
                  </button>
                  <button onClick={() => removeEmployee(emp)} title="Excluir" aria-label="Excluir" className="p-1.5 rounded-lg text-muted hover:text-danger hover:bg-line/60 transition-colors">
                    <Trash2 size={14} />
                  </button>
                </div>
              )}
...
export function Tarefas({ employees, gerentes = [], viewerRole = "master_admin", tasks, empresaId, lojaId, onChanged }) {
      ...
      <div className="card">
        ...
        {activeEmps.length === 0 && !canTargetGerentes ? (
          ...
          <form onSubmit={addPrize} className="grid sm:grid-cols-4 gap-3 items-end">
            <div>
              <label className="label">Colaborador{canTargetGerentes ? " ou gerente" : ""}</label>
              <SelectField className="w-full" value={empId} onChange={(e) => setEmpId(e.target.value)}>
                {canTargetGerentes ? (
                  <>
                    <optgroup label="Colaboradores">
                ...
                {empPrizes.map((p) => (
                  <li key={p.id} className="flex items-center justify-between gap-2 py-1.5 text-xs sm:text-sm">
                    ...
                    <span className="flex items-center gap-3 shrink-0">
                      ...
                      {canEdit && (
                      <button onClick={() => removePrize(p.id)} title="Excluir" aria-label="Excluir" className="p-1 rounded-lg text-muted hover:text-danger hover:bg-line/60 transition-colors">
                        <Trash2 size={13} />
                      </button>
                      )}
                    </span>
...
function Lancamentos({ employees, entries, empresaId, lojaId, onChanged, viewerRole = "master_admin" }) {
  ...
  // venda de um dia que ainda não aconteceu, então a data fica limitada a até ontem — mesmo aqui,
    ...
    <div className="space-y-6">
      ...
      <div className="card">
        ...
        <form onSubmit={save} className="grid sm:grid-cols-3 gap-3 items-end">
          <div>
            ...
            <input type="date" className="input date-input" value={date} max={yesterdayStr(todayStr())} onChange={(e) => setDate(e.target.value)} />
          </div>
          <div>
      ...
      <div className="card">
        ...
        <ul className="divide-y divide-line text-xs sm:text-sm">
          {myEntries.map((en) => (
            <li key={en.id} className="flex justify-between gap-2 py-2">
              <span className="text-muted truncate min-w-0">{en.entry_date}{en.edited_by_manager ? " (corrigido)" : ""}</span>
              <span className="shrink-0 whitespace-nowrap">{formatBRL(en.daily_amount)}</span>
            </li>
          ))}
