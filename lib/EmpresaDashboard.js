...
import { todayStr, firstDayOfMonth, monthLabel, yesterdayStr } from "./date";
...
function Tarefas({ employees, gerentes = [], viewerRole = "master_admin", tasks, empresaId, lojaId, onChanged }) {
  ...
  useEffect(() => {
    let active = true;
    (async () => {
      const taskIds = myActiveTasks.map((t) => t.id);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="card">
        ...
        <p className="text-[11px] text-muted mb-3">Você pode marcar ou corrigir tarefas de qualquer dia, inclusive dias anteriores.</p>
        <ul className="divide-y divide-line">
          {myActiveTasks.map((t) => {
          ...
          {myActiveTasks.length === 0 && <p className="text-sm text-muted py-2">Nenhuma tarefa ativa para este colaborador.</p>}
        </ul>
      </div>
    </div>
  );
