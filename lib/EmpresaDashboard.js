...
import { todayStr, firstDayOfMonth, nextMonth, monthLabel, isTaskDueOn, WEEKDAY_LABELS, daysElapsedInMonth, daysInMonth, remainingPeriodsInMonth, periodLabel, PACING_LABELS } from "./date";
        ...
        <div className="space-y-6">
          ...
          {atSub === "premiacoes" && (
            <Premiacoes
              ...
              onSaveExtraPrize={async (goalId, text) => {
                await supabase.from("sales_goals").update({ non_monetary_prize: text || null }).eq("id", goalId);
                await refresh();
              }}
            />
          )}
...
export function Premiacoes({ employees, gerentes = [], viewerRole = "master_admin", prizes, month, empresaId, lojaId, settings, onChanged, goals = [], soldTotal = 0, onSaveExtraPrize }) {
  ...
  // não batida entre as premiadas; se todas já foram batidas, a de maior valor entre elas). Pedido
  // do Felipe: quem gerencia precisa ver isso direto na aba Premiações, não só no herocard.
  const prizeGoalsAll = goals.filter((g) => g.non_monetary_prize);
  ...
  useEffect(() => {
    if (!extraGoalId && goals.length) setExtraGoalId(goals[0].id);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [goals]);
  ...
  async function saveExtraPrize(e) {
    ...
    setSavingExtra(true);
    await onSaveExtraPrize(extraGoalId, extraText.trim());
    ...
    <div className="space-y-6">
      ...
      {canEdit && goals.length > 0 && (
        <div className="card">
          ...
          <p className="text-xs text-muted mb-4">Um prêmio fora da comissão em dinheiro (festa, jantar, viagem…) vinculado a uma meta já cadastrada — libera pra toda a equipe da loja quando ela for batida.</p>
          {activeExtraPrizeGoal && (
            <div className="rounded-lg py-2 px-3 bg-pink/10 flex items-center gap-2 mb-4">
              ...
              <p className="text-xs font-bold text-navy">Ativa agora: {activeExtraPrizeGoal.non_monetary_prize} — vale se a loja bater a {activeExtraPrizeGoal.name}</p>
            </div>
          )}
