    ...
    <div className="space-y-6">
      {isSupervisorView ? (
        <div className="card animate-pop">
          <p className="label flex items-center gap-1.5"><Trophy size={14} /> Barra de vendas — vendido / meta — {monthLabel(month)}</p>
                ...
                <li key={g.id} className="py-2.5">
                  <div className="flex items-center justify-between gap-2 text-sm">
                    <span className="font-medium text-navy flex items-center gap-1.5">
                      {g.name}
                      {activeGoalId === g.id && <span className="badge bg-purple/15 text-purple">em jogo</span>}
                    </span>
      ...
      <div className="card sm:overflow-x-auto">
        ...
        {/* mobile: cada colaborador em um bloco, com a barra abaixo do nome — mais organizado
            que espremer uma tabela larga numa tela estreita */}
        <ul className="sm:hidden divide-y divide-line">
          {ranked.map((b) => (
            <li key={b.employee.id} className="py-3">
              <div className="flex items-center justify-between gap-2">
                <span className="font-medium text-navy text-sm truncate">{b.employee.full_name}</span>
                <span className="text-navy font-semibold text-sm shrink-0">{formatBRL(b.sold)}</span>
              </div>
              <div className="mt-2"><ProgressBar pct={b.pct} showLabel={false} height="h-2" /></div>
              <div className="flex items-center gap-3 mt-1.5 text-[11px] text-muted">
                <span>{b.completed}/{b.expected} tarefas</span>
                {b.warnings > 0 && (
                  <span className="flex items-center gap-1 text-warn"><AlertTriangle size={11} /> {b.warnings} advertência(s)</span>
                )}
              </div>
            </li>
          ))}
        ...
        <table className="hidden sm:table w-full text-sm">
          <thead>
            <tr className="text-left text-xs uppercase tracking-wider text-muted border-b border-line">
              ...
              <th className="pb-2">Vendido até ontem</th>
          ...
          <tbody>
            {ranked.map((b) => (
              <tr key={b.employee.id} className="border-b border-line last:border-0">
                ...
                <td className="py-2.5 text-navy font-medium">{formatBRL(b.sold)}</td>
...
function Colaboradores({ employees, gerentes = [], viewerRole = "master_admin", empresaId, lojaId, onChanged, onOpenEmployee, onOpenGerente }) {
  const canManageTeams = viewerRole !== "gerente"; // só supervisor/sócio/master admin escolhem loja/gerente e cadastram gerentes
  ...
  async function handleCreateGerente(e) {
    ...
    setGName(""); setGUsername(""); setGPassword(""); setGTeam([]);
    onChanged();
  }
        ...
        <div className="card">
          ...
          <form onSubmit={handleCreateGerente} className="grid sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input className="input" placeholder="gerado automaticamente se vazio" value={gUsername} onChange={(e) => setGUsername(e.target.value)} />
            </div>
            ...
            {employees.length > 0 && (
              <div className="sm:col-span-2">
                <label className="label">Colaboradores dessa loja pra já incluir na equipe (opcional)</label>
                <div className="flex flex-wrap gap-2 mt-1">
                  {employees.map((emp) => (
                    <label key={emp.id} className={`text-xs font-medium px-3 py-1.5 rounded-full border-2 cursor-pointer ${gTeam.includes(emp.id) ? "border-purple text-purple" : "border-line text-muted"}`}>
                      <input
                        type="checkbox"
                        className="hidden"
              ...
              <li key={g.id} className="flex items-center justify-between gap-3 py-2.5 text-sm flex-wrap">
                {editingGerenteId === g.id ? (
                  <div className="flex items-center gap-2 flex-1 flex-wrap">
                    <input className="input !py-1.5 !text-sm w-40" value={editGerenteName} onChange={(e) => setEditGerenteName(e.target.value)} placeholder="Nome completo" autoFocus />
                    <input className="input !py-1.5 !text-sm w-32" value={editGerenteUsername} onChange={(e) => setEditGerenteUsername(e.target.value)} placeholder="Usuário" />
        ...
        <ul className="divide-y divide-line">
          {employees.map((emp) => (
            <li key={emp.id} className="flex items-center justify-between gap-3 py-2.5 text-sm flex-wrap">
              {editingId === emp.id ? (
                <div className="flex items-center gap-2 flex-1 flex-wrap">
                  <input className="input !py-1.5 !text-sm w-40" value={editName} onChange={(e) => setEditName(e.target.value)} placeholder="Nome completo" autoFocus />
                  <input className="input !py-1.5 !text-sm w-32" value={editUsername} onChange={(e) => setEditUsername(e.target.value)} placeholder="Usuário" />
                  {canManageTeams && (
...
function Tarefas({ employees, gerentes = [], viewerRole = "master_admin", tasks, empresaId, lojaId, onChanged }) {
  ...
  // deveriam ter acontecido) e desativa a tarefa pra ela parar de valer daqui pra frente.
  async function removeTask(t) {
    if (!window.confirm(`Excluir "${t.title}"? A tarefa deixa de valer a partir de hoje. Dias já registrados (feitos ou perdidos) continuam no histórico e não são apagados.`)) return;
    await supabase
      .from("task_completions")
      .delete()
      .eq("task_id", t.id)
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="card">
        ...
        <form onSubmit={addTask} className="space-y-3 mb-4">
          ...
          <label className="flex items-center gap-2 text-xs text-muted font-medium">
            <input type="checkbox" checked={replicateAll} onChange={(e) => setReplicateAll(e.target.checked)} />
        ...
        <ul className="divide-y divide-line">
          {myTasks.map((t) => (
            <li key={t.id} className="flex items-center justify-between gap-2 flex-wrap py-2.5 text-sm">
              <span className={t.active ? "text-navy" : "text-muted line-through"}>
      ...
      <div className="card">
        ...
        <p className="text-[11px] text-muted mb-3">Você pode marcar ou corrigir tarefas de qualquer dia, inclusive dias anteriores. Só aparecem aqui as tarefas que valem nesse dia.</p>
        <ul className="divide-y divide-line">
          {dueTasks.map((t) => {
        ...
        {[...activeEmps, ...(canTargetGerentes ? gerentes : [])].map((emp) => {
          ...
          return (
            <div key={emp.id} className="mb-4 last:mb-0">
              <p className="text-sm font-semibold text-navy flex items-center justify-between">
                {emp.full_name}
                <span className="text-purple">{formatBRL(total)}</span>
              </p>
              <ul className="divide-y divide-line mt-1">
                {empPrizes.map((p) => (
                  <li key={p.id} className="flex items-center justify-between py-1.5 text-sm">
                    <span className="text-muted">{p.description || "sem descrição"}</span>
                    <span className="flex items-center gap-3">
                      <span className="font-medium text-navy">{formatBRL(p.amount)}</span>
...
function Metas({ employees, goals, allocations, commissionSettings, settings, month, empresaId, lojaId, onChanged, viewerRole = "master_admin" }) {
  ...
  return (
    <div className="space-y-6">
      {canManage && (
        <div className="card">
          <p className="label mb-3 flex items-center gap-1.5"><Gift size={14} /> Premiação mensal da loja — {monthLabel(month)}</p>
            ...
            <div key={g.id} className="card">
              <div className="flex justify-between items-start flex-wrap gap-2">
                ...
                {canManage && !isEditing && editingValuesId !== g.id && (
                  <div className="flex items-center gap-3">
                    <button className="text-xs uppercase tracking-wider text-muted hover:text-navy flex items-center gap-1" onClick={() => startEditValues(g)}>
                      <Coins size={12} /> editar valores
                    </button>
                    ...
                    <button className="text-xs uppercase tracking-wider text-danger hover:text-red-700" onClick={() => removeGoal(g)}>
                      excluir
                    </button>
                  </div>
                )}
              ...
              {editingValuesId === g.id ? (
                ...
                <ul className="mt-3 text-xs sm:text-sm divide-y divide-line">
                  ...
                  {goalAllocs.length === 0 && <p className="text-[11px] text-muted py-1.5">Sem distribuição definida ainda.</p>}
                </ul>
              )}
