...
import { todayStr, firstDayOfMonth, monthLabel, isTaskDueOn, WEEKDAY_LABELS, daysElapsedInMonth } from "./date";
...
export default function EmpresaDashboard({ lojaId, empresaId, viewerRole = "master_admin", viewerId, tab = "atividades", month: monthProp, onOpenEmployee, onOpenGerente, atSub: atSubProp, onAtSubChange }) {
  ...
  // venda diária da loja (dashboard Placar) — soma de TODOS os colaboradores da loja no dia de
  // hoje, não só os da equipe de quem está olhando (mesmo o gerente vê a loja inteira aqui,
  // igual ao texto pedido: "soma da venda de todos os colaboradores naquele dia"). `entries` já
  // vem sem filtro de mês (loadAll busca todos os sales_entries da loja), então dá pra filtrar
  // por data direto, sem query nova. Só faz sentido mostrar "hoje" enquanto o mês em exibição for
  // o mês corrente — mês fechado sendo revisado no MonthNav não deve mostrar um "hoje" de um mês
  // diferente do que está na tela (mesmo padrão de isCurrentMonth do herocard do colaborador).
  const isCurrentMonth = month === firstDayOfMonth(today);
  const storeSoldToday = entries
...
function Placar({ scoreboard, teamPct, settings, month, viewerRole = "master_admin", goals = [], storeMetaTotal = 0, storeSoldTotal = 0, storeSoldToday = 0, isCurrentMonth = true }) {
  ...
  return (
    <div className="space-y-6">
      {/* Venda diária da loja — gerente, supervisor e sócio (quando a loja está selecionada).
          Soma de todos os colaboradores da loja no dia, não só da equipe de quem olha. Some
          quando o mês em exibição não é o corrente (revisão de mês fechado no MonthNav) pra não
          mostrar "hoje" descolado do período sendo visto. */}
      {(viewerRole === "gerente" || isSupervisorView) && isCurrentMonth && (
        <div className="card-dark animate-pop">
          <p className="label-dark mb-2 flex items-center gap-1.5"><Coins size={14} className="text-goldlight" /> Venda de hoje — loja</p>
          <AutoFitText className="text-3xl sm:text-4xl font-extrabold text-white">{formatBRL(storeSoldToday)}</AutoFitText>
          <p className="text-xs text-white/50 mt-1">Soma das vendas de todos os colaboradores da loja hoje.</p>
        </div>
      )}
      ...
      {viewerRole === "master_admin" && leader && (
        <div className="card-dark animate-pop">
          ...
          <p className="text-sm text-white/50">{formatBRL(leader.sold)} vendido no mês</p>
        </div>
      )}
              ...
              <li key={g.id} className="row-card justify-between flex-wrap">
                ...
                {editingGerenteId !== g.id && (
                  <div className="flex items-center gap-1 shrink-0">
                    ...
                    <button onClick={() => setConfirmAction({ type: "delete", target: g })} title="Excluir" aria-label="Excluir" className="p-1.5 rounded-lg text-white/60 hover:text-danger hover:bg-white/10 transition-colors">
                      <Trash2 size={14} />
                    </button>
                  </div>
                )}
            ...
            <li key={emp.id} className="row-card justify-between flex-wrap">
              ...
              {editingId !== emp.id && canEdit && (
                <div className="flex items-center gap-1 shrink-0">
                  ...
                  <button onClick={() => setConfirmAction({ type: "delete", target: emp })} title="Excluir" aria-label="Excluir" className="p-1.5 rounded-lg text-white/60 hover:text-danger hover:bg-white/10 transition-colors">
                    <Trash2 size={14} />
                  </button>
                </div>
              )}
...
export function Premiacoes({ employees, gerentes = [], viewerRole = "master_admin", prizes, month, empresaId, lojaId, onChanged }) {
  const notifySaved = useSavedNotice();
  const canEdit = viewerRole !== "leitor";
  const canTargetGerentes = viewerRole !== "gerente" && gerentes.length > 0;
  const activeEmps = employees.filter((e) => e.active);
  ...
  async function addPrize(e) {
    ...
    if (replicateAll) {
      ...
      await supabase.from("employee_prizes").insert(rows);
    } else {
      await supabase.from("employee_prizes").insert({ employee_id: empId, ...baseFields });
    }
    setAmount("");
    setDescription("");
    setSaving(false);
...
function Lancamentos({ employees, entries, empresaId, lojaId, onChanged, viewerRole = "master_admin" }) {
  ...
  // onde é o gerente escolhendo por qual colaborador lançar.
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="card-dark">
        <p className="label-dark mb-3">Histórico</p>
        {myEntries.length === 0 ? (
          ...
          <ul>
            {myEntries.map((en) => (
              <li key={en.id} className="row-card justify-between">
                ...
                <span className="font-bold text-goldlight text-xs sm:text-sm shrink-0 whitespace-nowrap">{formatBRL(en.daily_amount)}</span>
              </li>
            ))}
          </ul>
        )}
