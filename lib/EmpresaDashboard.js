...
export default function EmpresaDashboard({ lojaId, empresaId }) {
    ...
    <div className="space-y-6">
      ...
      {tab === "metas" && (
        <div className="space-y-6">
          ...
          {metaSub === "metas" && (
            <Metas employees={employees} goals={goals} allocations={allocations} month={month} empresaId={empresaId} lojaId={lojaId} onChanged={refresh} />
          )}
          {metaSub === "lancamentos" && (
...
function Metas({ employees, goals, allocations, month, empresaId, lojaId, onChanged }) {
  ...
  async function createGoal(e) {
    ...
    const { data: goal, error } = await supabase
      .from("sales_goals")
      .insert({ month, name: name.trim(), store_total: Number(total), distribution_mode: mode, created_by: session.user.id, empresa_id: empresaId, loja_id: lojaId })
      .select()
      .single();
    ...
    if (mode === "equal") {
      ...
      const rows = activeEmps.map((emp) => ({
        ...
        employee_id: emp.id,
        amount,
        percentage: activeEmps.length ? 100 / activeEmps.length : 0,
      ...
      const initial = {};
      const initialComm = {};
      employees.filter((e) => e.active).forEach((emp) => { initial[emp.id] = ""; initialComm[emp.id] = commissionPct || ""; });
      setCustomVals(initial);
      setCustomCommission(initialComm);
    }
  }
  ...
  function openEditor(goal) {
    ...
    employees.filter((e) => e.active).forEach((emp) => {
      ...
      existingComm[emp.id] = a?.commission_pct != null ? String(a.commission_pct) : "";
    });
    ...
    <div className="space-y-6">
      <div className="card">
        ...
        <form onSubmit={createGoal} className="grid sm:grid-cols-5 gap-3 items-end">
          ...
          <div>
            <label className="label">% comissão{mode === "custom" ? " (padrão)" : ""}</label>
        ...
        {mode === "custom" && (
          <p className="text-[11px] text-muted mt-2">No modo custom, você define a meta e a comissão de cada colaborador logo em seguida.</p>
        )}
      </div>
