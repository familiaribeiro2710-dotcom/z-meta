    ...
    <div className="space-y-6">
      <div className="card">
        ...
        {canManage ? (
          ...
          <p className="text-sm text-navy">{Number(commissionSettings.non_achievement_colaborador_pct) || 0}% colaborador · {Number(commissionSettings.non_achievement_gerente_pct) || 0}% gerente</p>
        )}
      </div>
      ...
      {canManage ? (
        ...
        <div className="card">
          <p className="text-sm text-navy flex items-center gap-1.5"><Target size={14} className="text-purple" /> Metas e comissões definidas pelo sócio/supervisor.</p>
          <p className="text-xs text-muted mt-1">Você acompanha aqui como está a distribuição e as taxas de cada meta da loja.</p>
        </div>
      )}
