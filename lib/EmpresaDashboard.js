...
export default function EmpresaDashboard({ lojaId, empresaId, viewerRole = "master_admin", viewerId, tab = "atividades", month: monthProp, onOpenEmployee, onOpenGerente, atSub: atSubProp, onAtSubChange }) {
  ...
  const loadAll = useCallback(async () => {
    ...
    // placar/ranking não pode sumir com dado histórico real de quem foi desativado (bug real
    // corrigido em 2026-09-03: filtrar só por `emp.active` fazia o placar de um MÊS PASSADO mudar
    // retroativamente depois de uma desativação, porque active reflete o status ATUAL, não o status
    // durante aquele mês). Regra: entra no placar quem está ativo hoje OU quem teve algum dado
    // real no mês sendo visualizado (vendeu, teve tarefa concluída ou advertência) — só some quem
    // está inativo e não tem nada registrado naquele mês.
    ...
    }).filter((b) => b.employee.active || b.sold > 0 || b.warnings > 0 || b.completed > 0);
    setScoreboard(board);
  ...
  // Chamado SÓ quando um colaborador novo é criado — a meta do mês em distribuição "igual" se
  // redistribui automaticamente pra incluir o recém-chegado. 2026-09-03: bug real corrigido —
  // antes essa mesma função também rodava ao ATIVAR/DESATIVAR/EXCLUIR colaborador, e passava a
  // redistribuir o store_total só entre quem sobrou, mudando a meta individual de todo mundo sem
  // aviso nenhum (caso real: Felipe desativou 1 pessoa na ArmyBR e a meta de todos os outros mudou
  // na hora). Desativar/excluir agora só recarrega os dados (loadAll) — a alocação de quem saiu
  // fica órfã no banco (nunca somada em nenhum total, já que os cálculos de meta/comissão sempre
  // filtram por colaborador ativo), sem afetar o valor dos que continuam. Se quiser redistribuir de
  // propósito depois de uma saída, o caminho é a UI de "Editar distribuição" da própria meta.
...
function Placar({ scoreboard, settings, month, viewerRole = "master_admin", goals = [], storeMetaTotal = 0, storeSoldTotal = 0, entries = [], prizes = [], prizeValidations = [], empresaId, lojaId, onChanged }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {calendarOpen && (
        <div
          className="fixed inset-0 z-50 overflow-y-auto bg-navyfixed/70 p-4 pt-[calc(env(safe-area-inset-top)+1rem)]"
          ...
          <div className="min-h-full flex items-start sm:items-center justify-center">
            <div className="card max-w-sm w-full my-8 sm:my-0 animate-bounce-in border-gold/30" onClick={(e) => e.stopPropagation()}>
                  ...
                  return (
                    <button
                      ...
                      className={`aspect-square rounded-xl border-2 flex flex-col items-center justify-center gap-0.5 transition-colors ${
                        isSelected ? "bg-navyfixed border-navyfixed" : isToday ? "border-gold bg-surface" : "border-line bg-surface"
              ...
              <div className="flex items-center justify-center gap-4 mt-4 text-[10px] text-muted font-semibold">
                ...
                <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-[3px] bg-navyfixed inline-block" /> selecionado</span>
              </div>
            </div>
          </div>
        </div>
                ...
                <li key={b.employee.id} className="py-3 first:pt-0 last:pb-0">
                  ...
                  {canValidatePrize && (
                    <div className="flex items-center justify-between gap-2 mt-2">
                      <span className={`text-[11px] font-bold flex items-center gap-1 ${validated ? "text-success" : "text-muted"}`}>
                      ...
                      <div className="flex items-center gap-1.5 shrink-0">
                        <button
                          ...
                          className={`w-[30px] h-[30px] rounded-full border-2 flex items-center justify-center transition disabled:opacity-60 ${
                            validated ? "bg-success border-success text-white" : "border-line text-muted hover:border-success/50 hover:text-success hover:bg-success/5"
                          }`}
                        >
                        ...
                        <button
                          ...
                          className={`w-[30px] h-[30px] rounded-full border-2 flex items-center justify-center transition disabled:opacity-60 ${
                            !validated ? "bg-danger border-danger text-white" : "border-line text-muted hover:border-danger/50 hover:text-danger hover:bg-danger/5"
                          }`}
                        >
...
export function Colaboradores({ employees, gerentes = [], viewerRole = "master_admin", empresaId, lojaId, onChanged, onEmployeeCreated, onOpenEmployee, onOpenGerente }) {
  const notifySaved = useSavedNotice();
  ...
  // 2026-09-03, pedido do Felipe: a lista de equipe some os desativados por padrão (só ativos),
  // com um toggle pra mostrar também quem foi desativado — evita a lista poluir com gente que já
  // saiu, mas mantém acesso fácil pra reativar quando precisar.
  ...
  async function handleCreate(e) {
    ...
    // colaborador novo entra automaticamente na divisão "igual pra todos" das metas do mês
    // (onEmployeeCreated recalcula) — ativar/desativar/editar/excluir NÃO mexe em meta nenhuma,
    // só recarrega os dados (onChanged), ver nota em recalcEqualGoalAllocations.
    (onEmployeeCreated || onChanged)();
  }
    ...
    <div className="space-y-6">
              ...
              <li key={g.id} className="row-card justify-between flex-wrap">
                {editingGerenteId === g.id ? (
                  <div className="flex items-center gap-2 flex-1 flex-wrap">
                    ...
                    <button onClick={() => { setEditingGerenteId(null); setEditGerenteMsg(""); }} title="Cancelar" aria-label="Cancelar" className="p-1.5 rounded-lg text-white/60 hover:text-white hover:bg-surface/10 transition-colors">
                      <X size={16} />
                    </button>
                ...
                {editingGerenteId !== g.id && (
                  <div className="flex items-center gap-1 shrink-0">
                    <button onClick={() => startEditGerente(g)} title="Editar" aria-label="Editar" className="p-1.5 rounded-lg text-white/60 hover:text-goldlight hover:bg-surface/10 transition-colors">
                      <Pencil size={14} />
                    </button>
                    <button
                      ...
                      className="p-1.5 rounded-lg text-white/60 hover:text-warn hover:bg-surface/10 transition-colors"
                    >
                      <KeyRound size={14} />
                    </button>
                    <button
                      ...
                      className={`p-1.5 rounded-lg hover:bg-surface/10 transition-colors ${g.active ? "text-white/60 hover:text-white" : "text-danger"}`}
                    >
                      <Power size={14} />
                    </button>
                    <button onClick={() => requestDelete(g)} title="Excluir" aria-label="Excluir" className="p-1.5 rounded-lg text-white/60 hover:text-danger hover:bg-surface/10 transition-colors">
                      <Trash2 size={14} />
                    </button>
                  </div>
                )}
      ...
      <div className="card-dark">
        <div className="flex items-center justify-between gap-2 mb-3 flex-wrap">
          <p className="label-dark mb-0 flex items-center gap-1.5"><Users size={14} className="text-goldlight" /> Equipe ({visibleEmployees.length})</p>
          {inactiveCount > 0 && (
            <button
              type="button"
              onClick={() => setShowInactive((v) => !v)}
              className="text-[11px] font-bold text-white/60 hover:text-white transition-colors underline underline-offset-2 decoration-white/30"
            >
              {showInactive ? "Ocultar desativados" : `Mostrar desativados (${inactiveCount})`}
            </button>
          )}
        </div>
        <ul>
            ...
            <li key={emp.id} className="row-card justify-between flex-wrap">
              {editingId === emp.id ? (
                <div className="flex items-center gap-2 flex-1 flex-wrap">
                  ...
                  <button onClick={() => { setEditingId(null); setEditMsg(""); }} title="Cancelar" aria-label="Cancelar" className="p-1.5 rounded-lg text-white/60 hover:text-white hover:bg-surface/10 transition-colors">
                    <X size={16} />
                  </button>
              ...
              {editingId !== emp.id && canEdit && (
                <div className="flex items-center gap-1 shrink-0">
                  <button onClick={() => startEdit(emp)} title="Editar" aria-label="Editar" className="p-1.5 rounded-lg text-white/60 hover:text-goldlight hover:bg-surface/10 transition-colors">
                    <Pencil size={14} />
                  </button>
                  <button
                  ...
                  <button
                    ...
                    className={`p-1.5 rounded-lg hover:bg-surface/10 transition-colors ${emp.active ? "text-white/60 hover:text-white" : "text-danger"}`}
                  >
                    <Power size={14} />
                  </button>
                  <button onClick={() => requestDelete(emp)} title="Excluir" aria-label="Excluir" className="p-1.5 rounded-lg text-white/60 hover:text-danger hover:bg-surface/10 transition-colors">
                    <Trash2 size={14} />
                  </button>
                </div>
              )}
          ...
          {visibleEmployees.length === 0 && (
            <p className="text-sm text-white/50 py-2">
              {employees.length === 0 ? "Nenhum colaborador cadastrado ainda." : "Nenhum colaborador ativo — todos estão desativados."}
            </p>
          )}
        </ul>
      </div>
      ...
      {supportBlockOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-navyfixed/70 p-6" onClick={() => setSupportBlockOpen(false)}>
...
export function Tarefas({ employees, gerentes = [], viewerRole = "master_admin", tasks, empresaId, lojaId, onChanged, isConsorcio = false }) {
  ...
  // colaborador/gerente desativado continua selecionável (histórico de tarefas dele precisa
  // continuar visível/editável), só não pode RECEBER tarefa nova.
  const selectedPerson = employees.find((e) => e.id === selected) || gerentes.find((g) => g.id === selected);
  const selectedIsInactive = !!selectedPerson && !selectedPerson.active;
  ...
  async function addTask(e) {
    ...
    if (!replicateAll && selectedIsInactive) return;
    ...
    <div className="space-y-6">
      ...
      <div className="card">
        ...
        <form onSubmit={addTask} className="space-y-3 mb-4">
          {!replicateAll && selectedIsInactive && (
            <p className="text-xs text-danger">Colaborador desativado — não é possível atribuir tarefa nova. O histórico dele continua abaixo.</p>
          )}
          <div className="flex gap-3">
            <input className="input" placeholder="nome da tarefa" value={title} onChange={(e) => setTitle(e.target.value)} maxLength={40} />
            <button className="btn whitespace-nowrap" type="submit" disabled={!replicateAll && selectedIsInactive}>Adicionar</button>
          </div>
...
export function Advertencias({ employees, gerentes = [], viewerRole = "master_admin", warnings, settings, today, empresaId, lojaId, onChanged, onSaveSettings }) {
  ...
  // colaborador/gerente desativado continua selecionável (histórico de advertências dele precisa
  // continuar visível), só não pode RECEBER advertência nova.
  const selectedPerson = employees.find((e) => e.id === selected) || gerentes.find((g) => g.id === selected);
  const selectedIsInactive = !!selectedPerson && !selectedPerson.active;

  async function addWarning(e) {
    e.preventDefault();
    if (!selected || !reason.trim() || selectedIsInactive) return;
    const { data: { session } } = await supabase.auth.getSession();
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="card">
        ...
        {selectedIsInactive && (
          <p className="text-xs text-danger mb-3">Colaborador desativado — não é possível registrar advertência nova. O histórico dele continua abaixo.</p>
        )}
        <form onSubmit={addWarning} className="grid sm:grid-cols-3 gap-3 items-end">
          <div>
            <label className="label">Data</label>
            <input type="date" className="input date-input" value={date} onChange={(e) => setDate(e.target.value)} disabled={selectedIsInactive} />
          </div>
          <div className="sm:col-span-2">
            <label className="label">Motivo</label>
            <input className="input" value={reason} onChange={(e) => setReason(e.target.value)} placeholder="ex: atraso, falha de atendimento…" disabled={selectedIsInactive} />
          </div>
          <div className="sm:col-span-3">
            <button className="btn" type="submit" disabled={selectedIsInactive}>Registrar</button>
          </div>
        </form>
  ...
  return (
    <div className="space-y-6">
            ...
            <div className="flex flex-wrap gap-1.5">
              {[
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded-full text-xs font-bold border transition ${recipientMode === opt.key ? "bg-navyfixed text-white border-navyfixed" : "bg-surface text-muted border-line hover:border-navy/40"}`}
                >
                  {opt.label}
                </button>
              ))}
              ...
              <div className="flex flex-wrap gap-1.5">
                {["mes", "estagio", "semana", "quinzena"].map((pt) => (
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-full text-xs font-bold border transition ${extraPeriodType === pt ? "bg-navyfixed text-white border-navyfixed" : "bg-surface text-muted border-line hover:border-navy/40"}`}
                  >
                ...
                <div className="flex flex-wrap gap-1.5">
                  {goalPeriods(month, extraPeriodType).map((p, idx) => (
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-full text-xs font-bold border transition ${extraPeriodIdx === idx ? "bg-gold text-navyfixed border-gold" : "bg-surface text-muted border-line hover:border-navy/40"}`}
                    >
      ...
      <div className="card">
        ...
        {/* 2026-09-03: histórico usa `employees` (todos, não só ativos) — colaborador desativado não
            pode sumir com premiação já lançada pra ele; `if (!empPrizes.length) return null` abaixo
            já filtra quem não tem nada, então isso só afeta quem realmente tinha premiação lançada. */}
        {[...employees, ...(canTargetGerentes ? gerentes : [])].map((emp) => {
...
// Precisa viver FORA de Metas (não como função aninhada no corpo dela): componente redefinido a
// cada render do pai é uma identidade nova pro React, que desmonta e remonta o <input> inteiro —
// no iPad/mobile isso fecha o teclado a cada dígito digitado (bug real reportado pelo Felipe,
// 2026-09-04). Mesmo motivo por trás de PacingFields logo abaixo.
...
function Metas({ employees, goals, allocations, commissionSettings, settings, month, empresaId, lojaId, onChanged, viewerRole = "master_admin", soldTotal = 0 }) {
  ...
  async function createGoal(e) {
    ...
    if (!error && inserted) {
      ...
      if (rows.length) {
        // Blindagem defensiva (2026-09-04): bug real equivalente em consorcio_goal_allocations
        // (índice único faltando) fazia esse upsert falhar em silêncio, sem nunca checar o erro —
        // a tela mostrava "salvo" mesmo sem distribuição nenhuma. sales_goal_allocations já tem o
        // índice certo, mas checar o erro aqui evita essa classe de bug se algo mudar no futuro.
        const { error: allocError } = await supabase.from("sales_goal_allocations").upsert(rows, { onConflict: "goal_id,employee_id" });
        if (allocError) {
...
function OnlineTab({ employees, onlineGoals, onlineSales, activationSettings, lojaId, empresaId, month, viewerRole, onChanged }) {
  ...
  // ativo hoje OU vendeu algo no mês visualizado — mesma regra do placar (ver EmpresaDashboard,
  // loadAll): ranking de mês fechado não pode sumir a venda de quem foi desativado depois.
  const ranking = employees
    .filter((e) => e.active || totalsByEmp[e.id] > 0)
...
function OnlineLeadsTab({ employees, leads }) {
  ...
  // 2026-09-15 (pedido do Felipe): "Exportar Excel" na aba Leads — mesmo padrão já usado na aba
  // Leads de consórcio (ConsorcioDashboard.js), exportando exatamente o que está filtrado na tela.
  async function exportExcel() {
    setExporting(true);
    try {
      const XLSX = await import("xlsx");
      const leadRows = filtered.map((l) => ({
        Colaborador: empById[l.employee_id]?.full_name || "—",
        Cliente: l.client_name,
        Telefone: l.client_phone,
        Código: l.client_code || "",
        Data: l.contact_date || "",
      }));
      ...
      <div className="card overflow-x-auto">
        <div className="flex items-center justify-between gap-2 mb-3 flex-wrap">
          <div>
            ...
            {filtroAtivo && (
              <p className="text-[11px] text-muted mt-0.5">Total cadastrado (sem filtro): {scoped.length}</p>
            )}
          </div>
          {filtered.length > 0 && (
            <button
...
function Lancamentos({ employees, entries, month, empresaId, lojaId, onChanged, viewerRole = "master_admin" }) {
  const notifySaved = useSavedNotice();
  const canEdit = viewerRole !== "leitor";
  // 2026-09-05 (pedido do Felipe): colaborador desativado sumiu de vez desse seletor — antes
  // continuava selecionável só pra dar acesso ao histórico de vendas dele (lançar venda nova já
  // ficava bloqueado via `selectedIsInactive` abaixo), mas Felipe decidiu que prefere não ver
  // ninguém desligado aqui nem pra isso; pra corrigir um lançamento antigo de alguém que saiu, a
  // conta precisa ser reativada primeiro. `selectedIsInactive` continua existindo como defesa
  // (ex.: alguém foi desativado em outra aba enquanto essa tela já estava aberta com ela selecionada).
  const activeEmployees = employees.filter((e) => e.active);
  const [selected, setSelected] = useState(activeEmployees[0]?.id || "");
  const selectedPerson = employees.find((e) => e.id === selected);
  const selectedIsInactive = !!selectedPerson && !selectedPerson.active;
  const today = todayStr();
  const [date, setDate] = useState(today);
  ...
  async function save(e) {
    e.preventDefault();
    if (!selected || value === "" || selectedIsInactive) return;
    if (Number(value) === 0) {
  ...
  return (
    <div className="space-y-6">
      <div className="card">
        <label className="label">Colaborador</label>
        <SelectField className="w-full" value={selected} onChange={(e) => setSelected(e.target.value)}>
          {activeEmployees.map((emp) => <option key={emp.id} value={emp.id}>{emp.full_name}</option>)}
        </SelectField>
      </div>
      ...
      <div className="card">
        ...
        {selectedIsInactive && (
          <p className="text-xs text-danger mb-3">Colaborador desativado — não é possível lançar venda nova. O histórico dele continua no card abaixo.</p>
        )}
        <form onSubmit={save} className="grid sm:grid-cols-3 gap-3 items-end">
          <div>
            <label className="label">Dia da venda</label>
            <input type="date" className="input date-input" value={date} max={todayStr()} onChange={(e) => setDate(e.target.value)} disabled={selectedIsInactive} />
          </div>
          <div>
            <label className="label">Valor vendido nesse dia</label>
            <CurrencyInput value={value} onChange={setValue} disabled={selectedIsInactive} />
          </div>
          <div>
            <button className="btn w-full" type="submit" disabled={saving || selectedIsInactive}>{saving ? "Salvando…" : "Salvar"}</button>
          </div>
        </form>
      </div>
      )}
              ...
              editingEntryId === en.id ? (
                ...
                <li key={en.id} className="row-card justify-between">
                  ...
                  <span className="flex items-center gap-2 shrink-0">
                    ...
                    {canEdit && (
                      <>
                        <button type="button" title="Editar" aria-label="Editar" className="p-1.5 rounded-lg text-white/50 hover:text-white hover:bg-surface/10 transition-colors" onClick={() => startEdit(en)}>
                          <Pencil size={13} />
                        </button>
