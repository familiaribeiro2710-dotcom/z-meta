...
function Placar({ scoreboard, teamPct, settings, month, viewerRole = "master_admin", goals = [], storeMetaTotal = 0, storeSoldTotal = 0, entries = [] }) {
  ...
  // Venda diária da loja — navegável dia a dia com as setinhas do DateNav, independente do
  // mês/MonthNav em exibição no resto da tela (dá pra folhear dias anteriores mesmo olhando um
  // mês fechado). `entries` já vem sem filtro de data (loadAll busca toda a loja), então filtra
  // direto sem query nova. Soma de TODOS os colaboradores da loja, não só da equipe de quem olha.
  const today = todayStr();
      ...
      {(viewerRole === "gerente" || isSupervisorView) && (
        <div className="card-dark animate-pop">
          <div className="flex items-center justify-between gap-2 flex-wrap mb-2">
            ...
            <DateNav date={viewDate} onChange={setViewDate} maxDate={today} dark />
          </div>
