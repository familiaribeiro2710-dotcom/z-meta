...
import { todayStr, firstDayOfMonth, nextMonth, monthLabel, isTaskDueOn, WEEKDAY_LABELS, daysElapsedInMonth, daysInMonth } from "./date";
    ...
    <div className="space-y-6">
      {tab === "atividades" && (
        <div className="space-y-6">
          ...
          {atSub === "premiacoes" && (
            <Premiacoes employees={employees} gerentes={gerentes} viewerRole={viewerRole} prizes={prizes} month={month} empresaId={empresaId} lojaId={lojaId} settings={settings} onChanged={refresh} />
          )}
        </div>
      )}
...
function Placar({ scoreboard, teamPct, settings, month, viewerRole = "master_admin", goals = [], storeMetaTotal = 0, storeSoldTotal = 0, entries = [] }) {
  ...
  const activeGoalObj = goals.find((g) => g.id === activeGoalId) || null;
    ...
    <div className="space-y-6">
      ...
      {isSupervisorView ? (
        <div className="card animate-pop">
          ...
          {activeGoalObj?.non_monetary_prize && (
            <p className="text-xs font-bold text-navy bg-pink/10 rounded-lg py-2 px-3 mt-3 flex items-center gap-1.5">
              <PartyPopper size={13} className="shrink-0" /> Prêmio da equipe: {activeGoalObj.non_monetary_prize}
            </p>
          )}
        </div>
      ) : (
              ...
              <li key={g.id} className="row-card justify-between flex-wrap">
                ...
                {editingGerenteId !== g.id && (
                  <div className="flex items-center gap-1 shrink-0">
                    <button onClick={() => startEditGerente(g)} title="Editar" aria-label="Editar" className="p-1.5 rounded-lg text-white/60 hover:text-purplelight hover:bg-white/10 transition-colors">
                      <Pencil size={14} />
                    </button>
                    <button
          ...
          {employees.map((emp) => (
            <li key={emp.id} className="row-card justify-between flex-wrap">
              ...
              {editingId !== emp.id && canEdit && (
                <div className="flex items-center gap-1 shrink-0">
                  <button onClick={() => startEdit(emp)} title="Editar" aria-label="Editar" className="p-1.5 rounded-lg text-white/60 hover:text-purplelight hover:bg-white/10 transition-colors">
                    <Pencil size={14} />
                  </button>
                  <button
...
export function Premiacoes({ employees, gerentes = [], viewerRole = "master_admin", prizes, month, empresaId, lojaId, settings, onChanged }) {
  const notifySaved = useSavedNotice();
  const canEdit = viewerRole !== "leitor";
  const canTargetGerentes = viewerRole !== "gerente" && gerentes.length > 0;
  const activeEmps = employees.filter((e) => e.active);
...
function Metas({ employees, goals, allocations, commissionSettings, settings, month, empresaId, lojaId, onChanged, viewerRole = "master_admin" }) {
  ...
  // Prêmio não-monetário (2026-08-04, pedido do Felipe, paridade com consórcio): gerente não
  // gerencia meta (valor/comissão/distribuição/exclusão continuam só sócio/supervisor/master), mas
  // precisa poder escolher em qual meta cadastrada vale o prêmio não-monetário da equipe —
  // permissão só pra esse campo específico.
  const canEditPrize = canManage || viewerRole === "gerente";
  ...
  // Prêmio não-monetário — meta que, além (ou em vez) da comissão, dá direito a algo tipo festa/
  // jantar/viagem pra equipe inteira da loja (não é individual). Texto livre, opcional.
  const [premioNaoMonetario, setPremioNaoMonetario] = useState("");
  const [editingPrizeId, setEditingPrizeId] = useState(null);
  const [editPrizeText, setEditPrizeText] = useState("");
  const [savingPrizeText, setSavingPrizeText] = useState(false);

  function startEditPrize(g) {
    setEditingPrizeId(g.id);
    setEditPrizeText(g.non_monetary_prize || "");
  }

  async function savePrizeText(goalId) {
    setSavingPrizeText(true);
    await supabase.from("sales_goals").update({ non_monetary_prize: editPrizeText.trim() || null }).eq("id", goalId);
    setSavingPrizeText(false);
  ...
  async function createGoal(e) {
    ...
    const { data: inserted, error } = await supabase
      ...
      .insert({
        ...
        non_monetary_prize: premioNaoMonetario.trim() || null,
        created_by: session.user.id,
        empresa_id: empresaId,
        loja_id: lojaId,
      })
    ...
    setName(""); setTotal(""); setCommColab(""); setCommGerente(""); setPremioNaoMonetario(""); setNewGoalOpen(false);
    notifySaved();
    onChanged();
  }
            ...
            <>
              <form onSubmit={createGoal} className="grid sm:grid-cols-5 gap-3 items-end mt-3">
                ...
                <div className="sm:col-span-5">
                  <label className="label">Prêmio não-monetário (opcional)</label>
                  <input className="input" placeholder="ex: jantar de confraternização, viagem, festa da equipe" value={premioNaoMonetario} onChange={(e) => setPremioNaoMonetario(e.target.value)} maxLength={120} />
                  <p className="text-[11px] text-muted mt-1">Independente da comissão — vale pra toda a equipe da loja, não é individual.</p>
                </div>
                <div>
              ...
              {editingPrizeId === g.id ? (
                <div className="flex items-center gap-2 mt-3">
                  <input
                    ...
                    value={editPrizeText}
                    onChange={(e) => setEditPrizeText(e.target.value)}
                    maxLength={120}
                    autoFocus
                  />
                  <button className="btn !py-1.5 !px-3 !text-xs shrink-0" disabled={savingPrizeText} onClick={() => savePrizeText(g.id)}>
                    {savingPrizeText ? "Salvando…" : "Salvar"}
                  </button>
                  <button type="button" title="Cancelar" aria-label="Cancelar" className="p-1.5 rounded-lg text-muted hover:text-navy hover:bg-line/60 transition-colors shrink-0" onClick={() => setEditingPrizeId(null)}>
                    <X size={16} />
                  </button>
                </div>
                ...
                <button
                  type="button"
                  onClick={() => canEditPrize && startEditPrize(g)}
                  className="w-full flex items-center gap-1.5 mt-3 px-2.5 py-1.5 rounded-lg bg-pink/10 text-left"
                  title={canEditPrize ? "Editar prêmio" : undefined}
                >
                  <PartyPopper size={13} className="text-pink shrink-0" />
                  <span className="text-[11px] font-semibold text-navy truncate">Prêmio da equipe: {g.non_monetary_prize}</span>
                  {canEditPrize && <Pencil size={11} className="text-muted shrink-0 ml-auto" />}
                </button>
              ) : (
                canEditPrize && (
                  <button type="button" onClick={() => startEditPrize(g)} className="text-[11px] text-muted hover:text-navy mt-3 flex items-center gap-1">
                    <PartyPopper size={12} /> + adicionar prêmio não-monetário
                  </button>
                )
              )}
