              ...
              <li key={g.id} className="flex items-center justify-between gap-3 py-2.5 text-sm">
                {onOpenGerente ? (
                  <button type="button" onClick={() => onOpenGerente(g)} className="text-left hover:opacity-75 transition-opacity" title="Ver como este gerente">
                    ...
                    <p className="text-xs text-muted">usuário: {g.username} · {employees.filter((e) => e.gerente_id === g.id).length} colaborador(es)</p>
                  </button>
                ) : (
                  <div>
                    <p className="font-medium text-navy">{g.full_name}</p>
                    <p className="text-xs text-muted">usuário: {g.username}</p>
                  </div>
                )}
              </li>
            ))}
