...
function Metas({ employees, goals, allocations, month, empresaId, lojaId, onChanged }) {
  ...
  async function saveCustom(goalId) {
    const rows = Object.entries(customVals)
      .filter(([, v]) => v !== "")
      .map(([employee_id, v]) => ({ goal_id: goalId, employee_id, amount: Number(v), empresa_id: empresaId, loja_id: lojaId }));
    if (rows.length) {
...
function Lancamentos({ employees, entries, empresaId, lojaId, onChanged }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="card">
        <p className="label mb-3 flex items-center gap-1.5"><FileText size={14} /> Corrigir / lançar valor vendido</p>
